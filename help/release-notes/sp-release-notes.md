...
* AEM Assets現在可透過[!DNL Adobe I/O]使用Brand Portal進行設定。
...
* 新增支援使用BUNSETSU方法分隔日文句子，並在適當位置加上分行(NPR-31836)。
...
* PDF檔案的Experience Manager詳細資訊頁面在Dynamic Media Scene7模式下執行時沒有顯示動作按鈕(NPR-32316)。
...
* PSD資產擷取造成CPU尖峰和無反應的Experience Manager製作例項(NPR-32165)。
...
* Dynamic Media視訊在Scene7雲端設定放置於私人資料夾`/conf`而非`/conf/global`時無法上傳(NPR-31779)。
...
* 在Experience Manager雲端服務中設定Adobe Analytics時，選取&#x200B;**報表來源**&#x200B;後，會隱藏&#x200B;**Company**&#x200B;和&#x200B;**Reporting**&#x200B;套裝下拉式功能表(NPR-31729)。
...
* 檔案安全性：從ES4升級至AEM 6.3後，開啟受保護的PDF檔案時發生問題(NPR-32170)。

* 文檔服務：嘗試使用toPDFA轉換方法將PDF檔案轉換為PDF/A檔案時，系統會顯示錯誤訊息(NPR-32663)。

* 文檔服務：在PDF檔案上套用Reader延伸模組服務時出現例外狀況(NPR-32639)。
...
AEM 6.4.7.0為重要更新，包含效能、穩定性、安全性和重要客戶修正，以及自2018年4月&#x200B;**全面發行AEM 6.4以來所發行的增強功能。**
...
* 已在&#x200B;**DAM list**&#x200B;檢視和&#x200B;**List**&#x200B;檢視的資產搜尋結果中新增可排序的建立日期欄(NPR-31311)。
* 在&#x200B;**清單**&#x200B;檢視中，允許根據&#x200B;**名稱**&#x200B;欄進行資產排序。
* 重新處理和批次上傳的批次大小和工作流程步驟逾時現在可從Dynamic Media的UI設定。
* 在Scene 7雲端設定中，`pdfBrochure`已設為false，以在IPS儲存記憶體。

##### 資產 {#assets-6470}

**產品增強功能**

* `dam-handler`套件組合支援的API套件`package com.day.cq.dam.handler.standard.msoffice`匯出版本已升級至6.0.0(CQ-4279059)。
如果您在自訂實作中使用套件`com.day.cq.dam.handler.standard.msoffice`，建議您使用最新的uber jar編譯您的`dam-handler`套件組合。
...
* 修改部分PDF檔案的標題時，未更新其中繼資料並儲存至PDF(NPR-31575)。
...
* PPT轉譯工作流程無法產生上傳之PPT檔案的轉譯，因為AEM 6.4無法更新至com.adobe.granite.poi 2.0.28版(CQ-4279059)。
...
* 副檔名為.docx和.xlsx的MS Office檔案包含JPEG影像，無法使用Tika剖析器剖析(NPR-31693)。
...
* 套用修補程式以修正HTML轉PDF失敗後，SOAP端點設定會重設(NPR-31309)。
...
AEM 6.4.6.0為重要更新，包含效能、穩定性、安全性和重要客戶修正，以及自2018年4月&#x200B;**全面發行AEM 6.4以來所發行的增強功能。**
...
* 資料夾的資產共用連結名稱中包含空格和`&`字元，某些資產會顯示空白灰色卡片。 NPR-29934：CQ-4270187 的 Hotfix
...
* 在`DMHybrid`模式中設定Dynamic MediaCloud Services會導致在Analytics中建立多個空的報表套裝，而AEM上未儲存任何報表套裝id，導致報表套裝重複。 CQ-4276855 的 Hotfix
...
* `onTime` 或是 `offTime` 如果AEM伺服器重新啟動，系統不會重新叫用資產上儲存的中繼資料屬性。NPR-30413：CQ-4272784 的 Hotfix
...
* (IE11)在RTF編輯器元件中貼上HTML內容時，若以defaultPasteMode = platext,HTML會貼上格式，因此defaultPasteMode沒有作用。 NPR-30045：GRANITE-26094 的 Hotfix
...
* 開啟內容片段時，先後開啟字元美元(`$`)和大括弧(`{`)，發生解析錯誤。 CQ-4270266 的 Hotfix
...
* 產品中使用的資產不會在資產的「參考」區段中顯示任何對產品的參考，如果移動資產，資產路徑也不會更新。 NPR-30542：CQ-4270247 的 Hotfix
...
* Acrobat印前檢查中出現PDF/A標籤遺失錯誤。 NPR-30594：CQ-4276032 的 Hotfix
...
* 嘗試透過「phantomjs」程式庫使用HTML至PDF後端服務時，權限遭拒例外狀況。 NPR-30456：CQ-4258077 的 Hotfix
* JBoss Management Console未顯示HTML轉PDF服務的maxReuseCount。 NPR-30303、NPR-30135：CQ-4273763 的 Hotfix

#### AEM 6.4.5.0 {#experience-manager-6450}

AEM 6.4.5.0為重要更新，包含效能、穩定性、安全性和重要客戶修正，以及自2018年4月&#x200B;**全面發行AEM 6.4以來所發行的增強功能。**
...
* 依預設，在AEM發佈執行個體上停用`AssetDownloadServlet` OSGi元件。 如需詳細資訊，請參閱[從AEM](/help/assets/download-assets-from-aem.md)下載資產。
...
* 為Microsoft Dynamics內部部署整合的Active Directory聯合身份驗證服務(ADFS)v3.0啟用了支援。 NPR-30003、NPR-29484：CQ-4270586 的 Hotfix
...
* 無效的PDF/A檔案會以isPDFA操作回報為有效的PDF/A。 NPR-29076：CQ-4261541 的 Hotfix
* 「表單」欄位沒有外觀字典時，PDF無法轉換為PDF/A-1b。 NPR-29534：CQ-4269618 的 Hotfix
* 透過輸出服務產生的PDF/A轉換無法透過Acrobat DC驗證。 NPR-29647：CQ-4270448 的 Hotfix
...
AEM 6.4.4.0為重要更新，包含效能、穩定性、安全性和重要客戶修正，以及自2018年4月&#x200B;**全面發行AEM 6.4以來所發行的增強功能。**
...
* AEM OOTB PDF處理常式卡在處理含外文字元的PDF時。 NPR-28778：CQ-4254234 的 Hotfix
...
* 從`aem/start.html`頁面對Omnisearch結果執行案頭動作時無法運作。 NPR-27242：CQ-4248176 的 Hotfix
...
在版本/640分支上安裝Forms管理套件後，一個套件組合會維持已安裝狀態。 CQ-4265410的Hotfix
已提交且其中附件的Forms在提交動作「叫用AEM Forms工作流程」且已勾選「啟用入口網站提交」的工作流程中不會顯示。 CQ-4263110 的 Hotfix
...
* an.lca中具有預設提交程式的HTML5 Forms在JBoss 7上無法運作。 NPR-28675：CQ-4243928 的 Hotfix
...
* 啟用OSGI設定&#x200B;**AEM工作流程撤回功能標幟**&#x200B;中的功能。 撤回動作應在設定標幟後終止工作流程例項。 NPR-26451：CQ-4259090 的 Hotfix
...
* 使用者無法編輯頁面屬性中封閉的使用者群組權限。 NPR-27138:CQ-4256089的Hotfix
...
* 啟用資源頁面中僅列出10個群組，即使已針對群組清單啟用分頁亦然。 NPR-26934：CQ-4252985 的 Hotfix
...
* isUseXFABullets()函式未在PrintChannelRenderOptions類中禁用，且可在用戶端SDK中使用。 CQ-4246583、CQ-4252700
...
* 在AEM Forms中以HTML呈現（在Internet Explorer或Chrome中）時退出日期欄位時，不會顯示退出通知。 NPR-26483：CQ-4239352 的 Hotfix
...
AEM 6.4.2.0為重要更新，包含2018年4月全面發行之AEM 6.4的效能、穩定性、安全性和重要客戶修正及增強功能。****
...
* Sites頁面編輯器新增了內容內編輯和合成的支援，其中包含在React或Angular中結合<a href="../sites-developing/spa-walkthrough.md">AEM SPA Editor JS SDK</a>建立的用戶端元件。
* 內容片段增強功能：新增可在文字欄位中加上注釋，並並排比較版本的功能。
* 新增與Adobe Stock](/help/assets/aem-assets-adobe-stock.md)的整合，讓使用者能直接從AEM使用者介面搜尋、預覽、儲存及授權Adobe Stock資產。 [如需詳細資訊，請參閱[將Adobe Stock資產與AEM Assets搭配使用](https://docs.adobe.com/content/help/en/experience-manager-learn/assets/creative-workflows/adobe-stock.html)。
* 新增Assets支援動態條件式中繼資料結構，以及為資產資料夾設定中繼資料結構的功能。
* 在每個元件中新增設定，以啟用/停用資料夾縮圖建立/更新功能。
* 頁面編寫時的影像編輯器增強功能。
* 社群中的回歸修正，若刪除所選答案，計分事件無法正確處理。
* 將索爾的搜索結果最大限制修改為MAX_INT-10000。
...
* HTML範本語言規格1.4功能已向後移植至AEM 6.4.2.0 NPR-24585
...
* Dynamic Media無法為EPS檔案生成格式副本。 CQ-4243688 的 Hotfix
...
* 新增在內部部署使用[交易報表功能](https://docs.adobe.com/content/help/en/experience-manager-64/forms/transaction-reports/transaction-reports-overview.html)的支援。
...
* 在HTML預覽中預填多行欄位時，不會顯示資料。 NPR-24549：CQ-4244212 的 Hotfix
...
* PDF預覽URL在發佈時不顯示交易報表。 CQ-4246158 的 Hotfix
...
* 已裝配的PDF檔案標題中的字元將出現亂碼。 NPR-25113：CQ-4244682 的 Hotfix
...
* 新增與Adobe Stock](/help/assets/aem-assets-adobe-stock.md)的整合，讓使用者可直接從AEM使用者介面搜尋、預覽、儲存及授權Adobe Stock資產。 [如需詳細資訊，請參閱[將Adobe Stock資產與AEM資產搭配使用](https://docs.adobe.com/content/help/en/experience-manager-learn/assets/creative-workflows/adobe-stock.html)。 NPR-15779：CQ-30857 的 Hotfix
...
* 可為資產資料夾設定中繼資料結構。 如需詳細資訊，請參閱[資料夾中繼資料結構](/help/assets/folder-metadata-schema.md)，並參閱安裝AEM 6.4.2.0後的[組態設定](#configuration-settings-required-for-npr) 。 NPR-21268：CQ-4221574 的 Hotfix
...
* 影像編輯器增強功能已向後移植至AEM 6.4.2.0。如需詳細資訊，請參閱[影像編輯器](https://docs.adobe.com/content/help/en/experience-manager-64/developing/components/image-editor.html)。 NPR-24467
...
* 已在Day CQ Mail Service中啟用STARTTLS支援。 NPR-23941：CQ-4240397 的 Hotfix
...
* (DMS7-UA)按一下已發佈視訊資產的「內嵌」按鈕時，任何動作都不會發生。 「內嵌」對話方塊應會以HTML程式碼顯示。 CQ-4245237 的 Hotfix
* （DM混合）已發佈視訊資產或混合媒體集的複製URL會在URL對話方塊中取得「`[object Object]`」。 CQ-4245236、CQ-4245451
...
* (DMHybrid)在6.3 &amp;gt；之後6.4升級後，目錄設定會移至不正確的節點。 CQ-4239974、CQ-4239975
...
* 導入[增強智慧標籤](https://docs.adobe.com/content/help/en/experience-manager-64/assets/administer/enhanced-smart-tags.html)。 NPR-21951
...
* AEM Forms推出[交易報表功能](https://docs.adobe.com/content/help/en/experience-manager-64/forms/transaction-reports/transaction-reports-overview.html)，以追蹤並保留已提交表單、已處理檔案和AEM Forms部署中已轉譯檔案等交易的計數。 它提供產品使用情況的深入分析，並協助商務使用者了解數位處理量。
* 啟用XML表單的PDF/UA支援。
* 新增Clientlib **aemfd.ccm.channel.contentpage**&#x200B;的allowProxy = true
...
* 在「爭議類型」頁簽的`ACTION TAKEN`部分下，「支援我的索賠的其他文檔」中有其他欄位「證明類型」保存。 CQ-4239873 的 Hotfix
...
* 設為「浮點數」或「小數」的HTML5數值方塊，會在不同的瀏覽器中提供不同的驗證結果。 NPR-23528：CQ-4244097 的 Hotfix
* （影像上傳）影像未在DOR預覽中顯示。 CQ-4243178 的 Hotfix
* 嘗試以「電子郵件PDF」和「包含附件」提交適用性表單時，JEE伺服器擲回錯誤。 CQ-4239894 的 Hotfix
...
* 使用DDX完成PDF/A轉換時，不會報告交易。 CQ-4246039 的 Hotfix
...
* （HTML工作區）名稱中有括弧之應用程式的追蹤詳細資料遺失。 NPR-23402
...
* （HTML工作區）首次開啟草稿時，準備出現兩次的資料呼叫。 CQ-4243280 的 Hotfix
* （HTML工作區）關閉表單時會觸發準備資料處理。 CQ-4239456 的 Hotfix
* 在兩個任務之間遍歷時，工作空間將掛起。 CQ-4237125 的 Hotfix

**Workbench**

* 管理動作設定檔準備資料程式在預設轉譯程式設定設為HTML時失敗。 CQ-4244292 的 Hotfix
...
* 在InDesign中導入AEM Assets參考。 如需詳細資訊，請參閱InDesign](/help/assets/managing-linked-subassets.md)中的[AEM Assets參考。 NPR-23386
...
* Service Pack下載可在[Software Distribution portal](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html)上下載。
...
1. 在AEM中，登入套件管理器並新增下載的AEM 6.4.8.0套件。 選擇已上載的包，然後按一下&#x200B;**[!UICONTROL Install]**。
...
1. 匯出全域設定。
1. 在下載的XML檔案中，添加標籤&lt;client_tasksPollingInterval>10&lt;/client_tasksPollingInterval>此處，10是以秒為單位的示例值。 您可以視情況修改此值。
1. 儲存檔案。
...
適用於AEM 6.4.8.0的Uber Jar可在[Adobe公用Maven存放庫](https://repo.adobe.com/nexus/content/groups/public/com/adobe/aem/uber-jar/6.4.8/)中取得。
...
* 在6.4.2.0上方套用6.4.3.0後，無法編輯任何影像元件和頁面屬性。CQ-4260316和CQ-4260441
若要解決此問題：
   * 前往封裝管理員
