...
* 驗證PDF文檔中的數字簽名。 (請參閱 [驗證數字簽名](#unresolvedlink-lc-si).)
* 驗證PDF文檔中的所有數字簽名。 (請參閱 [驗證多個數字簽名](#unresolvedlink-lc-si).)
...
有關處理安全憑據的資訊，請參閱*安裝和部署應用程式伺服器的AEM Forms*指南。
...
您還必須參考安全憑據以數字簽名PDF文檔。 要引用安全憑據，請指定別名。 別名是對實際憑據的引用，該憑據可能位於PKCS#12檔案中（副檔名為.pfx），或硬體安全模組(HSM)中。 有關安全憑據的資訊，請參閱*安裝和部署應用程式伺服器的AEM Forms*指南。
...
[使用Java API驗證數位簽名](#unresolvedlink-lc-si)

[使用Web服務API驗證數字簽名](#unresolvedlink-lc-si)
...
1. [建立簽名客戶端](#unresolvedlink-lc-si)
...
逐個反覆檢查每個簽名。 即對於每個簽名，驗證數字簽名，並檢查簽名者的身份和每個簽名的狀態。 (請參閱 [驗證數字簽名](#unresolvedlink-lc-si).)
...
[使用Java API驗證多個數位簽名](#unresolvedlink-lc-si)

[使用Web服務API驗證多個數字簽名](#unresolvedlink-lc-si)
