...
* 驗證PDF檔案中的數位簽名。 （請參閱[驗證數字簽名](#unresolvedlink-lc-si)。）
* 驗證PDF檔案中的所有數位簽名。 （請參閱[驗證多個數字簽名](#unresolvedlink-lc-si)。）
...
您還必須參考安全憑證以數位簽署PDF檔案。 要引用安全憑據，請指定別名。 別名是對實際憑據的引用，該憑據可能位於PKCS#12檔案中（副檔名為.pfx），或硬體安全模組(HSM)中。 有關安全憑據的資訊，請參閱*安裝和部署應用程式伺服器的AEM Forms*指南。
