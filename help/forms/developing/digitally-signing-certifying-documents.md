...
如需有關處理安全憑證的詳細資訊，請參閱*應用程式伺服器的安裝與部署AEM Forms*指南。
...
您也必須參考安全憑證才能數位簽署PDF檔案。 要引用安全憑據，請指定別名。 別名是對PKCS#12檔案（副檔名為。pfx）或硬體安全模組(HSM)中實際憑據的引用。 有關安全憑證的詳細資訊，請參閱*應用程式伺服器的安裝與部署AEM Forms*指南。
...
[使用web service API驗證數位簽名](#unresolvedlink-lc-si)
...
1. [建立簽名用戶端](#unresolvedlink-lc-si)
...
逐一重複每個簽名。 也就是說，對於每個簽名，請驗證數位簽名，並檢查簽章者的身分和每個簽名的狀態。 （請參閱[驗證數位簽名](#unresolvedlink-lc-si)）。
...
[使用web service API驗證多個數位簽名](#unresolvedlink-lc-si)
