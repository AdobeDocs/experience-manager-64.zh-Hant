...
1. 建立PDF匯寫程式式用戶端。
   ...
   * 使用`System.ServiceModel.EndpointAddress`建構函式建立`AssemblerServiceClient.Endpoint.Address`物件。 將指定WSDL的字串值傳遞給AEM Forms服務，如`http://localhost:8080/soap/services/AssemblerService?blob=mtom`。 您不需要使用`lc_version`屬性。 建立服務參考時，將使用此屬性。
