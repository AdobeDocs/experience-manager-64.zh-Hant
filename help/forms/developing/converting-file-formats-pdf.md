...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>指定如何響應特定於應用程式的對話框。 </p><p>包含此資訊的檔案為appmon。<i>[appname]</i>對話框。<i>[區域設定]</i>.xml（例如appmon.word.en_US.xml）。</p></td> 
  ...
  <tr> 
   ...
   <td><p>指定應用程式特定對話框指令的覆蓋和添加。 本節提供此類資訊的示例。 </p><p>包含此資訊的檔案為appmon。<i>[appname]</i>新增。<i>[區域設定]</i>.xml。 例如appmon.addition.en_US.xml。</p></td> 
...
* appmon。*[appname]*&#x200B;其他。*[地區]*.xml
* appmon。*[appname].[地區]*.xml（請勿修改此檔案。）
* appmon.global.*[地區]*.xml（請勿修改此檔案。）
...
指令碼包含在本機應用程式的窗口元素中導航的指令，並為這些元素提供特定響應。 包含此資訊的檔案為appmon。*[appname]*.script.*[地區]*.xml。 例如appmon.notepad.script.en_US.xml。
...
>[!NOTE]
>
>在此情境中，「額外」一詞表示應用程式的內容。[應用程式名稱]新增。[地區].xml檔案。 此類檔案指定對話XML檔案的覆蓋和添加。
...
標識其他dialogXML檔案的檔案名為appmon。*[appname]*&#x200B;新增。*[地區]*.xml。 例如appmon.excel.addition.en_US.xml。

其他對話框XML檔案的名稱必須使用appmon格式。*[應用程式名稱]*&#x200B;新增。*[地區]*.xml，其中 *應用程式名稱* 必須與XML配置檔案和指令碼中使用的應用程式名稱完全匹配。
...
建立環境變數，指定本機應用程式執行檔的位置。 變數必須使用格式 *[應用程式名稱]*&#x200B;路徑(_P)，其中 *應用程式名稱* 必須與XML配置檔案和指令碼中使用的應用程式名稱完全匹配，且其中路徑包含雙引號中執行檔的路徑。 此類環境變數的範例為 `Photoshop_PATH`.
...
1. 在「新建系統變數」(New System Variable)對話框中， **變數名稱** 框中，鍵入使用格式的名稱 *[應用程式名稱]*&#x200B;路徑(_P)。
