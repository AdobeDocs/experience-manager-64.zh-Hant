...
1. 包含專案檔案。
   ...
   >[!NOTE]
   >
   >將`localhost`*替換為托管AEM Forms之伺服器的IP位址。*
...
>[!NOTE]
>
>有關輸出服務的詳細資訊，請參閱[AEM Forms的服務參考](https://www.adobe.com/go/learn_aemforms_services_64)。
...
1. 檢索操作的結果。
   ...
   * 調用`com.adobe.idp.Document`對象的`copyToFile`方法，將`com.adobe.idp.Document`對象的內容複製到檔案（確保使用`getStatusDoc`方法返回的`com.adobe.idp.Document`對象）。
   ...
   >[!NOTE]
   >
   >儘管輸出服務將PDF/A文檔寫入由傳遞給`PDFOutputOptionsSpec`對象的`setFileURI`方法的參數指定的位置，但通過調用`OutputResult`對象的`getGeneratedDoc`*方法，您可以以寫程式方式檢索PDF/A文檔。*
...
1. 設定打印到檔案所需的打印運行時選項。
   ...
   * 通過調用PrintedOutputOptionsSpec對象的`setFileURI`方法並傳遞表示檔案名稱和位置的字串值來指定檔案。 例如，如果希望輸出服務打印到位於C:\Adobe的名為* MortgageForm.ps*的PostScript檔案，請指定C:\\Adobe\MortgageForm.ps。
