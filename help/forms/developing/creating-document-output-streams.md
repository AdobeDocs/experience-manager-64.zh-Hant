...
1. 包含專案檔案。
   ...
   >[!NOTE]
   >
   >將`localhost`*替換為代管AEM Forms的伺服器的IP地址。*
...
1. 設定列印至檔案所需的列印執行時間選項。
   ...
   * 調用PrintedOutputOptionsSpec對象的`setFileURI`方法並傳遞代表檔案名稱和位置的字串值，以指定檔案。 例如，如果您希望輸出服務列印至C:\Adobe中名為* MortgageForm.ps*的PostScript檔案，請指定C:\\Adobe\MortgageForm.ps。
...
1. 設定列印執行時期選項
   ...
   >[!NOTE]
   >
   >如果您要產生ZPL列印串流，則無法使用`PrintedOutputOptionsSpec`物件的`setPagination`方法來設定分頁值。 同樣地，您無法為ZPL打印流設定以下選項：OutputJog、PageOffset和Stapler。 `setPagination`*方法對於PostScript的生成無效。 它僅適用於PCL生成。*
