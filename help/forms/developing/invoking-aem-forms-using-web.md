...
1. 使用其 `javax.activation.DataHandler` 建構函式並傳遞物件來建立物 `javax.activation.FileDataSource` 件。 可 `javax.activation.FileDataSource` 使用其建構函式並傳遞代表PDF檔案 `java.io.File` 的物件來建立物件。
