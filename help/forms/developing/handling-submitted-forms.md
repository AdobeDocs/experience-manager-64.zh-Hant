...
1. 擷取表單資料
   ...
   * 使用其建構子建立`RenderOptionsSpec`物件。 通過調用`RenderOptionsSpec`對象的`setLocale`方法並傳遞指定區域設定值的字串值來設定區域設定值。
   ...
   >[!NOTE]
   >
   >您可以叫用`RenderOptionsSpec`物件的`setPDF2XDP`方法並傳遞`true`，以及呼叫`setXMLData`並傳遞`true`，指示Forms服務從已提交的PDF內容建立XDP或XML資料。 然後，可以調用`FormsResult`對象的`getOutputXML`方法以檢索與XDP/XML資料對應的XML資料。 （`FormsResult`物件由`processFormSubmission`*方法傳回，將於下一個子步驟中說明。）*
