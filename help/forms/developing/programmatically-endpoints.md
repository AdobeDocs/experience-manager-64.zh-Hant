...
1. 指定連接器類型。
   ...
   調用物件的方法並傳 `ConnectorRegistryClient` 遞指定連 `getEndpointDefinition` 接器類型的字串值，以指定連接器類型。 例如，若要指定「Watched資料夾」連接器類型，請傳遞字串值 `WatchedFolder`。 此方法返回與 `Endpoint` 連接器類型對應的對象。
