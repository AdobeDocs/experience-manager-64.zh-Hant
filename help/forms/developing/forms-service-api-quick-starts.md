...
下面的程式碼範例會使用Forms服務Java API，在用戶端轉譯名為*Loan.xdp *的表格。 請注意，表單設計是應用程式的一部分，且是使用內容根URI值來參考的 `repository:///`。 (請參 [閱用戶端上的轉譯表單](/help/forms/developing/rendering-forms.md#rendering-forms-at-the-client)。)
...
以下Java快速入門會轉譯互動式PDF表單，該表單以名為 *Loan.xdp* by value的表單設計為基礎。 請注意，表單設計是用來填入名 `com.adobe.idp.Document` 為* inputXDP*的物件。 (請參 [閱依值轉換表單](/help/forms/developing/rendering-forms.md#rendering-forms-by-value)。)
