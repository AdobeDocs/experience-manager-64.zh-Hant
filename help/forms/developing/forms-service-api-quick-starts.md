...
下面的程式碼範例會使用Forms服務Java API，在用戶端轉譯名為*Loan.xdp *的表格。 請注意，表單設計是應用程式的一部分，使用內容根URI值`repository:///`來參考。 （請參閱用戶端上的[轉換表單。）](/help/forms/developing/rendering-forms.md#rendering-forms-at-the-client)
