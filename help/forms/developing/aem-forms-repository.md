...
description: 管理AEM Forms儲存庫，以使用Java API和Web Service API建立資料夾、寫入、清單、讀取、更新資源及搜尋資源。 此外，還要瞭解如何建立資源關係、鎖定和刪除資源。
