...
description: 使用組合器服務來組合PDF文檔、拆解PDF文檔、組合加密PDF文檔、組合帶有Bates編號的PDF文檔、組合非互動式PDF文檔、確定文檔是否符合PDF/A、驗證DDX文檔、組合帶書籤的PDF文檔、動態建立DDX文檔、組合PDF產品組合以及組合多個XDP片段。
...
以下Java代碼示例合併了名為* map.pdf*的兩個PDF源文檔，以及 *directions.pdf* 整合到單一PDF檔案中。 單個PDF文檔的名稱為 *AssemblerResultPDF.pdf*. DDX文檔的名稱為 *shell.xml*. (請參閱 [以寫程式方式組合PDF文檔](/help/forms/developing/assembling-pdf-documents.md#programmatically-assembling-pdf-documents).)
...
以下Java代碼示例拆分名為的PDF文檔 *AssemblerResultPDF.pdf*. 請注意，DDX文檔的名稱是 *shell_disrebumble.xml*. 每個已拆卸的PDF文檔均命名 *ResultPDF[數字].pdf*. 即，第一個已拆解的PDF文檔被命名 *ResultPDF1.pdf。* 如需 *shell_disrebumble.xml* 此程式碼範例中使用的DDX檔案，請參閱 [以寫程式方式分解PDF文檔](/help/forms/developing/assembling-pdf-documents.md#programmatically-disassembling-pdf-documents).
