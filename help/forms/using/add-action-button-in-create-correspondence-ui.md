...
1. 在應用程式檔案夾中，建立名為`defaultApp`的檔案夾，其路徑／結構類似於defaultApp檔案夾（位於config檔案夾中）。 使用下列步驟建立資料夾：
   ...
   * 在以下路徑中按一下右鍵&#x200B;**[!UICONTROL defaultApp]**&#x200B;資料夾，然後選擇&#x200B;**[!UICONTROL 覆蓋節點]**:
...
1. 複製acmExtensionsConfig.xml檔案（存在於/libs分支下），位於/apps分支下。
   ...
   * 在acmExtensionsConfig.xml檔案上按一下滑鼠右鍵，然後選取&#x200B;**[!UICONTROL Copy]**。

      ![複製acmExtensionsConfig.xml](assets/3_acmextensionsconfig_xml_copy.png)

   * 在「/apps/fd/cm/config/defaultApp/」的&#x200B;**[!UICONTROL defaultApp]**&#x200B;資料夾上按一下滑鼠右鍵，然後選取「貼上」。****
   * 按一下&#x200B;**[!UICONTROL 保存全部]**。
...
1. 在以下路徑中按一下右鍵&#x200B;**[!UICONTROL locale]**&#x200B;資料夾，然後選擇&#x200B;**[!UICONTROL 覆蓋節點]**:
...
1. 在apps資料夾中，在CRX的/apps分支中建立名為`js`的資料夾，其結構類似於下列資料夾：
   ...
   1. 在以下路徑中按一下右鍵&#x200B;**[!UICONTROL js]**&#x200B;資料夾，然後選擇&#x200B;**[!UICONTROL 覆蓋節點]**:
...
1. 在js資料夾中，使用下列步驟建立名為ccrcustomization.js的檔案，其中包含按鈕動作處理的程式碼：
   ...
   1. 在以下路徑中按一下右鍵&#x200B;**[!UICONTROL js]**&#x200B;資料夾，然後選擇&#x200B;**[!UICONTROL 建立>建立檔案]** :
...
### 添加LiveCycle進程以啟用操作<span class="acrolinxCursorMarker"></span>處理{#add-the-livecycle-process-to-enable-action-span-class-acrolinxcursormarker-span-handling}
...
下載並解壓縮`components.zip`檔案，以取得`DSCSample.jar`和`SendLetterForReview.lca`檔案。 請依照下列程式指定使用這些檔案。
...
1. 輸入以下詳細資訊，然後按一下&#x200B;**[!UICONTROL 保存]**:
   ...
   * **[!UICONTROL 伺服器Url]**:動作處理常式程式碼所使用之Send For Review服務之LC伺服器的URL。
   * **[!UICONTROL 使用者名稱]**:LC伺服器的管理員用戶名
   * **[!UICONTROL 密碼]**:管理員使用者名稱的密碼
...
1. 導覽至「**[!UICONTROL 首頁>服務>應用程式與服務>應用程式管理]**」。
...
1. 選擇&#x200B;**[!UICONTROL 匯入完成時將資產部署至執行階段]**。
...
1. 找到並按一下&#x200B;**[!UICONTROL AdobeLiveCycle客戶端SDK配置]**。 出現「AdobeLiveCycle客戶端SDK配置」面板。
...
1. 導覽至「**[!UICONTROL 首頁>服務>應用程式與服務>服務管理]**」。
...
若要使用Correponse Management API，請下載`DSCSample.jar`（附於本檔案中，作為`components.zip`的一部分）並將其上傳至LiveCycle伺服器。 將`DSCSample.jar`檔案上傳至LiveCycle伺服器後，AEM伺服器會使用`DSCSample.jar`檔案來存取renderLetter API。

有關詳細資訊，請參閱[將AEM Forms與AdobeLiveCycle連接](/help/forms/using/aem-livecycle-connector.md)。

1. 更新AEM`DSCSample.jar`中cmsa.properties中的伺服器URL，該位置如下：

   DSCSample.jar\com\adobe\livecycle\cmsa.properties

1. 在配置檔案中提供以下參數：

   * **crx.serverUrl**=https:/[host]/:port[/contextpath][/]context [path/AEM URL]
   * **crx.username**=AEM用戶名
...
`DSCSample.jar` 檔案會使用 `renderLetter` API，從C輸入的XML資料中，將字母轉譯為PDF位元組。如需renderLetter和其他API的詳細資訊，請參閱[Letter Render Service](https://helpx.adobe.com/aem-forms/6-2/javadocs/com/adobe/icc/ddg/api/LetterRenderService.html)。
...
1. 通過檔案瀏覽器選擇`DSCSample.jar`檔案，然後按一下&#x200B;**[!UICONTROL 開啟]**。
1. 按一下右鍵&#x200B;**[!UICONTROL RenderWrapper]**&#x200B;並選擇&#x200B;**[!UICONTROL 啟動元件]**。 如果元件啟動，則元件名稱旁會出現綠色箭頭。
...
1. 在「建立對應」UI中，按一下&#x200B;**[!UICONTROL Letter Review]**&#x200B;並指定審核者的電子郵件ID。
