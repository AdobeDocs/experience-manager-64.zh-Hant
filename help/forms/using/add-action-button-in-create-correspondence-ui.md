...
1. 在應用程式檔案夾中，建立名為路徑／結 `defaultApp` 構的檔案夾，類似於defaultApp檔案夾（位於config檔案夾中）。 使用下列步驟建立資料夾：
   ...
   * 以滑鼠右鍵按一 **[!UICONTROL 下下列路徑上的]** defaultApp資料夾，然後選取「 **[!UICONTROL 覆蓋節點」]**:
...
1. 複製acmExtensionsConfig.xml檔案（存在於/libs分支下），位於/apps分支下。
   ...
   * 在acmExtensionsConfig.xml檔案上按一下滑鼠右鍵，然後選取「復 **[!UICONTROL 制」]**。

      ![複製acmExtensionsConfig.xml](assets/3_acmextensionsconfig_xml_copy.png)

   * 以滑鼠右鍵按一 **[!UICONTROL 下「/apps/fd/cm/config/defaultApp/]** 」的defaultApp資料夾，並選取「貼上」 ****。
   * 按一下「 **[!UICONTROL 全部儲存]**」。
...
1. 在下列路徑上 **[!UICONTROL 右鍵按一下]** 「地區設定」檔案夾，然後選取「 **[!UICONTROL 覆蓋節點」]**:
...
1. 按一下右鍵以下路 **[!UICONTROL 徑的]** locale資料夾，然後選擇 **[!UICONTROL 貼上]**:
...
1. 在js資料夾中，使用下列步驟建立名為ccrcustomization.js的檔案，其中包含按鈕動作處理的程式碼：
   ...
   1. 在下列路徑上 **[!UICONTROL 右鍵按一下]** 「js」檔案夾，然後選 **[!UICONTROL 取「建立>建立檔案」]**:
   ...
   1. 在檔案中，貼上下列程式碼，然後按一下「全 **[!UICONTROL 部儲存]**:
...
### 新增LiveCycle程式以啟用動作處 <span class="acrolinxCursorMarker"></span>理 {#add-the-livecycle-process-to-enable-action-span-class-acrolinxcursormarker-span-handling}
...
1. 找到「Adobe LiveCycle Client SDK Configuration」（Adobe LiveCycle用戶端SDK設定），然後按一 **[!UICONTROL 下「Edit]** 」（編輯圖示）。 「配置」(Configurations)面板隨即開啟。
...
1. 按一 **[!UICONTROL 下「選擇檔案]** 」，然 **[!UICONTROL 後選取SendLetterForReview.lca]**。
...
1. 找到並按一 **[!UICONTROL 下「Adobe LiveCycle Client SDK Configuration」]**。 出現「Adobe LiveCycle Client SDK Configuration」（Adobe LiveCycle用戶端SDK設定）面板。
...
1. 在 **[!UICONTROL SMTP主機中]**，配置電子郵件服務。
...
1. 在「建立對應」UI中，按一下「 **[!UICONTROL 信函檢閱]** 」並指定審核者的電子郵件ID。
