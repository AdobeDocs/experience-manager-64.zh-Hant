...
您可以使用 [Forms以OSGi為中心的工作流程](/help/forms/using/aem-forms-workflow.md) 快速建立最適化表單式工作流程。 這些工作流程可協助您自動執行審核和核准工作流程、業務流程工作流程和其他重複性工作。 這些工作流程還有助於處理檔案(建立、組合、分發和歸檔PDF檔案、添加數字簽名以限制對文檔的訪問、對條碼式表單進行解碼等)，以及將Acrobat Sign簽名工作流程與表單和文檔一起使用。
...
>[!NOTE]
>
>透過OSGi上以Forms為中心的工作流程，您可以快速建立並部署OSGi堆疊上各種工作的工作流程，而無須在JEE堆疊上安裝完整的流程管理功能。 請參閱 [比較](/help/forms/using/capabilities-osgi-jee-workflows.md) OSGi和JEE上以Forms為中心的AEM工作流程，以了解功能的差異與相似之處。
>
...
>[!NOTE]
>
>跳到 [後續步驟](#next-steps) 一節，若您已按照 [安裝和配置資料捕獲功能](/help/forms/using/installing-configuring-aem-forms-osgi.md) 文章。
