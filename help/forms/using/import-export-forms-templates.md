...
>[!NOTE]
>
>* 在AEMForms6.4中，crx-repository的結構和路徑已經改變。 如果您從舊版將資產匯入至AEM6.4Forms，而表單對舊版結構有一些相依性，則必須手動匯出相依性。 有關儲存庫結構和路徑更改的詳細資訊，請參見[6.4&lt;a1/AEM>中的儲存庫重構。](/help/sites-deploying/repository-restructuring.md)
...
有了AEM Forms，您可以建立、下載或上傳主題。 主題的建立方式與其他資產（例如表單、檔案和字母）類似。 您可以建立主題、下載主題，然後將主題上傳至個別的例項，以重複使用主題。 有關主題的詳細資訊，請參閱AEM Forms的[主題](/help/forms/using/themes.md)。
...
例如，如果「對應管理」有使用影像和文字的字母，則下載的套件也會包含影像和與字母相關的文字。 也會下載與資產相關聯的所有中繼資料屬性（包括自訂屬性）。 下載包(.cmp)後，可以[將包導入到不同的AEM Forms實例](/help/forms/using/import-export-forms-templates.md#p-upload-forms-documents-assets-p)。
...
1. 點選&#x200B;**下載匯出的套件**&#x200B;連結以下載套件檔案。
   ...
   要將包中的資產添加到另一個Correponsement Management實例，[將包導入到AEM Forms實例](/help/forms/using/import-export-forms-templates.md#p-upload-forms-documents-assets-p)。
...
>[!NOTE]
>
>匯入舊的Correponse Management資產進行移轉時，請使用管理員帳戶登入。 有關遷移舊的通信管理資產的詳細資訊，請參閱[遷移AEM Forms資產和文檔](/help/forms/using/migration-utility.md)。
