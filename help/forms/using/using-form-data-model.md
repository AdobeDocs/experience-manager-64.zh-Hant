...
當使用者根據表單資料模型提交表單時，您可以設定表單，將資料模型物件的提交資料寫入其資料來源。 為了實現此使用案例，AEM Forms提供了「表單資料模型提交操作」（僅針對基於表單資料模型的自適應表單提供現成可用的操作）。 [](/help/forms/using/configuring-submit-actions.md)它將資料模型對象的提交資料寫入其資料源。
