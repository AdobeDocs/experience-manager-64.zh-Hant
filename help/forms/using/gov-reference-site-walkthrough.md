...
在申請獲得批准後，申請人會從We.Gov網站收到一封電子郵件。 電子郵件中的&#x200B;**查看文檔**&#x200B;選項將連結到具有申請人註冊詳細資訊的文檔。
...
在瀏覽器中，開啟`https://<hostname>:<PublishPort>/content/we-gov/en.html`。 在We.Gov網站中，按一下「子系支援」。 在「子支援」頁面中，按一下「檢查您的資格」。

若要查看規則：

1. 在作者實例的編輯模式下開啟表單。 URL: `https://<hostname>:<AuthorPort>/editor.html/content/forms/af/we-gov/child-support/css.html`.
1. 選擇元件並按一下![edit-rules](assets/edit-rules.png)。

   規則編輯器隨即開啟，列出表單中套用的所有規則。

1. 在左側面板中，按一下規則`passMsg`和`failMsg`以瞭解資格檢查的運作方式。

## Sarah啟動她的兒童支援申請{#sarah-starts-her-application-for-child-support}

Sarah在獲悉符合兒童支援資格後，按一下「開始申請」。****\
...
預設情況下，Sarah會進入&#x200B;**[!UICONTROL 關於申請人]**&#x200B;部分。

![案頭上的子支援應用程式](assets/desktop.png)

Sarah隨時可以按一下&#x200B;**[!UICONTROL 稍後回來]**&#x200B;並繼續使用其應用程式。 當她按一下「稍後回來&#x200B;**[!UICONTROL 」時，她的進度會儲存為草稿，而且她可以以電子郵件傳送草稿。]**

當她按一下「傳送電子郵件」(**[!UICONTROL Send Email]**)時，她會收到一封電子郵件，內含表單草稿的連結。
...
在Sarah按一下&#x200B;**[!UICONTROL 啟動應用程式]**&#x200B;後，她將被帶到「申請子代支援服務」頁面的「申請人資訊」部分。 在「申請人資訊」下，Sarah會導覽各個標籤，並為申請提供個人資訊。 她按一下&#x200B;**[!UICONTROL Next]**&#x200B;瀏覽各頁籤。
...
在Sarah提供個人資訊並按一下&#x200B;**[!UICONTROL Next]**&#x200B;後，她就會進入「子項資訊」區段。
...
在Sarah提供收養子女的資訊並按一下&#x200B;**[!UICONTROL Next]**&#x200B;後，她將進入&#x200B;**[!UICONTROL 付款資訊]**&#x200B;部分。

在「付款資訊」區段中，她提供銀行帳戶詳細資訊，以取得子女支援福利。\
她輸入了10位數的銀行帳號。

## Sarah支付申請費並簽署{#sarah-pays-the-application-fee-and-signs-the-form}表格

在莎拉同意申請的條款和條件後，她支付了25美元的申請費。 處理申請時需要支付申請費。\
Sarah輸入信用卡詳細資訊，然後按一下「立即付款」。 ****&#x200B;付費後，應用程式的PDF版本會顯示簽名欄位。
...
在您的瀏覽器中，開啟`https://<hostname>:<PublishPort>/content/forms/af/we-gov/child-support/css.html`並填寫所需資訊。 當您填妥所需資訊、支付費用並簽署檔案後提交申請時，您會收到確認電子郵件。
...
Gloria可以批准或拒絕該應用程式。 不過，Gloria按一下「需要評估」**[!UICONTROL ，以取得應用程式評估。]**
...
訪問位於https://&lt;***hostname***>的收件箱：&lt;***PublishPort***>/content/we-gov/en.html。 在頁面上，點選「登入&#x200B;****」，選取「以代表身分登入&#x200B;**[!UICONTROL 」核取方塊，使用Gloria的預設憑證：]**
...
當Gloria按一下「**[!UICONTROL 必要評估]**」時，Conard會在其AEM收件匣中取得審閱工作。 此工作是AEM工作流程中在工作流程模型中定義的下一個步驟。 他看到審核任務並開啟它。
...
Conard按一下「提交以進行重新評估」**[!UICONTROL ,AEM工作流程就會將評估提交給We.Gov組織。]**
...
### Gloria審查評估並批准應用程式{#gloria-reviews-the-assessment-and-approves-the-application}

在Conard提交評估後，Gloria會在收件箱中看到「審閱」任務。 她選擇並開啟&#x200B;**[!UICONTROL Review]**。
...
## We.Gov會分析應用程式的效能{#we-gov-analyzes-the-performance-of-the-application}
...
若要檢視並探索分析報表，我們會為參考網站的子支援服務應用程式提供種子資料。 使用種子資料之前，請參閱[設定Analytics](/help/forms/using/setup-reference-sites.md#configureanalytics)。 在作者實例中執行以下步驟以查看包含種子資料的報告：

1. 請至&#x200B;**[!UICONTROL 表單與檔案]** UI，網址為https://&lt;*hostname*:&lt;*AuthorPort*>/aem/forms.html/content/dam/formsanddocuments。

1. 按一下以開啟&#x200B;**We.Gov**&#x200B;資料夾。
1. 選擇&#x200B;**[!UICONTROL 子項支援服務應用程式]**&#x200B;最適化表單，然後按一下工具列中的&#x200B;**[!UICONTROL 啟用Analytics]**。
...
1. 複製測試資料檔案中的所有內容。
1. 在CRXDE中，導覽至：**/content/dam/formsanddocuments/we-gov/child-support/css/jcr:content/analyticsdatando/lastsevendays**
1. 在「屬性」下的「分析資料」欄位中，貼上測試資料檔案的複製內容。
1. 現在，請針對&#x200B;**[!UICONTROL 子支援服務應用程式]**&#x200B;重新產生分析報表。 您可以在產生的報表中看到種子資料。
