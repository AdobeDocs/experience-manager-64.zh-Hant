...
* 對於使用[XFA表單模板](/help/forms/using/prepopulate-adaptive-form-fields.md)的自適應表單，請使用與XFA模板的資料模式相容的預填充XML。
* 對於使用[XML架構](#xml-schema-af)的自適應表單，請使用與XML架構結構相容的預填充XML。
* 對於使用[JSON結構描述的最適化表單，請使用與JSON結構描述相容的預填JSON。](/help/forms/using/prepopulate-adaptive-form-fields.md#json-schema-based-adaptive-forms)
* 對於使用FDM架構的最適化表單，請使用與FDM架構相容的預填JSON。
* 對於[無表單模型](/help/forms/using/prepopulate-adaptive-form-fields.md#p-adaptive-form-with-no-form-model-p)的自適應表單，沒有綁定資料。 每個欄位都是未綁定欄位，並且使用未綁定的XML預填充。
...
對於以JSON結構描述為基礎的最適化表單，以下說明預先填寫JSON和已提交JSON的結構。 如需詳細資訊，請參閱[使用JSON結構描述建立最適化表單](/help/forms/using/adaptive-form-json-schema-form-model.md)。
...
1. 為&#x200B;**[!UICONTROL 資料檔案位置]**&#x200B;輸入資料位置或規則表達式。 有效資料檔案位置的範例包括：
...
* SERVICE_NAME是指OSGI預填充服務的名稱。 請參閱[建立並運行預填充服務](/help/forms/using/prepopulate-adaptive-form-fields.md#create-and-run-a-prefill-service)。
...
1. 在「屬性」控制台中，導覽至「AEM Forms」容器>「基本」>「預填服務」]**。**[!UICONTROL 
