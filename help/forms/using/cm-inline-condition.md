...
# 內嵌條件，並在互動式通訊和字母{#inline-condition-and-repeat-in-interactive-communications-and-letters}中重複

使用內嵌條件並在互動式通訊和信件中重複，您就可建立高度情境化且結構化的通訊。
...
如需詳細資訊，請參閱互動式通訊中[文字中的「建立規則」。](/help/forms/using/texts-interactive-communications.md)
...
1. 請執行下列動作來建立條件，以評估選取／條件化文字是否出現在字母中，然後點選核取標籤以儲存運算式：
   ...
   如需有關建立運算式的詳細資訊，請參閱[運算式產生器](/help/forms/using/expression-builder.md)中的&#x200B;**使用運算式產生器**&#x200B;建立運算式和遠端函式。 運算式中指定的值必須支援資料字典中的元素。 如需詳細資訊，請參閱[資料字典](/help/forms/using/data-dictionary.md)。
