...
# 內嵌條件，並在互動式通訊和信函中重複{#inline-condition-and-repeat-in-interactive-communications-and-letters}
...
如需詳細資訊，請參閱互動式通訊](/help/forms/using/texts-interactive-communications.md)中的[文字中的建立規則。
...
1. 執行以下操作可構建一個條件，用於評估所選/條件化文本是否出現在信函中，然後點選複選標籤以保存表達式：
   ...
   如需建立運算式的詳細資訊，請參閱[運算式產生器](/help/forms/using/expression-builder.md)中的&#x200B;**使用運算式產生器**&#x200B;建立運算式和遠端函式。 必須支援資料字典中的元素使用運算式中指定的值。 如需詳細資訊，請參閱[資料字典](/help/forms/using/data-dictionary.md)。
