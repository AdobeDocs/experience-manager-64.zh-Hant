...
如果您打算使用PDF生成器服務將Microsoft Word、Microsoft Excel、Microsoft PowerPoint、OpenOffice、WordPerfect X7和Adobe Acrobat等本機檔案格式轉換為PDF文檔，請確保這些應用程式已安裝在AEM Forms伺服器上。
...
如果運行的是Microsoft Windows Server，請配置Ink and Shartting服務。 需要此服務才能開啟使用Microsoft Office的連結功能的Microsoft PowerPoint檔案：
...
1. 開啟Microsoft Windows的群組原則編輯器。 要開啟組策略編輯器，請按一下 **[!UICONTROL 開始]**，類型 **gpedit.msc** 在「開始搜索」框中，按一下 **[!UICONTROL 組策略編輯器]**.
...
1. 驗證UAC的註冊表設定是否設定為0（零）。 執行下列步驟以驗證：
   ...
   1. Microsoft建議您先備份註冊表，然後再修改它。 如需詳細步驟，請參閱 [如何在Windows中備份和還原註冊表](https://support.microsoft.com/en-us/help/322756).
...
在Windows Server上使用PDF生成器服務將文檔轉換為PDF時，Windows Server偶爾會報告執行檔遇到問題，需要關閉。 不過，它不會影響PDF轉換，因為會在背景繼續。
...
1. 登入AEM製作例項並導覽至 **[!UICONTROL Adobe Experience Manager]** > **[!UICONTROL 工具]** > **[!UICONTROL Forms]** > **[!UICONTROL 設定PDF產生器]**. 預設URL為http://localhost:4502/libs/fd/pdfg/config/ui.html。
...
1. 執行系統整備工具(SRT)以驗證Acrobat安裝。 此工具會檢查電腦是否已正確設定以執行PDF產生器轉換，並在指定路徑產生報表：
   ...
   1. 開啟命令提示符。 導覽至 `[extracted-adobe-aemfd-pdfg-common-pkg]\jcr_root\etc\fd\ pdfg\tools\adobe-aemfd-pdfg-utilities-[version]-win.zip\srt` 檔案夾。 從命令提示符運行以下命令：

      `cscript SystemReadinessTool.vbs [Path_of_reports_folder] en`

      >[!NOTE]
      >
      >如果「系統整備工具」報告pdfgen.api檔案不在acrobat外掛程式資料夾中，則從 `[extracted-adobe-aemfd-pdfg-common-pkg]\plugins\x86_win32` 目錄 `[Acrobat_root]\Acrobat\plug_ins` 目錄。
