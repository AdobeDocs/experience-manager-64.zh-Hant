...
1. 在AEM登入頁面上輸入您的認證。
   ...
   登入後，在左上角，點選「 **[!UICONTROL Adobe Experience Manage >表單>表單與檔案」]**。
...
>[!NOTE]
>
>當您開啟使用進階範本建立的最適化表單進行編輯時，會出現錯誤訊息。 進階範本有「簽名步驟」元件，預設會啟用Adobe Sign。 建立並選取 [Adobe Sign雲端設定](/help/forms/using/adobe-sign-integration-adaptive-forms.md) , [並設定簽署者](/help/forms/using/working-with-adobe-sign.md#addsignerstoanadaptiveform) ，以解決錯誤。
