...
適用性表單需要提交動作，才能處理使用者指定的資料。 「提交」動作會決定您使用最適化表單提交之資料所執行的任務。 Adobe Experience Manager(AEM)包含 [OOTB提交操作](/help/forms/using/configuring-submit-actions.md) 展示可使用使用者提交的資料執行的自訂工作。 例如，您可以執行工作，例如傳送電子郵件或儲存資料。
...
XML資料會使用 **`jcr:data`** 要求參數。 提交動作可存取參數以處理資料。 以下代碼描述XML資料的格式。 綁定到表單模型的欄位將顯示在 **`afBoundData`** 區段。 未綁定的欄位顯示在 `afUnoundData` 區段。 如需 `data.xml` 檔案，請參閱 [預先填入最適化表單欄位的簡介](/help/forms/using/prepopulate-adaptive-form-fields.md).
...
>[!NOTE]
>
>作者提供重新導向URL（使用感謝頁面設定）。 [OOTB提交操作](/help/forms/using/configuring-submit-actions.md) 使用「重新導向URL」，從正向路徑參考的資源重新導向瀏覽器。
>
