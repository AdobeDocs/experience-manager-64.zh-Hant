...
最適化表單需要「提交」動作來處理使用者指定的資料。 「提交」操作決定使用自適應表單對提交的資料執行的任務。 Adobe Experience Manager(AEM)包含[OOTB Submit動作](/help/forms/using/configuring-submit-actions.md)，可展示您可以使用使用者提交資料執行的自訂工作。 例如，您可以執行工作，例如傳送電子郵件或儲存資料。
...
使用&#x200B;**`jcr:data`**&#x200B;請求參數將XML資料發送到servlet。 提交動作可存取參數以處理資料。 以下程式碼說明XML資料的格式。 綁定到Form模型的欄位將出現在&#x200B;**`afBoundData`**&#x200B;部分。 未綁定的欄位將出現在`afUnoundData`部分中。 有關`data.xml`檔案格式的詳細資訊，請參閱[預填充自適應表單域的簡介](/help/forms/using/prepopulate-adaptive-form-fields.md)。
...
>[!NOTE]
>
>作者提供「重新導向URL」（使用「感謝頁面設定」）。 [OOTB Submit ](/help/forms/using/configuring-submit-actions.md) Actionsuse the Redirect URL to redirect the browser from the resource the forward path references.
>
