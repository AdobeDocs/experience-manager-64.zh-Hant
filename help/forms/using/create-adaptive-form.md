...
1. 點選 ![「切換側面板](assets/toggle-side-panel.png) >樹狀 ![擴充」](assets/treeexpandall.png)。 元件瀏覽器隨即開啟。 將Header元 **件從元件瀏覽器** 拖曳至最適化表單。
...
元件是自適應形式的構建塊。 AEM Forms提供許多元件，可擷取並顯示最適化表單中的資訊。 您可以將元件從樹狀擴 ![充全部拖曳](assets/treeexpandall.png) 至表單。 若要瞭解可用的元件和相應的功能，請參 [閱製作最適化表單簡介](/help/forms/using/introduction-forms-authoring.md)。
