...
* AEM 6.4與Forms附加元件套件的執行中例項。 如需詳細資訊，請 [參閱「安裝和設定AEM表格」](installing-configuring-aem-forms-osgi.md)。
...
1. 根據您的環境更新主機和埠設定。
1. 儲存設定。
1. (僅&#x200B;**限**)「工具」 **[!UICONTROL >「工具]** 」>「 **[!UICONTROL 雲服務]** 」>「Sources ********」>「Go globalSources」資料作者。 選取 **[!UICONTROL 保留投資]** ，並點選 **[!UICONTROL 「屬性」]**。 點選「 **[!UICONTROL 驗證設定]** 」，並將「驗 **[!UICONTROL 證類型]** 」設 **[!UICONTROL 定為「基本驗證]**」。 指定 `admin`/ `admin`作為用於訪問服務的用戶名／密碼。 點選「 **[!UICONTROL 儲存並關閉]**」。
