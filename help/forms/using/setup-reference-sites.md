...
* **AEM Essentials**
   ...
   AEM quickStart、AEM Forms附加元件套件和參考網站套件。 請參 [閱AEM Forms發行](https://helpx.adobe.com/aem-forms/kb/aem-forms-releases.html) ，以取得附加元件和參考網站套件的詳細資訊。
...
* AEM 6.4與Forms附加元件套件的執行中例項。 如需詳細資訊，請 [參閱「安裝和設定AEM表格」](https://helpx.adobe.com/experience-manager/6-3/forms/using/installing-configuring-aem-forms-osgi.html)。
...
如在OSGi上安裝和設定AEM表 [單中所述，安裝和部署AEM表單](/help/forms/using/installing-configuring-aem-forms-osgi.md)。
...
1. 尋找並點選 **[!UICONTROL Day CQ Link Externalizer組態]** 。
...
1. 點選「 **[!UICONTROL 儲存]**」。 等待一段時間，以便所有服務重新啟動。
...
在參考網站使用案例中，「發佈」例項中必須有AEM DS服務設定，才能進行電子郵件通訊。 如需在「發佈」例項上設定AEM DS服務設定的詳細步驟，請參閱「 [設定AEM DS設定」](/help/forms/using/configuring-the-processing-server-url-.md)。
...
1. 在您的Microsoft Dynamics實例中，轉至「設 **[!UICONTROL 定]** 」功能表中的「解決方 **[!UICONTROL 案]** 」，然後按一下「 **[!UICONTROL 匯入]**」。 選擇並導入包。
...
1. 在您的Microsoft Dynamics實例中，執行下列操作：
   ...
   * 前往「銷 **[!UICONTROL 售> We.Finance客戶]** 」，然後按一 **[!UICONTROL 下「匯入」]**。
   * 前往「銷 **[!UICONTROL 售> We.Finance Auto Insurance」]** ，然後按一 **[!UICONTROL 下「匯入」]**。
   * 前往「 **[!UICONTROL 銷售> We.Finance房屋抵押」]** ，然後按一 **[!UICONTROL 下「匯入」]**。
...
1. 前往Microsoft Azure Active Directory帳戶。 在註冊應用程式的「回覆URL」設定中 **[!UICONTROL 新增複製的雲端服務設定URL]** 。 保存配置。
...
1. 提供您的登入認證。 登入後，會將您重新導向至AEM Forms雲端服務設定頁面。 按一 **[!UICONTROL 下儲存並關閉]**。 雲端服務設定已儲存。
...
1. 將狀 **[!UICONTROL 態更新調度器表達式]****配置為0 0/2 &amp;ast;&amp;ast;&amp;ast;?**.
...
1. 前往「 **[!UICONTROL 工具>雲端服務> Adobe Sign >全域]**」。 選取 **[!UICONTROL AEM Forms Reference Site Sign]** ，然後點選「 **[!UICONTROL 屬性」]**。

   >[!CAUTION]
   >
   >請確定https://[host]:[ssl_port]/mnt/overlay/adobesign/cloudservices/adobesign/properties.html URL已新增至Adobe Sign API應用程式的OAuth設定的重新導向URL清單。
...
1. 根據您的環境更新主機和埠設定。
1. 儲存設定。
1. (僅&#x200B;**限作者例項**)前往 **[!UICONTROL 「工具」** >**Cloud Services**>**Data Sources**]**> **globalLignalScript。 選取 **[!UICONTROL 保留投資]** ，並點選 **[!UICONTROL 「屬性」]**。 點選「 **[!UICONTROL 驗證設定]** 」，並將「驗 **[!UICONTROL 證類型]** 」設 **[!UICONTROL 定為「基本驗證]**」。 指定 `admin`/ `admin`作為用於訪問服務的用戶名／密碼。 點選「 **[!UICONTROL 儲存並關閉]**」。
...
若要在參考網站中體驗此功能，請依照設定分析和報表中的說 [明設定您的Analytics帳戶](/help/forms/using/configure-analytics-forms-documents.md)。
...
1. 請確定AEM Cloud services中提供We.Finance和We.Gov分析設定。 您可透過下列其中一種方式找到雲端服務：
   ...
   * 在「雲 **[!UICONTROL 端服務]** 」頁面的「 **[!UICONTROL Adobe Analytics」區段下，按一下]**`Show Configurations`。 您可以看到We.Finance和We.Gov配置。 按一下以開啟配置。 在設定頁面中，按一下「 **[!UICONTROL 編輯]**」。 提供有效的公司、使用者名稱、共用密碼（密碼）和資料中心，然後按一 **[!UICONTROL 下「連線至Analytics」]**。 在獲得「連接成功」對話框後，按一下 **[!UICONTROL 配置對話框]** 中的「確定」。 在Analytics設定下設定架構，如設定Analytics和報 [告所述](/help/forms/using/configure-analytics-forms-documents.md)。
...
信用卡申請和住房抵押貸款申請使用案例演示了如何執行A/B測試並展示報告以進行演示。 如需逐步，請參 [閱We.Finance參考網站逐步說明](/help/forms/using/finance-reference-site-walkthrough.md)。
