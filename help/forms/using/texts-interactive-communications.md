...
* **重複**:您的互動式通訊中可能包含動態資訊，例如信用卡對帳單中的交易，其發生次數可能會隨著每個產生的互動式通訊而不斷變更。使用重複，可以格式化和構造此類動態資訊。 如需詳細資訊，請參閱[內嵌條件並重複](cm-inline-condition.md)。
...
* 要將FDM屬性插入文本中，請將游標置於要插入屬性的位置，然後點選左窗格中的&#x200B;**`[A]`**&#x200B;屬性，然後點選&#x200B;**`[B]`** **[!UICONTROL 添加選定項]**。 您也可以點選兩下屬性，將其插入&#x200B;**`[C]`**&#x200B;游標位置。 表單資料模型屬性會以褐色背景色彩反白顯示。

* 要允許代理在[使用代理UI準備和發送Interactive Communication](/help/forms/using/prepare-send-interactive-communication.md)時編輯代理UI中的FDM屬性值，請為該屬性點選&#x200B;**`[D]`**&#x200B;鎖定表徵圖，並確保該屬性處於解鎖狀態。 屬性的預設狀態已鎖定，且代理無法在代理UI中編輯屬性。
