...
條件是可包含在「互動式通訊」中的檔案片段。 其他檔案片段為 [文字](/help/forms/using/texts-interactive-communications.md)、清單和版面片段。 條件可讓您根據提供的資料和規則，定義一或多個內容相關的資產，以納入互動式通訊中。
...
1. 指定下列資訊：
   ...
   * **[!UICONTROL 表單資料模型]**:（可選）選擇「表單資料模型」單選按鈕，以基於表單資料模型建立條件。 選擇「表單資料模型」單選按鈕時，將 **[!UICONTROL 顯示「表單資料模型*]** 」欄位。 瀏覽並選取表單資料模型。 在建立互動式通訊的條件時，請確定您使用的資料模型與您要在互動式通訊中使用的資料模型相同。 如需表單資料模型的詳細資訊，請參 [閱資料整合](/help/forms/using/data-integration.md)。
...
1. 點選「 **[!UICONTROL 儲存]**」。
   ...
   條件隨即建立。 現在，您可以在建立互動式通訊時，繼續將條件當做建置區塊。
...
1. 輸入下列資訊並點選「 **[!UICONTROL 建立]**:
   ...
   * **[!UICONTROL 僅允許特定值]**:對於字串和數字變數，您可以確保代理從特定值集中選擇，以用於代理UI中的預留位置。 若要指定值集，請選取此選項，然後指定「值*」欄位中允許的逗號 **[!UICONTROL 分隔值]** 。
