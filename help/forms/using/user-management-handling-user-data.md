...
用戶管理將用戶資料儲存在資料庫中，如My Sql、Oracle、MS SQL Server和IBM DB2。 此外，任何使用者只要至少在AEM作者的Forms應用程式中登入一次， `https://[*server*]:[*host*]/lc`，則會在AEM存放庫中建立使用者。 因此，用戶管理儲存在以下資料儲存中：
...
至少曾存取下列Forms應用程式一次的使用者的使用者管理資料 `https://[*server*]:[*host*]/lc` 也會儲存在AEM存放庫中。
