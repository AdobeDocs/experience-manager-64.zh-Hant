...
JavaScript是最適化表單的運算式語言。 所有運算式都是有效的JavaScript運算式，並使用最適化表單指令碼模型API。 這些運算式會傳回特定類型的值。 如需最適化表單類別、事件、物件和公用API的完整清單，請參閱[最適化表單的JavaScript程式庫API參考](https://helpx.adobe.com/aem-forms/6/javascript-api/index.html)。
...
* **[值提交指令碼](/help/forms/using/adaptive-form-expressions.md#p-value-commit-script-p):** 在更改欄位值後更改表單的元件。
...
**範例**:要根據另一個欄位的值填充欄位的值，請提供一個簡單的選項表達式。例如，若要填入欄位&#x200B;**Number of Kids**，根據另一欄位中表示的&#x200B;**Firmeny Status**，運算式為：

**`marital_status.value == "married" ? ["1=One", "2=two"] : ["0=Zero"]`.**

每當&#x200B;**frimery_status**&#x200B;欄位的值變更時，就會擷取運算式。 您也可以從REST服務填入下拉式清單。 如需詳細資訊，請參閱[動態填入下拉式清單](/help/forms/using/dynamically-populate-dropdowns.md)。
...
要使元件成為必備元件，可在元件的&#x200B;**[!UICONTROL 編輯]**&#x200B;對話框中，選擇選項&#x200B;**[!UICONTROL 標題和文本>必需]**。 您也可以新增適當的&#x200B;**必要訊息**（選用）。.

### 驗證模式{#validation-patterns}

欄位有多個可用的現成可用驗證模式。 要選擇驗證模式，在元件的&#x200B;**[!UICONTROL 編輯]**&#x200B;對話框中，找到&#x200B;**[!UICONTROL 模式]**&#x200B;部分並選擇&#x200B;**[!UICONTROL 模式]**。 您可以在&#x200B;**Pattern**&#x200B;文字方塊中建立您自己的自訂驗證模式。 僅當填入的資料符合驗證模式，否則返回&#x200B;**False**&#x200B;時，才返回驗證狀態。 ****&#x200B;若要編寫您自己的自訂驗證模式，請參閱[Picture子句支援HTML5表單](/help/forms/using/picture-clause-support.md)。
...
「顯示格式」可用來顯示不同格式的資料。 例如，您可使用顯示格式來顯示含連字型大小、郵遞區號格式或日期選擇器的電話號碼。 這些顯示模式可以從元件的&#x200B;**[!UICONTROL 編輯]**&#x200B;對話框的&#x200B;**[!UICONTROL 模式]**&#x200B;部分中選擇。 您可以編寫類似上述驗證模式的自訂顯示模式。
