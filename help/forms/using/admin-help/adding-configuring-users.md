...
使用MySQL時，用戶ID可以包含擴展字元。 但是，當比較兩個字串（例如abcde和abcdè）時，它們會被視為相同。 例如，同步時，如果新用戶已添加到資料庫，則會進行比較以檢查資料庫中是否存在具有相同用戶ID的用戶。 如果在 *新增新用戶* *âbcdè *時，資料庫中已存在用戶簡碼，則無法區分這兩個名稱。 假定用戶已存在於資料庫中，並且新用戶被忽略且不添加。
