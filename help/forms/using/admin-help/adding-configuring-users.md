...
使用MySQL時，用戶ID可以包含擴展字元。 不過，當在兩個字串（例如abcde和âbcdè）之間進行比較時，兩者會視為相同。 例如，同步時，如果新用戶被添加到資料庫，則會進行比較以檢查資料庫中是否存在具有相同用戶ID的用戶。 若使用者 *abcde* 新增新使用者*âbcdè *時，資料庫中已存在，比較無法區分這兩個名稱。 假設該用戶已存在於資料庫中，並且會忽略新用戶，而不會添加。
