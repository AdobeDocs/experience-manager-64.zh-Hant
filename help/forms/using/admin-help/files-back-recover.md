...
* IBM DB2封存記錄加速器
* IBM DB2資料封存專家(請參閱 [IBM DB2 Data Archive Expert使用手冊與參考](https://publib.boulder.ibm.com/infocenter/mptoolic/v1r0/topic/com.ibm.db2tools.aeu.doc.ug/ahxugb13.pdf?noframes=true).)

DB2具有將資料庫備份到Tivoli Storage Manager的內置功能。 通過使用Tivoli Storage Manager, DB2備份可以儲存在其他介質或本地硬碟上。

有關DB2資料庫備份和恢復的詳細資訊，請參見 [為DB2制定備份和恢復策略](https://publib.boulder.ibm.com/infocenter/db2luw/v9/index.jsp?topic=/com.ibm.db2.udb.admin.doc/doc/c0005945.htm).
...
>[!NOTE]
>
>MySQL的預設二進位記錄模式為「語句」，與Content Services使用的表不相容（已廢止）。 在此預設模式下使用二進位記錄會導致內容服務（已過時）失敗。 如果您的系統包含內容服務（已淘汰），請使用「混合」記錄模式。 若要啟用「混合」記錄，請將下列引數新增至my.ini檔案：
>
...
內容儲存根目錄的預設位置為 *[aem-forms根]*/lccs_data。
...
內容儲存根目錄的預設位置為 *[GDS根]*/lccs_data，其中 *[GDS根]* 是 [GDS位置](files-back-recover.md#gds-location). 備份位於「內容儲存根目錄」中的以下目錄：
...
索引根目錄的預設位置為 *[aem-forms根]*&#x200B;每個節點上的/lucene-indexes。
...
>[!NOTE]
>
>依預設，隨AEM表單安裝的Adobe字型位於 [aem-forms根]/fonts目錄。
