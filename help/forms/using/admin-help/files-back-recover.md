...
>[!NOTE]
>
>MySQL的預設二進位日誌記錄模式是「語句」，它與Content Services使用的表（已過時）不相容。 在此預設模式中使用二進位記錄會導致Content Services（已過時）失敗。 如果您的系統包含Content Services（已過時），請使用「混合」記錄模式。 要啟用「混合」日誌，請將以下引數添加到my.ini檔案：
>
...
「內容儲存根目錄」的預設位置為&#x200B;*[aem-forms root]*/lccs_data。
