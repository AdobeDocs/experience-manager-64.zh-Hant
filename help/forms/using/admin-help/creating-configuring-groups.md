...
* 啟用根據電子郵件網域（例如@adobe.com）自動建立動態群組。 當您啟用此功能時，「使用者管理」會針對AEM表單資料庫中的每個唯一電子郵件網域建立動態群組。 使用cron運算式來指定「使用者管理」在AEM表單資料庫中搜尋新電子郵件網域的頻率。 這些動態群組會新增至DefaultDom本機網域，並命名為「所有具有*[電子郵件網域] *mail ID的使用者」。
