...
1. 點選「互動式通訊」編輯器中的圖表元件，然後從「元件」工具列選取「**[!UICONTROL 設定」(]** ![configure_icon](assets/configure_icon.png))。
...
1. 為打印通道和Web通道配置圖表的基本屬性。 除了通用屬性，還有專用於打印和Web通道以及圖表類型的屬性。
   ...
   * 為&#x200B;**[!UICONTROL x軸]**&#x200B;和&#x200B;**[!UICONTROL y軸]**&#x200B;指定下列項目：
      ...
      * **[!UICONTROL 函式]**:要使用統計函式計算軸上的值，請選擇X/Y軸的函式。有關函式的詳細資訊，請參閱圖表](#usefunction)和[示例2中的[使用函式：求和和和平均函式線上圖](#applicationsumfrequency)中的應用。
...
1. （僅打印通道）在「代理設定」中，指定代理是否必須使用此圖表。 如果未選擇「代理使用此圖表」選項，則代理可以點選「代理UI」的「內容」頁簽中圖表的眼睛表徵圖，以顯示/隱藏圖表。****
...
* **[!UICONTROL 圖表類型]**  — 在此示例中，打印管道的列和Web管道的環圈圖
* **[!UICONTROL 資料模]** 型對象作為圖表的X和Y軸的源 — 在此示例中，X軸的事務處理金額和Y軸的費用名稱
* **** X和Y軸的標題（僅在此示例中針對打印通道中的列類型圖表） — 在此示例中，X軸的金額($)和Y軸的費用。
* **[!UICONTROL 標籤方向]** （僅在此示例中，針對打印通道中的列類型圖表） — 在此示例中  `Tilt Left`

* **** 工具標籤顯示在費用的滑鼠上（僅限Web渠道） — 在此示 `${x}: $ ${y}`例中，它顯 `[Expense Label: $ Amount]` 示為(示例：主題公園參觀：315美元)

![互動式通信打印輸出中的](assets/chartprintchannel.png)
**清單圖** *：互動式通信打印輸出中的清單*

**A.** Y軸 — 從表單資料模型屬性中擷取的金額，以及設為「金額($)」的「標題」屬性 **B.** 將X軸的標籤方向設定為「傾斜左」 **C.** X軸 — 從表單資料模型屬性中擷取的費用說明，以及設為「費用」的「標題」屬性

![互動式通信的Web輸出中的環](assets/chartwebchannel.png)
**形圖圖：** *互動式通信的Web輸出中的環形圖*

**A.** 環圈的「內半徑」屬性已設 **定B.** 已選取「顯示圖例」屬性，且「圖例位置」屬性已設定為「右側 **C」。** 工具提示在滑鼠上顯示項的詳細資訊 — 工具提示設定為${x}:$ ${y}

## 範例2:和頻函式線上圖中的應用 {#applicationsumfrequency}

在圖表中套用函式，即可繪製非表單資料模型直接提供的資料。 在此示例中，我們使用信用卡對帳單示例來了解如何將總和和頻率函式應用於圖表。

![不含帶有三個「Bed and Breakfast」事務處理的函式的折](assets/creditcarddatalinechartcopy.png)
**線圖圖：** *不含帶有三個「Bed and Breakfast」事務處理的函式的折線圖*
...
**** 名稱圖表元素的識別碼。圖表上不會顯示名稱，但在參考其他元件、指令碼和SOM運算式中的元素時有幫助。

**標題（僅限列印管道）** 指定圖表的標題。

**圖** 表類型指定要生成的圖表類型。可用選項有餅圖、列、環圈圖、條形圖（僅限Web通道）、線條圖、線條圖和點、點和區域圖。 如需詳細資訊，請參閱範例1:打印和Web中的圖表輸出。

**X軸>標** 題指定X軸的標題。

**X軸>資料模型對象(&amp;** A)；指定要在X軸上繪製的表單資料模型收集項目的名稱。

**X軸>函** 式指定用於計算X軸上值的統計/自定義函式。如需函式的詳細資訊，請參閱圖表中的使用函式和範例2:求和函式和均值函式線上圖中的應用

**X軸>打印通** 道圖表上標籤的標籤方向。如果選擇標籤的方向作為「自定義旋轉」(Custom Rotation)，則會出現「自定義旋轉角度（度）」(Custom Rotation Angle(deges))欄位。 在「自定旋轉角度（度）」(Custom Rotation Angle(degeres))欄位中，可以按15度的步驟選擇旋轉角度。

**Y軸>標** 題指定Y軸的標題。

**Y軸>資料模型對象(&amp;A)** ；指定要在Y軸上繪製的表單資料模型收集項。在「打印」通道中，Y軸的資料模型對象應為「編號」類型。

**Y軸>函** 式指定用於計算Y軸上值的統計/自定義函式。如需函式的詳細資訊，請參閱圖表中的使用函式和範例2:求和函式和均值函式線上圖中的應用

**顯** 示圖例啟用時顯示圓形圖或環圈圖的圖例。

**圖** 例位置指定圖例相對於圖表的位置。可用選項有「右」、「左」、「上」和「下」。
...
**工具提示（僅限Web通道）** 指定工具提示出現在Web通道圖表中資料點上滑鼠移動器的格式。預設值為\${x}(\${y})。 根據圖表類型，當您將滑鼠指向圖表中的點、條或切片時，變數\${x}和\${y}將動態替換為x軸和y軸上的相應值，並顯示在工具提示中。

要禁用工具提示，請將工具提示欄位留空。 此選項不適用於折線圖和區域圖。 例如，請參閱[範例1:打印和Web](#chartoutputprintweb)中的圖表輸出。

**CSS類（僅限Web通道）** 在CSS類欄位中指定要將自定義樣式應用於圖表的CSS類的名稱。

**強制分頁前（僅限列印管道）** 選取此選項，在圖表前新增強制分頁，並將圖表放在新頁面的頂端。

**強制分頁後（僅限列印管道）** 選取此選項可在圖表後新增強制分頁，並將圖表後的內容放在新頁面頂端。

**縮排（僅限列印管道）** 從頁面左側指定圖表的縮排。

**圖表特定** 配置除了常見配置外，還提供以下圖表特定配置：
...
**** 模式傳回值，在X或Y軸上，最常出現另一個軸上指定值的值

### Web通道{#custom-functions-in-web-channel}中的自定義函式
