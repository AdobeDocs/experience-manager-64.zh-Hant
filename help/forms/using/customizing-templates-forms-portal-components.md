...
1. 在*/apps *下建立sling:Folder節點
   ...
   *注意：標題可能與您建立的sling:Folder的節點名稱不同。 *
...
每個佔位符都有一個專用的OOTB元資料集。 若要在表單上的特定位置顯示自訂中繼資料，請在該位置新增&#x200B;**$metadata_prop屬性**。
...
* **htmlStyle**  &amp;  **pdfStyle**:用於呈現的HTML和PDF表徵圖的顯示樣式。有效值為&quot;**__FP_display_none**&quot;或&#x200B;**blank**
...
1. **本地化支援**:若要本地化任何靜態文 **字，請使用屬性${localize-***YOUR_TEXT***}** ，並使本地化值可用（如果尚未存在）。

   *在討論的示例中，屬性${localize-Apply}和${localize-Download}用於本地化應用和下載文本。*
...
1. 請確認任何自訂中繼資料或現有中繼資料的名稱不包含冒號(:)。 若有，則無法在使用者介面上顯示。
