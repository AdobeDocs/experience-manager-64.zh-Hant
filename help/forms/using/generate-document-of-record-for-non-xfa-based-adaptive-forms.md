...
請參閱記錄的文檔的[基本模板](#base-template-of-a-dor)
...
1. 在「表單模型」頁簽的「記錄模板配置文檔」部分，選擇以下選項之一。
   ...
   **[!UICONTROL 選擇此選項可將XDP檔案用作記錄文檔的樣式和外觀的基本模板。選取此選項時，會顯示AEM Forms存放庫中可用的所有XDP檔案。 選擇相應的檔案。]**

   **選擇「Forms模板」作為生成記錄文檔的基** 本模板選擇此選項可將XDP檔案用作定義記錄文檔的樣式和外觀的基本模板。選取此選項時，會顯示AEM Forms存放庫中可用的所有XDP檔案。 選取適當的檔案。
