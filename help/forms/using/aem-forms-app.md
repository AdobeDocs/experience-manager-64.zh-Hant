...
如需使用AEM Forms伺服器之AEM Forms應用程式的詳細資訊，請參閱refsite檔案中的[ Conard gets assessment task](/help/forms/using/gov-reference-site-walkthrough.md#contard-assessment-task) 。
...
使用AEM Forms工作流程伺服器，表單可以與Workbench程式和AEM收件匣應用程式中的起始點相關聯。 AEM收件匣應用程式可以有與其相關聯的最適化表單。 起始點可以具有與其相關聯的最適化表單、HTML5表單或表單集。 起始點可以作為任務提交，或者任務可以另存為草稿。 如需AEM收件匣應用程式與起始點之間差異的詳細資訊，請參閱[OSGi和AEM Forms JEE工作流程上以表單為中心的AEM工作流程的動作和功能](/help/forms/using/capabilities-osgi-jee-workflows.md)。
