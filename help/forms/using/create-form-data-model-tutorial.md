...
* [設定資料庫](#step-set-up-the-database)
...
* 具有[設定資料庫](#step-set-up-the-database)部分中所述示例資料的MySQL資料庫。
* MySQL JDBC驅動程式的OSGi捆綁包，如[捆綁JDBC資料庫驅動程式](https://helpx.adobe.com/experience-manager/6-3/sites-developing/jdbc.html#bundling-the-jdbc-database-driver)中所述
...
您可以設定不同類型的資料來源以建立表單資料模型。 在本教程中，您將配置已配置並填入示例資料的MySQL資料庫。 如需其他支援資料來源的詳細資訊以及如何設定這些資料來源，請參閱「AEM Forms資料整合」。[](data-integration.md)
...
1. 將MySQL資料庫配置為資料源：
   ...
   1. 在設定對話方塊中，指定下列詳細資訊：
      ...
      * **驗證查詢：** 指定SQL SELECT查詢以驗證池中的連接。查詢至少必須返回一行。 例如，**選擇&amp;ast;來自客戶**。
...
AEM Forms提供直覺式使用者介面，以從設定的資料來源建立表單資料模式。 [](data-integration.md)您可以在表單資料模型中使用多個資料來源。 在本教程中，您將使用MySQL作為資料源。
