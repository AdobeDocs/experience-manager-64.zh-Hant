...
在「通 [信管理」中建立信函模板後](/help/forms/using/create-letter.md)，最終用戶／代理／索賠調整器可以在「建立通信」用戶介面中開啟信函，並通過輸入資料、設定內容和管理附件來建立通信。 最後，理賠代理可以在預覽模式下管理內容並提交信件。
...
1. 對於基於「資料字典」的字母，請選 **擇「預覽** 」>「 **預覽」**。 或者，對於非資料字典型字母，請選擇「預 **覽」**。 您也可以將滑鼠指標暫留在字母上（不需選取它），並點選「字母預覽」圖示來預覽。
   ...
   >[!NOTE]
   >
   >如果資料字典未與字母關聯，字母預覽會開啟。 否則，如果字母是基於資料字典的，則「對應管理」會在「預覽」菜單中顯示「預覽」和「自定義」選項，您可以選擇兩個選項之一。 您也可以將測試資料與資料字典建立關聯。 當「資 [料字典」有關聯的測試資料](/help/forms/using/data-dictionary.md#p-working-with-test-data-p)，則選取預覽選項時，一般預覽會開啟並填入測試資料。
...
1. 視需要在欄位中輸入資料和內容變數。 填寫所有標有星號(&amp;ast;)的必填欄位，以啟用「提 **交** 」按鈕。
...
1. 要將字母另存為草稿（您稍後可繼續處理），請按一下「另存為草稿」。 若要使用此選項，您的信件必須 [發佈](/help/forms/using/publishing-unpublishing-forms.md#publishanasset)。 如需詳細資訊，請參閱「儲存草稿 [並送出信函例項」下方的「草稿例項」](#savingdrafts)。
...
1. 如果您發現平面化的PDF預覽效果令人滿意，請點選「 **Submit** 」以送出信函。 或者，若要變更字母，請點選「退出預覽 **** 」，返回字母的「建立對應UI」預覽，以變更字母。 當您點選「送出」時，如果「發佈」例項上啟用「管理字母例項」設定，則會產生送出字母例項。
   ...
   如需HTML5表單和PDF表單之間差異的詳細資訊，請參 [閱HTML5表單和PDF表單的功能區隔](/help/forms/using/feature-differentiation-html5-forms-pdf-forms.md)。
