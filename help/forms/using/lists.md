...
1. 文字編輯器隨即開啟，請輸入文字。 使用頁面頂端的工具列來設定文字、插入條件、連結和分頁格式。
   ...
   * **連結**:在文字中插入超文字連結。 點選 **[!UICONTROL 連結]**，請在 **[!UICONTROL URL]** 欄位，從 **[!UICONTROL 替代文字]** 欄位，然後點選 ![儲存](assets/save_icon.svg).
...
**注意：** 如果在「查找」文本框中輸入任何特殊字元，則禁用「全字」選項。
...
1. 點選 **表格** ，並指定版面的下列資訊：
   ...
   * **配置**:選擇要配置的表。 如果表為靜態表，則下拉式清單中表名的尾碼為（靜態）；如果表為動態表，則為（動態）。 靜態表包含固定的行數。 靜態表可以包含目標區域和欄位。 這些目標區域和欄位不能綁定到重複的DDE。 綁定到表單元格的資料決定動態表的行數。
   ...
   * **寬比：** 表列寬的比率。
   ...
   如需在版面片段中使用靜態和動態表格的詳細範例，請參閱 [範例檔案：在字母中使用靜態和動態表](create-letter.md#insert-data-modules-and-layout-fragments-in-a-letter-and-configure-them).
