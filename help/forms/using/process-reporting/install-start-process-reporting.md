...
# {#getting-started-with-process-reporting}流程報告快速入門
...
有關預定義報表的詳細資訊，請參閱[ Pre-defined Reporting in Process](pre-defined-reports-in-process-reporting.md)。

**臨機查詢：** 此項目包含對進程和任務執行基於篩選的搜索的選項。

如需臨機查詢的詳細資訊，請參閱「處理中的臨機查詢」[](adhoc-queries-in-process-reporting.md)。
