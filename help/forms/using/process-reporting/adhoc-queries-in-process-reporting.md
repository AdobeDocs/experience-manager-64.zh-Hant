...
1. 要建立查詢，請按一下&#x200B;**添加**(+)。
   ...
   **要定義篩選器**
   ...
   1. 要向查詢添加另一個篩選器，請按一下篩選器行右側的&#x200B;**Add(+)**。

      要從查詢中刪除篩選器，請按一下篩選器行右側的&#x200B;**Delete(-)**。
...
1. 執行進程／任務搜索。 有關詳細資訊，請參閱[要執行進程搜索查詢](/help/forms/using/process-reporting/adhoc-queries-in-process-reporting.md#p-to-execute-a-search-query-p)。
