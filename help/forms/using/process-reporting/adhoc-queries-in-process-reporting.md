...
1. 若要建立查詢，請按一下 **新增** (+)。
   ...
   **定義篩選**
   ...
   1. 若要新增其他篩選器至查詢，請按一下 **Add(+)** 在篩選列的右側。

      若要從查詢中移除篩選器，請按一下 **Delete(-)** 在篩選列的右側。
...
1. 執行進程/任務搜索。 如需詳細資訊，請參閱 [執行進程搜索查詢](/help/forms/using/process-reporting/adhoc-queries-in-process-reporting.md#p-to-execute-a-search-query-p).
