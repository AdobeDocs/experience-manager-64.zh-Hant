...
1. 更新 `integration/javascript` 資料夾（視需要）。
   ...
   * 此 `getCommitValue` 方法在 `commit`事件。 一般來說，它是退出事件，但下拉式清單、選項按鈕和核取方塊元素在變更時發生除外)。 如需詳細資訊，請參閱 [適用性Forms運算式](/help/forms/using/adaptive-form-expressions.md#p-value-commit-script-p).
