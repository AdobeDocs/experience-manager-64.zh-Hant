...
# 建立最適化表單欄位{#create-custom-appearances-for-adaptive-form-fields}的自訂外觀
...
1. 視需要更新`integration/javascript`資料夾中的JavaScript檔案。
   ...
   * 當`commit`事件發生時，適用性表單架構會呼叫`getCommitValue`方法。 一般來說，它是退出事件，但下拉式清單、選項按鈕和核取方塊元素在變更時發生除外)。 如需詳細資訊，請參閱[適用性Forms運算式](/help/forms/using/adaptive-form-expressions.md#p-value-commit-script-p)。
