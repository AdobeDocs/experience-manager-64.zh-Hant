...
Microsoft Dynamics是客戶關係管理(CRM)和企業資源規劃(ERP)軟體，提供企業解決方案，以建立和管理客戶帳戶、連絡人、潛在客戶、商機和案例。 [AEM Forms Data ](/help/forms/using/data-integration.md) Integration提供OData雲端服務設定，將Forms與線上和內部Microsoft Dynamics伺服器整合。它可讓您根據Microsoft Dynamics服務中定義的實體、屬性和服務來建立表單資料模型。 表單資料模型可用來建立與Microsoft Dynamics伺服器互動的最適化表單，以啟用商業工作流程。 例如：
...
只有當AEM例項的執行模式設為`samplecontent`（預設值）時，OData Cloud服務和表格資料模型（含預先設定的Microsoft Dynamics實體和服務）才可在您的AEM Forms例項上使用。 如需設定AEM例項執行模式的詳細資訊，請參閱[執行模式](https://helpx.adobe.com/in/experience-manager/6-4/sites-deploying/configure-runmodes.html)。

## 必備條件 {#prerequisites}

在開始設定和設定Microsoft Dynamics之前，請確定您有：

* 已安裝[AEM 6.4 Forms add-on套件](https://helpx.adobe.com//experience-manager/6-4/forms/using/installing-configuring-aem-forms-osgi.html)
...
1. 選擇&#x200B;**[!UICONTROL MS Dynamics OData Cloud Service(OData Service)]**&#x200B;配置，然後按一下&#x200B;**[!UICONTROL Properties]**。 雲端服務設定屬性對話方塊隨即開啟。
...
若要檢閱表單資料模型，請前往&#x200B;**[!UICONTROL 表單>資料整合]**。 選擇&#x200B;**[!UICONTROL Microsoft Dynamics FDM]**&#x200B;並按一下&#x200B;**[!UICONTROL 編輯]**&#x200B;以在編輯模式下開啟表單資料模型。 或者，您也可以直接從下列URL開啟表單資料模型：
