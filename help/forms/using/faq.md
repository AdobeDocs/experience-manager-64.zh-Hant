...
# HTML5表單{#frequently-asked-questions-faq-for-html-forms}的常見問題(FAQ)
...
1. 為什麼條碼和簽名欄位不會出現在我的表單中？
   ...
   答案：條碼和簽名欄位與HTML或行動藍本無關。 這些欄位會顯示為非互動區域。 不過，AEM Forms Designer提供新的簽名塗鴉欄位，可供使用，而非簽名欄位。 您也可以新增條碼的[自訂Widget](/help/forms/using/custom-widgets.md)並加以整合。
...
1. 是否有建議的命名慣例，以及是否有要避免的保留關鍵字？
   ...
   * 在AEM Forms Designer中，建議您不要以底線(_)開始物件名稱（例如子表單或文字欄位）。 要在名稱的開頭使用下划線，請在下划線後面添加前置詞*_&lt;prefix>&lt;objectname>。*
...
1. HTML5表格是否支援浮動欄位？
   ...
   1. 新增String類型的屬性`mfDataDependentFloatingField`，並將屬性值設為&#x200B;`true`**。**
