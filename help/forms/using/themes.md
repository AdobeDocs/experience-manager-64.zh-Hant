...
1. 在「主題」頁中，按一下「建立」>「主題」]**。**[!UICONTROL 

   啟動建立主題的精靈。

1. 在「建立主題」嚮導的「基本」頁簽中，提供主題的&#x200B;**[!UICONTROL Title]**&#x200B;和&#x200B;**[!UICONTROL 名稱]**。 這些是必填欄位。

1. 在「進階」標籤中，您會看到兩個欄位：

   * **[!UICONTROL Clientlib位置]**:儲存主題之clientlib的存放庫位置。
   * **[!UICONTROL Clientlib類別]**:提供文本欄位以輸入主題的clientlib類別名稱。

1. 按一下「**[!UICONTROL 建立]**」，然後按一下「**[!UICONTROL 編輯]**」以開啟主題編輯器中的主題，或按一下「**[!UICONTROL 完成]**」以返回主題頁。
...
1. 在「主題」頁中，選擇主題，然後按一下&#x200B;**[!UICONTROL Download]**。 此時將顯示一個對話框，其中包含主題的詳細資訊。
...
1. 在「主題」頁中，按一下「**[!UICONTROL 建立>檔案上載]**」。
1. 在「檔案上載」提示符中，瀏覽並選擇電腦上的主題包，然後按一下&#x200B;**[!UICONTROL Upload]**。
  ...
  <tr> 
   ...
   <td> 
    <ul> 
     ...
     <li>當此值更改時，clientlib節點結構將移至輸入的新位置。<br /> <em><strong>注意：</strong> 如果您變更預設clientlib位置，請在CRXDE存放庫中將 <code>crx:replicate, rep:write, rep:glob:*, rep:itemNames:: js.txt, jcr:read </code>指 <code>forms-users</code> 派給新位 <code>crx:replicate</code>置 <code>jcr:read </code>中的， <code>fd-service</code> 並指派給新位置中的。另外，通過為<code>forms-user</code></em>添加<span class="kbd">deny jcr:addChildNodes</span>來附加另一個ACL</li> 
    </ul> </td> 
  </tr> 
  <tr> 
   <td>13.</td> 
...
* **畫布**  — 顯示在右側。它會顯示範例最適化表單或互動式通訊，讓所有樣式變更都能立即反映。 您也可以直接從畫布選取物件，以尋找與物件相關聯的樣式，並編輯這些樣式。 頂部的設備解析度標尺控制畫布。 從尺標中選擇解析度斷點顯示示例表單的預覽或相應解析度的交互通信。 畫布在](/help/forms/using/themes.md#using-canvas)下方詳細討論。[
...
* **側欄** — 顯示在左側。它有下列項目：
   ...
   * **選取器：** 顯示為樣式選取的元件，及其您可設定樣式的屬性。選取器代表類型的所有元件。 如果您在主題中選擇樣式的文本框元件，則窗體或互動通信中的所有文本框都將繼承樣式。 選取器可讓您選取一般元件或樣式的特定元件。 例如，欄位元件是一般元件，文字方塊是特定元件。
      ...
      **樣式特定元件**:
      ...
      特定元件會影響特定類別的對象。 在主題中設定數字框元件的樣式時，只有中的數字框對象會繼承樣式。
   ...
   * **檢視CSS**:可讓您檢視所選元件的CSS
   ...
   此外，在側欄底部會顯示箭頭。 按一下箭頭時，您還會看到兩個選項：**模擬成功**&#x200B;和&#x200B;**模擬錯誤。** 以下將詳細說明這些選項以及上述 [選項](/help/forms/using/themes.md#using-rail)。
...
在資產資料庫中新增樣式時，其他主題和表單編輯器或互動式通訊編輯器UI的[樣式模式](/help/forms/using/inline-style-adaptive-forms.md)中都可使用樣式。 同樣地，使用表單編輯器或互動式通訊編輯器UI的樣式模式來設定元件的樣式時，會快取樣式，並可在主題中使用。
...
1. 在「畫布」工具欄上，按一下「主題選項」]**![theme-options](assets/theme-options.png) >**[!UICONTROL &#x200B;管理樣式」]**。**[!UICONTROL 
1. 在管理樣式對話框中，選擇保存的樣式，按一下&#x200B;**[!UICONTROL 刪除]**。

   ![刪除保存的樣式](assets/manage-styles.png)

### 即時預覽、儲存和捨棄變更{#live-preview-save-and-discard-changes}

在樣式中所做的修改會立即反映在畫布中載入的表單或互動式通訊中。 即時預覽可讓您以互動方式定義及查看樣式的影響。 變更元件的樣式時，側欄中會啟用&#x200B;**[!UICONTROL Done]**&#x200B;按鈕。 要保留更改，請使用&#x200B;**[!UICONTROL Done]**&#x200B;按鈕。
...
1. 在「常規」頁簽中，瀏覽並選擇&#x200B;**[!UICONTROL 適用性表單/文檔]**&#x200B;欄位的表單或互動式通信。
...
主題編輯器可讓您編輯您建立或上傳的主題。 導覽至「**[!UICONTROL Forms與檔案>主題]**」，然後選取主題並開啟。 主題在主題編輯器中開啟。
...
* **[!UICONTROL 切換「側]** ![面板」切換「側面板」](assets/toggle-side-panel.png):讓您顯示或隱藏側欄。

* **[!UICONTROL 主題]** ![選項主題選項](assets/theme-options.png):提供三個選項

   * 配置：提供選項，用於選擇預覽表單或互動式通信、基本clientlib和typekit配置。
   * 查看主題CSS:為所選主題生成CSS。
   * 管理樣式：提供管理文字和影像樣式的選項
   * 說明：執行主題編輯器的影像引導導覽。
...
* **[!UICONTROL 數值方塊Widget]**:可讓您選取元件以自訂側欄的外觀。
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td><p>尺寸及位置</p> </td> 
   <td><p>可讓您設定主題中元件的對齊方式、大小、位置和放置方式。 </p> <p>選項包括顯示設定、邊框間距、邊距、寬度、高度和Z索引。 </p> </td> 
  </tr> 
  <tr> 
   <td><p>文字</p> </td> 
...
* **模擬成功**:
   ...
   如果您為成功狀態指定其樣式，可讓您查看元件的外觀。 例如，在表單中，客戶設定密碼。 用戶可以根據您提供的准則設定密碼。 當使用者依照您提供的所有准則輸入密碼時，文字方塊會變成綠色。 文字方塊變成綠色時，就會處於成功狀態。 您可以為處於成功狀態的元件指定樣式，並使用「模擬成功」選項來模擬其外觀。

* **模擬錯誤**:

   如果您為錯誤狀態指定其樣式，可讓您查看元件的外觀。 例如，在表單中，客戶設定密碼。 用戶可以根據您提供的准則設定密碼。 當用戶鍵入的密碼不符合您提供的所有准則時，文本框將變為紅色。 文字方塊變成紅色時，會處於錯誤狀態。 您可以為處於錯誤狀態的元件指定樣式，並使用「模擬錯誤」選項來模擬其外觀。
...
1. 選取數值方塊時，您會看到包含三個選項的元件工具列：
   ...
   * ![欄位層級](assets/field-level.png)

1. 選擇&#x200B;**[!UICONTROL 數字框Widget]**。
1. 側欄標題變更為「數值框小部件」，並顯示自定義其外觀的選項。

   在側邊欄中使用&#x200B;**[!UICONTROL Dimension和位置]**&#x200B;選項來自訂元件的大小。 確保狀態為&#x200B;**[!UICONTROL Default]**。

選擇元件工具欄中的&#x200B;**[!UICONTROL 欄位介面工具集]**，並執行上述步驟，而不選擇&#x200B;**[!UICONTROL 數字框介面工具集]**。 當您為&#x200B;**[!UICONTROL 欄位介面工具集]**&#x200B;選項選擇維度時，除數字框外的所有文本框都具有相同的大小。
...
1. 在側欄中選取狀態。 例如，錯誤狀態。
1. 在側邊欄中使用&#x200B;**[!UICONTROL 邊框、背景]**&#x200B;等選項來自訂元件的外觀。
1. 使用側欄底部的&#x200B;**[!UICONTROL 模擬錯誤]**&#x200B;選項，查看樣式在編輯中的外觀。

在指定元件狀態後自訂元件的樣式時，只會針對指定狀態顯示元件的自訂。 例如，如果您在選取暫留狀態時自訂元件的樣式。 當您在已轉譯的表單或您套用主題的互動式通訊中，將指標移至元件上時，元件便會顯示自訂。

若要模擬錯誤和成功以外的狀態行為，請使用「預覽」模式。 若要使用「預覽」模式，請按一下頁面工具列中的&#x200B;**[!UICONTROL 預覽]**。
...
您現在可以透過最適化表單或互動式通訊，使用網路服務中可用的字型。 現成可用的[Typekit](https://typekit.com/)Adobe的Web字型服務可作為配置使用。 要使用Typekit，請在其中建立套件和字型，然後從[Typekit網站](https://typekit.com/)獲取套件ID。

執行下列步驟以在AEM中設定Typekit:

1. 在製作例項中，按一下「![adobeexperiencemanager](assets/adobeexperiencemanager.png)**[!UICONTROL Adobe Experience Manager>工具&#x200B;]**![hammer](assets/hammer.png)**[!UICONTROL >部署>Cloud Services ]**」。
1. 在&#x200B;**[!UICONTROL Cloud Services]**&#x200B;頁面，導覽至&#x200B;**[!UICONTROL 第三方服務]** > **[!UICONTROL Typekit]** ，然後按一下Typekit下的&#x200B;**[!UICONTROL 立即配置]** 。 如果配置已可用，請按一下&#x200B;**+**&#x200B;按鈕以建立新實例。
...
1. 在「**[!UICONTROL Typekit配置]**」欄位中，選擇一個工具包，然後按一下「**[!UICONTROL 保存]**」。
...
1. 在編輯模式中開啟表單。 要在編輯模式下開啟表單，請選擇表單並按一下&#x200B;**[!UICONTROL 開啟]**。
1. 在編輯模式中，選取元件，然後按一下![欄位層級](assets/field-level.png) > **[!UICONTROL 適用性表單容器]**，然後按一下![cmpr](assets/cmppr.png)。

   您可以在側邊欄中編輯表單的屬性。

1. 在側欄中，按一下&#x200B;**[!UICONTROL 樣式]**。
1. 從&#x200B;**[!UICONTROL 適用性表單主題]**&#x200B;下拉式清單中選取主題，然後按一下&#x200B;**[!UICONTROL Done]** ![check-button](assets/check-button.png)。

要將主題應用於互動式通信，請執行以下操作：

1. 在編輯模式中開啟您的互動式通訊。 要在編輯模式下開啟互動式通信，請選擇一個表單並按一下&#x200B;**[!UICONTROL 開啟]**。
1. 在編輯模式中，選擇元件，然後按一下![欄位級](assets/field-level.png) >**[!UICONTROL 文檔容器]**，然後按一下![cmpr](assets/cmppr.png)。

   您可以在側邊欄中編輯表單的屬性。

1. 在側欄的&#x200B;**[!UICONTROL Basic]**&#x200B;下，從&#x200B;**[!UICONTROL Theme]**&#x200B;下拉式清單中選擇主題，然後按一下&#x200B;**[!UICONTROL Done]** ![check-button](assets/check-button.png)。
...
1. 在製作例項中，導覽至&#x200B;**[!UICONTROL Adobe Experience Manager > Forms >主題]**。
1. 開啟「主題庫」資料夾。
1. 在「主題庫」資料夾中，將指標暫留在對應的現成可用主題上，然後點選&#x200B;**[!UICONTROL Copy]**。
...
* **避免資產出自其他主題**
   ...
   編輯主題時，您可以瀏覽並新增其他主題的資產（例如影像）。 例如，您正在編輯頁面的背景。 例如，當您選擇&#x200B;**[!UICONTROL Page]** ![edit-button](assets/edit-button.png)> **[!UICONTROL Background > Add > Image]**&#x200B;時，您會看到一個對話框，該對話框允許您瀏覽和添加其他主題中的影像。
...
* **使用基本clientlib、主題編輯器和內嵌樣式**
   ...
   * **基clientlib**:
      ...
      1. 在「主題」頁中，選擇主題並按一下&#x200B;**[!UICONTROL 查看屬性]**。
      1. 在開啟的「屬性」頁面中，按一下&#x200B;**[!UICONTROL Advanced]**。
   ...
   * **內嵌樣式：**
      ...
      處理表單時，您可以使用表單中的樣式模式或互動式通訊多頻道編輯器來設定元件的樣式。 使用樣式模式更改表單元件樣式將覆蓋主題中指定的樣式。 如果要更改特定表單中某些元件的樣式，請參閱[元件的內嵌樣式](/help/forms/using/inline-style-adaptive-forms.md)。
