...
1. 按一 **[!UICONTROL 下「Adobe Experience Manager]**」、「 **[!UICONTROL Forms]**」，然後按一下「 **[!UICONTROL 主題]**」。

1. 在「主題」頁面中，按一下「 **[!UICONTROL 建立>主題」]**。

   會啟動建立主題的精靈。

1. 在「建立主題」精靈的「基本」索引標籤中， **[!UICONTROL 提供]****[!UICONTROL 主題的「標題]** 」和「名稱」。 這些是必填欄位。
...
1. 按一 **[!UICONTROL 下「建立]** 」，然後按一下「編 **[!UICONTROL 輯」以在「主題編輯器」中開啟主題，或按一]** 下「完成 **** 」以返回主題頁面。
...
1. 在「主題」頁面中，選取主題，然後按一下「下 **[!UICONTROL 載」]**。 此時將顯示一個包含主題詳細資訊的對話框。

1. 按一 **[!UICONTROL 下下載]**。 主題會下載為zip檔案。
...
1. 在「主題」頁面中，按一下「 **[!UICONTROL 建立>檔案上傳」]**。
1. 在「檔案上傳」提示中，瀏覽並選取電腦上的主題套件，然後按一下「上 **[!UICONTROL 傳]**」。
...
* **畫布** -顯示在右側。 它顯示樣本的自適應表單或互動式通訊，讓所有樣式變更都能立即反映。 您也可以直接從畫布選取物件，以尋找與其相關的樣式，並編輯這些樣式。 頂部的裝置解析度尺標控制畫布。 從尺標中選擇解析度斷點，可顯示各解析度的範例表單預覽或互動式通訊。 Canvas將在下面詳細討 [論](/help/forms/using/themes.md#using-canvas)。
...
* **側欄**-顯示在左側。 它包含下列項目：
   ...
   * **檢視CSS**: 可讓您檢視所選元件的CSS
   ...
   此外，在側欄中，底部有箭頭。 當您按一下箭頭時，您會再看到兩個選項： **模擬成功** , **模擬錯誤。** 這些選項以及上述選項將在下文中詳 [細討論](/help/forms/using/themes.md#using-rail)。
...
在資產庫中新增樣式時，其他主題和表格編輯器或互動式通訊編輯 [器](/help/forms/using/inline-style-adaptive-forms.md) UI的樣式模式都可使用。 同樣地，當您使用表單編輯器或互動式通訊編輯器UI的樣式模式來設定元件的樣式時，會快取樣式，並可在主題中使用。
...
1. 在「畫布」工具列上，按一 **[!UICONTROL 下「主題選項]**![」主題選項](assets/theme-options.png) >「管 **[!UICONTROL 理樣式」]**。
1. 在「管理樣式」對話方塊中，選取儲存的樣式，按一下「刪 **[!UICONTROL 除」]**。

   ![刪除保存的樣式](assets/manage-styles.png)

### 即時預覽、儲存和放棄變更 {#live-preview-save-and-discard-changes}

樣式中所做的修改會立即反映在畫布中載入的表單或互動式通訊中。 即時預覽可讓您以互動方式定義並檢視樣式的影響。 當您變更元件的樣式時，側欄中會啟 **[!UICONTROL 用「完成]** 」按鈕。 若要保留變更，請使用「完 **[!UICONTROL 成]** 」按鈕。
...
1. 在「主題編輯器」面板中，按一 **[!UICONTROL 下「主題選項]**![-](assets/theme-options.png) theme-options ****>設定」。

1. 在「一般」索引標籤中，瀏覽並選取「最適化表單／檔案」欄位 **[!UICONTROL 的表單或互動式通訊]** 。
...
主題編輯器可讓您編輯您建立或上傳的主題。 導覽至「 **[!UICONTROL 表單與檔案>主題]**」，然後選取並開啟主題。 主題在主題編輯器中開啟。
...
* **[!UICONTROL 切換側面板]** (Toggle Side Panel ![)](assets/toggle-side-panel.png)切換側面板(Toggle-side-panel): 可讓您顯示或隱藏邊欄。

* **[!UICONTROL 主題選項]**![主題選項](assets/theme-options.png): 提供3種選項
...
* **[!UICONTROL 模擬器]** 尺標 ![](assets/ruler.png): 模擬不同顯示大小的主題外觀。 顯示大小在模擬器中被視為斷點。 可以選擇斷點並為其指定樣式。 例如，Desktop和Tablet是兩個中斷點。 您可以為每個斷點指定不同的樣式。

當您在畫布中選取元件時，會在其上方看到元件工具列。 元件工具列可讓您選取元件，或切換至一般元件。 例如，您在面板中選擇一個數字文本框。 您可在元件工具列中看到下列選項：

* **[!UICONTROL 數值方塊介面工具集]**: 可讓您選取元件，以自訂其側邊欄外觀。
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td><p>尺寸及位置</p> </td> 
   <td><p>可讓您在主題中設定元件對齊方式、大小、定位和放置方式。 </p> <p>您的選項有顯示設定、間距、邊界、寬度、高度和Z索引。 </p> </td> 
  </tr> 
  <tr> 
   <td><p>文字</p> </td> 
...
1. 選擇 **[!UICONTROL 數值框構件]**。
1. 邊欄標題會變更為「數值方塊介面工具集」，並顯示自訂其外觀的選項。

   使用 **[!UICONTROL 側欄中的「維度與位置]** 」選項，自訂元件的大小。 請確定「狀態」為「 **[!UICONTROL 預設」]**。

與其選擇「 **[!UICONTROL 數值方塊介面工具集]**」，請在元件工具列 **[!UICONTROL 中選取「欄位介面工具集]** 」，然後執行上述步驟。 當您選取欄位介面工具集 **[!UICONTROL 選項的維度時]** ，除了數值方塊外，所有文字方塊的大小都相同。
...
1. 在側欄中選取狀態。 例如，錯誤狀態。
1. 使用側邊欄中 **[!UICONTROL 的「邊框」]** 、「背景」等選項來自訂元件的外觀。
1. 使用側 **[!UICONTROL 欄底部的「模擬錯誤]** 」選項，查看樣式在編輯時的外觀。

在指定元件狀態後自訂元件的樣式時，只會針對指定狀態顯示元件的自訂。 例如，如果您在選取暫留狀態時自訂元件的樣式。 當您將指標移至套用主題的轉譯表單或互動式通訊中的元件上時，就會顯示元件的自訂。

若要模擬除錯誤和成功以外的狀態行為，請使用「預覽」模式。 若要使用「預覽」模式，請按一 **[!UICONTROL 下頁面工具]** 列中的「預覽」。
...
1. 在作者例項中，按一 ![](assets/adobeexperiencemanager.png)**[!UICONTROL 下「Adobe Experience Manager > Tools ]**![hammer](assets/hammer.png)> Deployment > Cloud Services」****。
1. 在「 **[!UICONTROL Cloud Services]** 」頁面上，導覽至「協力 **[!UICONTROL 廠商服務]** > **[!UICONTROL Typekit」，然後按一下「Typekit下]****** 的「立即設定」。 如果配置已可用，請按一下 **+** 按鈕以建立新實例。
1. 在「創 **[!UICONTROL 建配置]** 」對話框中，指定配置的標題，然後按一下「 **[!UICONTROL 建立」]**。
...
1. 在主題編輯器中，導覽至「主 **[!UICONTROL 題選項]**![主題選項](assets/theme-options.png) >設 **[!UICONTROL 定]**」。
1. 在「 **[!UICONTROL Typekit設定]** 」欄位中，選取套件，然後按一下「 **[!UICONTROL 儲存」]**。
...
1. 在編輯模式下開啟表格。 要在編輯模式下開啟表單，請選擇一個表單，然後按一下「打 **[!UICONTROL 開」]**。
1. 在編輯模式中，選取元件，然後按一下欄 ![位層級](assets/field-level.png) > **[!UICONTROL 最適化表單容器]**，然後按 ![一下cmppr](assets/cmppr.png)。

   您可以在側欄中編輯表單的屬性。

1. 在側欄中，按一下「樣 **[!UICONTROL 式」]**。
1. 從「最適化表單主 **[!UICONTROL 題」下拉式清單中選取您的主題]** ，然後按一下「完成 **** 」核取按鈕 ![](assets/check-button.png)。

若要將主題套用至互動式通訊：

1. 以編輯模式開啟您的互動式通訊。 要在編輯模式下開啟互動式通信，請選擇一個表單並按一下「打 **[!UICONTROL 開」]**。
1. 在編輯模式中，選取元件，然後按一 ![下欄位層級](assets/field-level.png) >**[!UICONTROL 檔案容器]**，然後按 ![一下cmppr](assets/cmppr.png)。

   您可以在側欄中編輯表單的屬性。

1. 在側欄的「基本 **[!UICONTROL 」下方，從「]** Theme **[!UICONTROL 」下拉式清單中選取您的主題，然後按一下「]** Done **[!UICONTROL check-button]**![](assets/check-button.png)」。
...
1. 在製作例項中，導覽至「 **[!UICONTROL Adobe Experience Manager >表單>主題」]**。
1. 開啟「主題庫」檔案夾。
1. 在「主題庫」檔案夾中，將指標暫留在對應的現成可用主題上，然後點選「復 **[!UICONTROL 制」]**。
1. 將複製的主題貼在「主題庫」檔案夾外。
1. 自訂複製的主題。
...
* **避免其他主題中的資產**
   ...
   當您編輯主題時，可以瀏覽並新增其他主題的資產（例如影像）。 例如，您正在編輯頁面的背景。 例如，當您選取「頁 **[!UICONTROL 面]** 」 ![edit-button](assets/edit-button.png)>「背景」>「新增」>「影像 ****」時，您會看到一個對話方塊，可讓您瀏覽並新增其他主題中的影像。
...
* **使用基本clientlib、主題編輯器和內嵌樣式**
   ...
   * **基本clientlib**:
      ...
      1. 導覽至 **[!UICONTROL Experience Manager >表單>主題]**。
      1. 在「主題」頁面中，選擇一個主題，然後按一下「查 **[!UICONTROL 看屬性」]**。
      1. 在開啟的「屬性」頁面中，按一下「進 **[!UICONTROL 階」]**。
