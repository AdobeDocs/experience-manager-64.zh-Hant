...
1. 在主題頁中，按一下 **[!UICONTROL 建立>主題]**.

   啟動建立主題的精靈。

1. 在「建立主題」嚮導的「基本」頁簽中，提供 **[!UICONTROL 標題]** 和 **[!UICONTROL 名稱]** 主題。 這些是必填欄位。
...
1. 在主題頁中，選擇主題，然後按一下 **[!UICONTROL 下載]**. 此時將顯示一個對話框，其中包含主題的詳細資訊。
...
* **畫布**  — 顯示在右側。 它會顯示範例最適化表單或互動式通訊，讓所有樣式變更都能立即反映。 您也可以直接從畫布選取物件，以尋找與物件相關聯的樣式，並編輯這些樣式。 頂部的設備解析度標尺控制畫布。 從尺標中選擇解析度斷點顯示示例表單的預覽或相應解析度的交互通信。 畫布會詳加討論 [low](/help/forms/using/themes.md#using-canvas).
...
* **側欄** — 顯示在左側。 它有下列項目：
   ...
   * **檢視CSS**:可讓您檢視所選元件的CSS
   ...
   此外，在側欄底部會顯示箭頭。 按一下箭頭時，您還會看到兩個選項： **模擬成功** 和 **模擬錯誤。** 詳細討論這些選項以及上述選項 [low](/help/forms/using/themes.md#using-rail).
...
在資產庫中新增樣式時，可供其他主題和 [樣式模式](/help/forms/using/inline-style-adaptive-forms.md) 表單編輯器或互動式通訊編輯器UI。 同樣地，使用表單編輯器或互動式通訊編輯器UI的樣式模式來設定元件的樣式時，會快取樣式，並可在主題中使用。
...
在樣式中所做的修改會立即反映在畫布中載入的表單或互動式通訊中。 即時預覽可讓您以互動方式定義及查看樣式的影響。 變更元件的樣式時， **[!UICONTROL 完成]** 按鈕。 若要保留變更，請使用 **[!UICONTROL 完成]** 按鈕。
...
1. 在「常規」頁簽中，瀏覽並選擇 **[!UICONTROL 適用性表單/檔案]** 欄位。
...
主題編輯器可讓您編輯您建立或上傳的主題。 導覽至 **[!UICONTROL Forms與檔案>主題]**，然後選取主題並開啟它。 主題在主題編輯器中開啟。
...
您現在可以透過最適化表單或互動式通訊，使用網路服務中可用的字型。 現成可用， [Typekit](https://typekit.com/)，則Adobe的網頁字型服務可作為設定使用。 若要使用Typekit，請在其中建立套件和字型，然後從中取得套件ID [Typekit網站](https://typekit.com/).

執行下列步驟以在AEM中設定Typekit:

1. 在製作例項中，按一下 ![adobeexperiencemanager](assets/adobeexperiencemanager.png)**[!UICONTROL Adobe Experience Manager >工具&#x200B;]**![錘](assets/hammer.png)**[!UICONTROL >部署>Cloud Services ]**.
1. 在 **[!UICONTROL Cloud Services]** 頁面，導覽至 **[!UICONTROL 第三方服務]** >  **[!UICONTROL Typekit]** ，然後按一下 **[!UICONTROL 立即配置]** 在Typekit下。 如果配置已可用，請按一下 **+** 按鈕以建立新實例。
...
1. 在 **[!UICONTROL Typekit配置]** 欄位，選擇套件，然後按一下 **[!UICONTROL 儲存]**.
...
1. 在編輯模式中，選取元件，然後按一下 ![欄位層級](assets/field-level.png) > **[!UICONTROL 適用性表單容器]**，然後按一下 ![cppr](assets/cmppr.png).
...
1. 在編輯模式中開啟您的互動式通訊。 若要在編輯模式中開啟互動式通訊，請選取表單並按一下 **[!UICONTROL 開啟]**.
1. 在編輯模式中，選取元件，然後按一下 ![欄位層級](assets/field-level.png) >**[!UICONTROL 文檔容器]**，然後按一下 ![cppr](assets/cmppr.png).

   您可以在側邊欄中編輯表單的屬性。

1. 在側欄的下方 **[!UICONTROL 基本]**，請從 **[!UICONTROL 主題]** 下拉式清單，按一下 **[!UICONTROL 完成]** ![check-button](assets/check-button.png).
...
* **避免資產出自其他主題**
   ...
   編輯主題時，您可以瀏覽並新增其他主題的資產（例如影像）。 例如，您正在編輯頁面的背景。 例如，當您選取 **[!UICONTROL 頁面]** ![編輯按鈕](assets/edit-button.png)> **[!UICONTROL 背景>新增>影像]**，您會看到一個對話方塊，可讓您瀏覽和新增其他主題中的影像。
...
* **使用基本clientlib、主題編輯器和內嵌樣式**
   ...
   * **基clientlib**:
      ...
      1. 在主題頁中，選擇主題並按一下 **[!UICONTROL 檢視屬性]**.
   ...
   * **內嵌樣式：**
      ...
      處理表單時，您可以使用表單中的樣式模式或互動式通訊多頻道編輯器來設定元件的樣式。 使用樣式模式更改表單元件樣式將覆蓋主題中指定的樣式。 如果要更改特定表單的某些元件的樣式，請參閱 [元件的內嵌樣式](/help/forms/using/inline-style-adaptive-forms.md).
