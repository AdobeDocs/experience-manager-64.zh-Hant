...
The [HTML Templating Language (HTL)](https://helpx.adobe.com/experience-manager/htl/user-guide.html), introduced with AEM 6.0, takes the place of JSP (JavaServer Pages) as the preferred and recommended server-side template system for HTML. 對於需要建立強穩企業網站的網頁開發人員，HTL有助於提高安全性和開發效率。
...
切換 [至「預覽](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/api/WCMMode.html) 」模式時，即使未重新整理頁面，也會設定 **** WCM模式Cookie。

對於具有對WCM模式敏感的轉譯元件，需要定義這些元件，以明確重新整理它們，然後依賴Cookie的值。

>[!NOTE]
>
>在啟用觸控的UI中，只有值 `EDIT` 和 `PREVIEW` 用於 [WCM模式Cookie](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/api/WCMMode.html) 。
...
[Coral UI](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/coral-ui/coralui3/index.html) 和 [Granite UI](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/granite-ui/api/index.html) ，定義了AEM的現代外觀和感覺。
...
更具體地說，Granite UI提供一系列欄位元件，適合用於對話方塊(或更一般地，以 [表單](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/granite-ui/api/jcr_root/libs/granite/ui/components/foundation/form/index.html))。
...
要僅為元件對話框（即不為其他元件載入）載入客戶端庫，您需要將對話框的屬性設定為剛建立的客戶端庫的類別名稱。 `extraClientlibs` 如果您的用戶端資料庫相當大且／或您的欄位是該對話方塊專屬的，且在其他對話方塊中不需要，則建議您這麼做。
...
現在，處理對話欄位上事件的方法已在自訂用戶端程 [式庫中對聆聽器完成](#listeners-in-a-custom-client-library)。 這與舊式的內容結構中有監聽器 [的方法有所不同](#listeners-in-the-content-structure)。
...
* 在模板的 [段落系統上定義元件屬性](/help/sites-developing/components-basics.md#adding-your-component-to-the-paragraph-system)。

## 設定段落系統，以便拖曳資產可建立元件例項 {#configuring-a-paragraph-system-so-that-dragging-an-asset-creates-a-component-instance}

AEM提供在您的頁面上設定段落系統的可能性，讓當使用者將（適當）資產拖曳至該頁面的例項時 [](/help/sites-authoring/editing-content.md) （而不必總是拖曳空白元件至頁面），就會自動建立新元件的例項。
...
* 提供 [HTL](https://helpx.adobe.com/experience-manager/htl/user-guide.html) 支援，此範本語言旨在簡化元件開發並提高安全性。
...
* HTL
   ...
   * 使用 [HTL](https://helpx.adobe.com/experience-manager/htl/user-guide.html) 並非強制性，但如果您的元件需要更新，則是考慮從JSP移 [轉至HTL的理想時機](/help/sites-developing/components-basics.md#htl-vs-jsp)。
