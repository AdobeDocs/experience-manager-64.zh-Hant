...
>[!NOTE]
>
>從AEM 6.4.8.1開始，已無法再匿名存取CRXDE Lite。
>系統會將使用者重新導向至登入畫面。
...
1. 安裝AEM。
1. 在瀏覽器中輸入https://`<host>`:`<port>`/crx/de。 預設為 `http://localhost:4502/crx/de`.
...
只有節點類型才能使用此處描述的功能 `cq:Component` 可在存放庫中使用。
...
有了CRXDE Lite，您可以在 [CND（緊密命名空間和節點類型定義）記號](http://jackrabbit.apache.org/jcr/node-type-notation.html).
