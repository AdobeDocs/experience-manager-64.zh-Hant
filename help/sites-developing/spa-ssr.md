...
>[!NOTE]
>
>AEM 6.4.5.0或更新版本才能使用本文SPA件所述的伺服器端轉換功能。
...
如果您[確信您的項目需要實施SSR](#when-to-use-ssr)，則Adobe推薦的解決方案是使用Adobe I/O Runtime。
...
* [Adobe I/O-執行時期驅動的通訊流程](#adobe-io-driven-communication-flow)
...
必AEM須知道可在何處檢索遠程渲染的內容。 無論您選擇為SSR](#adobe-io-runtime)實作哪個型號，都需要指定如何AEM訪問此遠程渲染服務。[
...
>[!NOTE]
>
>無論您選擇實作[AEM-driven通訊流](#aem-driven-communication-flow)或[Adobe I/O Runtime-driven通訊流](#adobe-io-driven-communication-flow)，您都必須定義遠端內容轉譯器組態。
>
...
當使用SSR時，中的[元件交互工作流SPA程AEM包括由Adobe I/O Runtime生成應用程式初始內容的階段。](/help/sites-developing/spa-overview.md#workflow)

1. 瀏覽器向請求SSR內容AEM。
1. AEM把模型發給Adobe I/O Runtime。
1. Adobe I/O Runtime傳回產生的內容
...
### Adobe I/O Runtime驅動的通信流{#adobe-io-driven-communication-flow}

[AEM-Driven Communication Flow](#aem-driven-communication-flow)一節說明了針對執行內容引導和服務的伺服器端SPA渲染的AEM標AEM準和建議實現。
...
| 引導 | 優勢 | 缺點 |
|---|---|---|
| viaAEM | 在AEM需要時管理注入庫<br>僅需要在 | 開發人員可能不SPA熟悉 |
| Adobe I/O Runtime | 開發人員更熟SPA悉 | 開發人員必須透過[`allowProxy`屬性](/help/sites-developing/clientlibs.md#locating-a-client-library-folder-and-using-the-proxy-client-libraries-servlet)<br>讓應用程式所需的Clientlib資源AEM可供使用。資源必須在與AEMAdobe I/O Runtime之間同步。若要製作Clientlib資源，則可能需要為Adobe I/O RuntimeSPA提供Proxy伺服器<br> |

## SSR {#planning-for-ssr}規劃

通常只需要轉換應用程式的一部分， 常見的範例是，在頁面初始載入時將顯示在折疊上方的內容需要轉譯為伺服器端。 如此可將已轉譯的內容傳送至用戶端，以節省時間。 當使用者與互動時，SPA用戶端會轉譯其他內容。

當您考慮為您實作伺服器端演算時，SPA您需要檢閱應用程式中需要SSR的哪些部分。
