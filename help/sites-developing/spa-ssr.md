...
單頁應用程式(SPA)可為使用者提供多樣化的動態體驗，以熟悉的方式反應和運作，通常就像原生應用程式一樣。 [這是透過依賴用戶端在前面載入內容，然後進行繁重的使用者互動處理](/help/sites-developing/spa-walkthrough.md#how-does-a-spa-work) ，進而將用戶端與伺服器之間所需的通訊量減至最少，讓應用程式更具反應性。
...
如果您確 [定專案需要實作SSR](#when-to-use-ssr),Adobe建議的解決方案是使用Adobe I/O Runtime。
...
AEM必須知道可擷取遠端轉譯內容的位置。 不論您 [選擇為SSR實作何種模型](#adobe-io-runtime)，您都需要指定AEM如何存取此遠端轉譯服務。

這是通過RemoteContentRenderer - **Configuration Factory** OSGi服務完成的。 在「Web控制台配置」控制台中搜索「RemoteContentRenderer」字串，網址為 `http://<host>:<port>/system/console/configMgr`。
...
>[!NOTE]
>
>不論您選擇實作 [AEM導向的通訊流程](#aem-driven-communication-flow) ，或 [](#adobe-io-driven-communication-flow)Adobe I/O Runtime導向的流程，您都必須定義遠端內容轉譯器設定。
>
>如果您選擇使用自訂Node.js伺服 [器，也必須定義此設定](#using-node-js)。

>[!NOTE]
>
>此配置利用「遠 [程內容渲染器」](#remote-content-renderer)，該轉換器具有其他擴展和定制選項。

## AEM導向的通訊流程 {#aem-driven-communication-flow}

使用SSR時，AEM中 [SPA的元件互動工作流程](/help/sites-developing/spa-overview.md#workflow) ，包含由Adobe I/O Runtime產生應用程式初始內容的階段。
...
### Adobe I/O執行時期導向的通訊流程 {#adobe-io-driven-communication-flow}

「 [AEM-Driven Communication Flow](#aem-driven-communication-flow) 」一節說明針對AEM中的SPA,AEM會執行內容的引導載入和伺服的伺服器端轉譯標準與建議實作。
...
| 引導 | 優勢 | 缺點 |
|---|---|---|
| 透過AEM | AEM管理在需要時<br>只需在AEM上維護資源的注入程式庫 | SPA開發人員可能不熟悉 |
| 透過Adobe I/O Runtime | SPA開發人員更熟悉 | AEM開發人員必須透過 [`allowProxy` propertyResources在AEM和Adobe I/O](/help/sites-developing/clientlibs.md#locating-a-client-library-folder-and-using-the-proxy-client-libraries-servlet)<br><br>Runtime之間同步，才能使用應用程式（例如CSS和JavaScript）所需的Clientlib資源。若要啟用SPA的編寫功能，可能需要Adobe I/O Runtime的代理伺服器 |
