...
範本可用來建立類型的頁 `cq:Page` 面（如前所述，頁面是特殊類型的元件）。 每個AEM頁面都有結構化節點 `jcr:content`。 此特性：
...
若要為您的網站啟用現有範本，並希望在從 **Websites** ******** Console的「網站」下建立頁面時，將其顯示在「建立頁面」對話方塊中，請將範本節點的allowedPaths屬性設為： **/content(/)。&amp;ast;)?**
...
1. 將現有範本（最好以您想要達到的定義盡可能接近）複製到新節點。
   ...
   >[!NOTE]
   >
   >可用的範本清單取決於新頁面的位置以及每個範本中指定的位置限制。 請參閱 [範本可用性](/help/sites-developing/templates.md#template-availability)。
...
1. 變更用 `allowedPaths` 於範本可用性的屬性 [和其他屬性](/help/sites-developing/templates.md#template-availability)。 例如， `allowedPaths`: `/content/geometrixx-outdoors/[^/]+(/.*)?` 表示此範本可在下列任何路徑中使用 `/content/geometrixx-outdoors`。
