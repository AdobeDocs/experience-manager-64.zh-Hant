...
* [com.day.cq.wcm.mobile.api.device](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/mobile/api/device/package-summary.html)  — 定義裝置、DeviceGroup和DeviceGroupList。
...
* [com.day.cq.wcm.mobile.api.wurfl](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/workflow/api/package-summary.html)  — 定義WurflQueryEngine。

* [com.day.cq.wcm.mobile.core](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/mobile/core/package-summary.html)  — 定義MobileUtil，其提供可繞WCM Mobile運作的各種公用程式方法。
...
行動裝置群組會根據裝置功能提供行動裝置的分段。 裝置群組提供在製作執行個體上以模擬器為基礎製作以及在發佈執行個體上正確呈現內容所需的資訊：一旦作者將內容新增至行動頁面並發佈後，即可在發佈例項上請求頁面。 在那裡，內容頁面不是模擬器編輯視圖，而是使用其中一個配置的設備組呈現。 根據[移動設備檢測](#client-side-device-detection)進行設備組的選擇。 然後，相符的裝置群組會提供必要的樣式資訊。
...
AEM使用截斷版本的[WURFL](http://wurfl.sourceforge.net/index.php)™資料庫，根據裝置的使用者代理查詢裝置功能，例如螢幕解析度或javascript支援。
...
1. 將下列屬性新增至節點：
   ...
   * **regexp**:定義使用者代理的規則運算式，例如：.&amp;ast;Mozilla。&amp;ast;iPad。&amp;ast;AppleWebKit。&amp;ast;Safari。&amp;ast;
...
1. [設備組特定URL](#defining-a-device-group-specific-url)
...
>[!NOTE]
>
>有關更多詳細資訊，請閱讀[自定義BrowserMap](#using-browsermap-on-your-pages)部分。
...
1. iPad會傳送要求至AEM發佈例項，例如`http://localhost:4503/content/geometrixx_mobile/en/products.html`
...
1. AEM會將請求重新導向至`http://localhost:4503/content/geometrixx_mobile/en/products.touch.html.`
1. AEM會將回應傳送至iPad:
