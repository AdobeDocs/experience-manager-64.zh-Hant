...
行動裝置群組根據裝置功能提供行動裝置的區段。 裝置群組提供在作者執行個體上進行模擬器製作以及在發佈執行個體上進行正確內容轉換所需的資訊：一旦作者將內容新增至行動頁面並發佈後，就可以在發佈例項上要求頁面。 內容頁面會使用其中一個已設定的裝置群組呈現，而非模擬器編輯檢視。 根據[移動設備檢測](#client-side-device-detection)來選擇設備組。 然後，比對的裝置群組會提供必要的樣式資訊。
...
>[!NOTE]
>
>如需詳細資訊，請閱讀[自訂BrowserMap](#using-browsermap-on-your-pages)一節。
