...
**快取、煎炸和烘焙** -傳統上，烘焙與煎炸的概念是不同Web內容管理系統之間的重要區別。 在CMS術語中，「烘焙」是指在發佈時將資料提交至靜態檔案的概念，而「煎炸」則指在要求時處理資料以進行最終呈現的概念（即即及時）。
...
**Component** —— 在AEM中，Component是物件類型，通常可以從Sidekick等拖放來建立其例項。 例如，隨附AEM的現成可用元件包括「文字」、「標題」、「標籤雲端」、「轉盤」、「影像」和「清單」元件，這些元件都可從Sidekickat執行時期取得。
