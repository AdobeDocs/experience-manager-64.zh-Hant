...
依預設， `cq:ClientLibraryFolder` 節點可以放置在 `/apps`, `/libs` 和 `/etc` 存放庫的子樹(這些預設值和其他設定可透過 **AdobeGraniteHTML程式庫管理員** 面板 [系統控制台](http://localhost:4502/system/console/configMgr))。
...
>[!CAUTION]
>
>`<cq:includeClientLib>`，過去通常用於包含用戶端程式庫，自AEM 5.6起即已過時。 [ `<ui:includeClientLib>`](/help/sites-developing/taglib.md#amp-lt-ui-includeclientlib) 應如上所述使用。
...
>[!NOTE]
>
>客戶端庫資料夾下的靜態資源必須位於名為的資料夾中 *資源*. 如果資料夾下沒有靜態資源，例如影像 *資源*，則無法在發佈例項上參照。 以下是範例：http://localhost:4503/etc.clientlibs/geometrixx/components/clientlibs/resources/example.gif
...
1. 前往Apache Felix Config Manager，網址為 [http://localhost:4502/system/console/configMgr](http://localhost:4502/system/console/configMgr)
...
如需設定服務的詳細資訊，請參閱 [AEMHTML程式庫管理員](/help/sites-deploying/osgi-configuration-settings.md).
