...
預設情況下，`cq:ClientLibraryFolder`節點可以放置在儲存庫的`/apps`、`/libs`和`/etc`子樹中的任意位置(這些預設值以及其他設定可通過[系統控制台](http://localhost:4502/system/console/configMgr)的&#x200B;**AdobeGranite HTML庫管理器**&#x200B;面板進行控制)。
...
如需完整資訊，包括篩選JS、CSS或主題程式庫的屬性，請參閱[ui:includeClientLib](/help/sites-developing/taglib.md#amp-lt-ui-includeclientlib)。

>[!CAUTION]
>
>`<cq:includeClientLib>`，過去通常用於包含用戶端程式庫，自AEM 5.6起即已棄用。 [ `<ui:includeClientLib>`](/help/sites-developing/taglib.md#amp-lt-ui-includeclientlib) 應如上所述使用。
...
>[!NOTE]
>
>客戶端庫資料夾下的靜態資源必須位於名為&#x200B;*resources*&#x200B;的資料夾中。 如果您在資料夾&#x200B;*resources*&#x200B;下沒有靜態資源（例如影像），則無法在發佈執行個體上參照該資源。 以下是範例：http://localhost:4503/etc.clientlibs/geometrixx/components/clientlibs/resources/example.gif
...
1. 前往[http://localhost:4502/system/console/configMgr](http://localhost:4502/system/console/configMgr)前往Apache Felix Config Manager
...
有關配置服務的資訊，請參閱[AEM HTML Library Manager](/help/sites-deploying/osgi-configuration-settings.md)。
