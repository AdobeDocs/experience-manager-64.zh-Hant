...
* **觸控式UI**
   [AEM 5.](/help/sites-developing/touch-ui-concepts.md) 6.0導入的標準使用者介面，作為預覽並在6.x中延伸。這是以Adobe Marketing Cloud的統一使用者體驗為基礎，使用Coral UI和 [Granite](/help/sites-developing/touch-ui-concepts.md#coral-ui)  UI [的基礎技術](/help/sites-developing/touch-ui-concepts.md#granite-ui)。

* **基**
於CQ 5.1中引入的ExtJS技術的傳統UIUser介面。
...
[HTL](https://helpx.adobe.com/experience-manager/htl/user-guide.html)支援此理念，該模板語言故意限制以確保使用真正的寫程式語言定義基礎業務邏輯。 系統會透過特定命令從HTL叫用此（選用）邏輯。 此機制會反白標示為指定檢視所呼叫的程式碼，並視需要為相同元件的不同檢視允許特定邏輯。

### HTL與JSP {#htl-vs-jsp}

HTL是AEM 6.0推出的HTML範本語言。

在開發您自己的元件時，有關是使用[ HTL](https://helpx.adobe.com/experience-manager/htl/user-guide.html)還是JSP（Java伺服器頁面）的討論應該簡單明瞭，因為HTL現在是AEM的建議指令碼語言。
...
1. `cq:icon`  — 指向Coral UI資料庫中標準圖示的字 [串屬](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/coral-ui/coralui3/Coral.Icon.html) 性，以在元件瀏覽器中顯示
