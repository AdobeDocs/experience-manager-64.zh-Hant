...
所有這些都是以一系列預設值來實現的 [表單元件](/help/sites-authoring/default-components.md)，可在標準AEM安裝中使用。
...
1. 在資料夾中建立以下任一項：
   ...
   1. 貼文指令碼。
      ...
      提交表單以處理表單時，會叫用貼文指令碼，其中包含處理從表單送達之資料的程式碼 `POST`.
...
1. 在資料夾中再次選擇新增：
   ...
   1. 用於添加欄位的指令碼。
      ...
      在寫入表單開始的HTML後，會立即叫用addfields指令碼。 這可讓動作在表單內新增自訂輸入欄位或其他這類HTML。
...
如需撰寫指令碼時可使用的API元素詳細資訊，請參閱 [與表單相關的javadoc](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/foundation/forms/package-summary.html).
