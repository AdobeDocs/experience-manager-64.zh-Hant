...
所有這些都是透過一系列預設的 [Form元件來實現](/help/sites-authoring/default-components.md)，這些元件可在標準AEM安裝中取得。
...
1. 在資料夾中建立以下任一項：
   ...
   1. 貼文指令碼。
      ...
      指令碼的名稱 `post.POST.<extension>`為，例如 `post.POST.jsp`

      當提交表單以處理表單時，會叫用貼文指令碼，其中包含處理從表單傳來之資料的程式碼 `POST`。
...
1. 在資料夾中再次選擇新增：
   ...
   1. 用於添加欄位的指令碼。
      ...
      指令碼的名稱 `addfields.<extension>`為，例如 `addfields.jsp`

      在寫入表單開始的HTML後，會立即調用addfields指令碼。 這可讓動作在表單中新增自訂輸入欄位或其他HTML。

   1. 初始化指令碼。

      指令碼的名稱 `init.<extension>`為，例如 `init.jsp`

      在轉譯表單時會叫用此指令碼。 它可用於初始化操作細節。 ``

   1. 清除指令碼。

      指令碼的名稱 `cleanup.<extension>`為，例如 `cleanup.jsp`
...
1. 在此資料夾中，您可以需要下列指令碼：
   ...
   * 客戶端驗證指令碼：
      ...
      指令碼的名稱 `clientvalidation.<extension>`為，例如 `clientvalidation.jsp`

      在轉譯表單欄位時會叫用此功能。 它可用於建立用戶端javascript，以驗證用戶端上的欄位。

   * 伺服器驗證指令碼：

      指令碼的名稱 `servervalidation.<extension>`為，例如 `servervalidation.jsp`
...
有關在編寫指令碼時可使用的API元素的詳細資訊，請參閱與 [表單相關的javadoc](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/wcm/foundation/forms/package-summary.html)。
