...
>[!NOTE]
>
>用戶端內容已由觸控UI中的ContextHub取代。 如需詳細資訊，請參閱[相關檔案](/help/sites-developing/contexthub.md)。
...
* 用戶端網頁瀏覽器。
* 伺服器（請參閱[JSONP Store](/help/sites-administering/client-context.md)以儲存來自第三方來源的資訊）
...
獲取會話儲存對象以讀取或寫入資料到儲存。 [`CQ_Analytics.ClientContextMgr`](/help/sites-developing/ccjsapi.md#cq-analytics-clientcontextmgr) 根據商店名稱提供對商店的存取。取得後，使用[`CQ-Analytics.SessionStore`](/help/sites-developing/ccjsapi.md#cq-analytics-sessionstore)或[`CQ-Analytics.PersistedSessionStore`](/help/sites-developing/ccjsapi.md#cq-analytics-persistedsessionstore)的方法與儲存資料互動。
...
會話儲存器是基於`Observable`模式構建的。 它們會延伸[`CQ_Analytics.Observable`](/help/sites-developing/ccjsapi.md#cq-analytics-observable)，提供[`addListener`](/help/sites-developing/ccjsapi.md#addlistener-event-fct-scope)方法。
...
* 延伸一般儲存屬性元件。
* 使用`CQ_Analytics.JSONPStore` javascript物件初始化商店。
...
1. 在您的網頁瀏覽器中開啟CRXDE Lite([http://localhost:4502/crx/de](http://localhost:4502/crx/de))。
...
1. 按一下右鍵`/apps/myapp/contextstores/geoloc`節點，然後按一下&#x200B;**建立->建立檔案**。 指定init.js.jsp的名稱，然後按一下「確定」。
...
1. 開啟作者例項([http://localhost:4502/content/geometrixx-outdoors/en.html](http://localhost:4502/content/geometrixx-outdoors/en.html))上的Geometrixx Outdoors首頁。
