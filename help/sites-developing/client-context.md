...
>[!NOTE]
>
>用戶端內容已由觸控式UI中的ContextHub取代。 如需詳細資訊，請參閱[相關檔案](/help/sites-developing/contexthub.md)。
...
* 客戶端Web瀏覽器。
* 伺服器（請參閱[JSONP儲存](/help/sites-administering/client-context.md)以儲存來自第三方源的資訊）
...
* 保存的對象：[`CQ_Analytics.PersistedJSONStore`](/help/sites-developing/ccjsapi.md#cq-analytics-persistedjsonstore)和[`CQ-Analytics.PersistedJSONPStore`](/help/sites-developing/ccjsapi.md#cq-analyics-persistedjsonpstore)。
...
獲取會話儲存對象以讀取或寫入儲存資料。 [`CQ_Analytics.ClientContextMgr`](/help/sites-developing/ccjsapi.md#cq-analytics-clientcontextmgr) 根據商店名稱提供對商店的訪問。取得後，請使用[`CQ-Analytics.SessionStore`](/help/sites-developing/ccjsapi.md#cq-analytics-sessionstore)或[`CQ-Analytics.PersistedSessionStore`](/help/sites-developing/ccjsapi.md#cq-analytics-persistedsessionstore)方法與儲存資料互動。
...
會話儲存區是以`Observable`模式建置。 它們擴展[`CQ_Analytics.Observable`](/help/sites-developing/ccjsapi.md#cq-analytics-observable)，提供[`addListener`](/help/sites-developing/ccjsapi.md#addlistener-event-fct-scope)方法。
...
* 使用`CQ_Analytics.JSONPStore` javascript對象初始化儲存。
...
1. 按一下右鍵`/apps/myapp/contextstores/geoloc`節點，然後按一下&#x200B;**Create -> Create File**。 指定init.js.jsp的名稱，然後按一下確定。
...
1. 開啟製作例項上的Geometrixx Outdoors首頁([http://localhost:4502/content/geometrixx-outdoors/en.html](http://localhost:4502/content/geometrixx-outdoors/en.html))。
