...
>[!NOTE]
>
>用戶端內容已由觸控UI中的ContextHub取代。 如需詳細資訊， [請參閱相關](/help/sites-developing/contexthub.md) 檔案。
...
* 用戶端網頁瀏覽器。
* 伺服器(請參 [閱JSONP商店](/help/sites-administering/client-context.md) ，以儲存來自第三方來源的資訊)
...
將「用戶端內容」元件加入網頁的內文區段，以啟用「用戶端內容」。 客戶端上下文元件節點的路徑為 `/libs/cq/personalization/components/clientcontext`。 若要包含此元件，請將下列程式碼新增至頁面元件的JSP檔案，該檔案位於頁面元 `body` 素的正下方：
...
* 持久對象： [`CQ_Analytics.PersistedJSONStore`](/help/sites-developing/ccjsapi.md#cq-analytics-persistedjsonstore) 和 [`CQ-Analytics.PersistedJSONPStore`](/help/sites-developing/ccjsapi.md#cq-analyics-persistedjsonpstore)。
...
使用javascript與工作階段儲存區互動。

### 訪問會話儲存 {#accessing-session-stores}

獲取會話儲存對象以讀取或寫入資料到儲存。 [`CQ_Analytics.ClientContextMgr`](/help/sites-developing/ccjsapi.md#cq-analytics-clientcontextmgr) 根據商店名稱提供對商店的存取。 取得後，請使用或的方 [`CQ-Analytics.SessionStore`](/help/sites-developing/ccjsapi.md#cq-analytics-sessionstore) 法與 [`CQ-Analytics.PersistedSessionStore`](/help/sites-developing/ccjsapi.md#cq-analytics-persistedsessionstore) 儲存資料互動。
...
會話儲存器是基於模式 `Observable` 構建的。 它們可 [`CQ_Analytics.Observable`](/help/sites-developing/ccjsapi.md#cq-analytics-observable) 以擴展，提供 [`addListener`](/help/sites-developing/ccjsapi.md#addlistener-event-fct-scope) 方法。
...
只有當 [`CQ_Analytics.ClientContextUtils`](/help/sites-developing/ccjsapi.md#cq-analytics-clientcontextutils) 對象和方 [`onStoreRegistered`](/help/sites-developing/ccjsapi.md#onstoreregistered-storename-callback) 法可 [`onStoreInitialized`](/help/sites-developing/ccjsapi.md#onstoreinitialized-storename-callback-delay) 用時，才能訪問會話儲存。 這些方法可讓您註冊回應作業註冊和初始化事件的事件接聽程式。
...
* 延伸一般儲存屬性元件。
* 使用Javascript物件初始化 `CQ_Analytics.JSONPStore` 商店。
...
1. 在您的網頁瀏覽器(http://localhost:4502/crx/de)中開啟CRXDE[Lite](http://localhost:4502/crx/de)。
...
1. 按一下右鍵該節 `/apps/myapp/contextstores/geoloc` 點，然後按一下 **建立->建立檔案**。 指定init.js.jsp的名稱，然後按一下「確定」。
...
1. 在作者例項(http://localhost:4502/content/geometrixx-outdoors/en.html)上開啟Geometrixx Outdoors首[頁](http://localhost:4502/content/geometrixx-outdoors/en.html)。
