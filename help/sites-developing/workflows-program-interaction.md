...
description: 瞭解如何使用API和指令碼來管理工作流程並以程式設計方式與工作流程互動。
contentOwner: User
...
工作流Java API由[`com.adobe.granite.workflow`](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/adobe/granite/workflow/package-summary.html)包和多個子包組成。 API中最重要的成員是`com.adobe.granite.workflow.WorkflowSession`類。 `WorkflowSession`類別提供對設計時和運行時工作流對象的訪問：
...
| 處於工作流（或不處於）中的節點的資訊 | [`WorkflowStatus`](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/adobe/granite/workflow/status/WorkflowStatus.html) |
...
您可以使用[WorkflowStatus API](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/adobe/granite/workflow/status/WorkflowStatus.html)來檢索有關節點工作流狀態的資訊。
...
使用OSGi事件框架監聽[ `com.adobe.granite.workflow.event.WorkflowEvent`](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/adobe/granite/workflow/event/WorkflowEvent.html)類定義的事件。 此類別也提供數種有用的方法，以取得事件主題的相關資訊。 例如，`getWorkItem`方法返回事件中涉及的工作項目的`WorkItem`對象。
