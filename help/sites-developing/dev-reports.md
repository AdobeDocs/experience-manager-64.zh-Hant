...
>  * `N:<name> [<nodeType>]`
   ...
   > 
   >     說明名稱為 `<*name*>` 和節點類型 `<*nodeType*>`*.*
>
>  * `P:<name> [<propertyType]`
   >
   >     說明名稱為 `<*name*>` 和屬性類型 `<*propertyType*>`.
>
>  * `P:<name> = <value>`
   >
   >     說明屬性 `<name>` 的值 `<value>`.
...
* 是根據 [CQ QueryBuilder](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/javadoc/com/day/cq/search/QueryBuilder.html).
...
此對話方塊是標準CQ [對話](/help/sites-developing/components-basics.md#dialogs) 並可依此設定(請參閱 [CQ.Dialog](https://helpx.adobe.com/experience-manager/6-4/sites/developing/using/reference-materials/widgets-api/index.html?class=CQ.Dialog) 以取得詳細資訊)。
