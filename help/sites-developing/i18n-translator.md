...
1. 要編輯其中一個字串的翻譯文本，您可以執行以下操作：
   ...
   * 連按兩下必要字 **串的字串** 或注釋欄位，以開啟 **Edit string** （編輯字串）對話方塊、視需要編輯轉譯，然後按一下 ******** OKJozing以關閉對話方塊：
...
1. 語言清單儲存在以下位置：
   ...
   然後變更或擴充清單。 語言節 `defaultCountry` 點上的屬性(例如 `ja`)必須包含完整的程式碼，例如 `ja_jp`，此程式碼會定 `jp` 義為語言的預設國家／地區 `ja`。
