...
上述在例如「部落格貼文」節點上使用 `lastModified` Date屬性的範例，並不表示需要特殊nodetype。 至少一開始，我 `nt:unstructured` 一定會用於我的部落格貼文節點。 因為在我的部落格應用程式中，我只要顯示最後一個修改日期（可能是「依順序」），我幾乎不在乎它是否為日期。 由於我隱含地相信我的部落格撰寫應用程式會將「日期」放在那裡，所以您不需要以nodetype的形式宣 `lastModified` 告日期的存在。
...
根據我的經驗，如果想到存取控制和限制通常是內容階層的好驅動因素。 將它想成是您的檔案系統。 甚至可以使用檔案和檔案夾，在本機磁碟上建立模型。
