...
>[!NOTE]
>
>僅支援產品附帶的`at.js`版本。 查看位於`/etc/clientlibs/foundation/testandtarget/atjs/source/at.js`的`at.js`檔案，即可獲得產品附帶的`at.js`版本。
...
JSP會新增所需的分析javascript物件和用戶端javascript程式庫的參考。 `testandtarget.js`檔案包含`mbox.js`函式。 指令碼生成的HTML與以下示例類似：
...
緊接在`<body>`標籤後面新增下列程式碼，以新增用戶端內容功能至頁面：
...
用於建立mbox的預設mbox.js檔案位於`/etc/clientlibs/foundation/testandtarget/mbox/source/mbox.js`。 若要使用客戶mbox.js檔案，請將檔案新增至Target雲端設定。 要添加檔案，檔案系統上必須提供`mbox.js`檔案。
...
1. 前往「CQ **工具**」頁面，然後選擇「**雲端服務**」。 ([http://localhost:4502/libs/cq/core/content/tools/cloudservices.html](http://localhost:4502/libs/cq/core/content/tools/cloudservices.html))
...
您的Target設定包含自訂mbox.js檔案，頁面標題區段[中所需的程式碼會將檔案新增至用戶端程式庫架構，而非對testandtarget.js程式庫的參考。](/help/sites-developing/target.md#the-head-section)
...
例如，若要停用Geometrixx Demo Site頁面標題元件的定位，請將屬性新增至`/apps/geometrixx/components/title/cq:editConfig`節點。
