...
* [使用內容片段建](/help/assets/content-fragments.md) 立和管 [理內容片段和](/help/sites-authoring/content-fragments.md) 頁面編寫，以取得內容片段的詳細資訊。

* [管理資](/help/assets/managing-assets-touch-ui.md) 產和自 [訂及擴充資](/help/assets/extending-assets.md) 產，以取得標準資產的詳細資訊。
...
* 中繼資料和相關內容會儲存在`jcr:content/metadata`下方
   ...
   除標題和說明外，標題和說明不被視為傳統中繼資料並儲存在`jcr:content`
...
此參數可在[Web控制台](/help/sites-deploying/configuring-osgi.md#osgi-configuration-with-the-web-console)中配置，用於OSGi捆綁&#x200B;**DAM內容片段配置**。

* **資源類型**

   可提供`sling:resourceTypes`的清單，以定義用於呈現內容片段的元件，以及應套用背景處理的元件。

* **參考屬性**

   屬性清單可被配置以指定對所述片段的引用被儲存於所述各個元件的位置。
