...
* 中繼資料和相關內容儲存在下方 `jcr:content/metadata`
   ...
   除標題和說明外，標題和說明不被視為傳統中繼資料，並儲存在 `jcr:content`
...
此參數可在 [Web Console中設定](/help/sites-deploying/configuring-osgi.md#osgi-configuration-with-the-web-console)，以用於OSGi搭售 **DAM內容片段設定**。

* **資源類型**

   可提供一 `sling:resourceTypes` 份清單，以定義用於呈現內容片段的元件，以及應套用背景處理的元件。
