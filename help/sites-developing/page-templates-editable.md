...
1. 配置新模板的結構、內容策略、初始內容和佈局。
   ...
   * 這些規則適用於範本（以及使用範本建立的頁面）。
   ...
   如需原則的技術詳細資訊，請參閱 [內容原則](/help/sites-developing/page-templates-editable.md#content-policies) 在此文檔中。
...
* 範本類型可讓您定義：
   ...
   * 建議為回應式格線定義斷點，並在範本類型上設定行動模擬器。 這是選用的，因為也可在個別範本上定義設定(請參閱 [範本類型和行動裝置群組](/help/sites-developing/page-templates-editable.md#template-type-and-mobile-device-groups))。
...
1. **啟用範本**
   ...
   * [啟用範本](/help/sites-authoring/templates.md#enabling-and-allowing-a-template-template-author) 從 **範本** 控制台。
   * 在 `jcr:content` 節點。
...
* Model - OSGi套件組合
   ...
   此 [OSGI套件組合](/help/sites-deploying/osgi-configuration-settings.md) 實作功能。
