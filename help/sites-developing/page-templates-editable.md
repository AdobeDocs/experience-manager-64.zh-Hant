...
* 範本類型可讓您定義：
   ...
   * 建議您在範本類型上定義回應式格點和行動模擬器設定的中斷點。 這是可選的，因為配置也可以在單個模板上定義（請參閱[模板類型和移動設備組](/help/sites-developing/page-templates-editable.md#template-type-and-mobile-device-groups)）。
...
1. **啟用範本**
   ...
   * [從範本](/help/sites-authoring/templates.md#enabling-and-allowing-a-template-template-author) 主控台啟 **** 用範本。
