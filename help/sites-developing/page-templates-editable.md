...
>[!NOTE]
>
>AEM 6.4.5.0或更新版本是必要項目，才能搭配 [SPA編輯器使用可編輯的範本](/help/sites-developing/spa-overview.md)。
...
* 範本類型可讓您定義：
   ...
   * 建議您在範本類型上定義回應式格點和行動模擬器設定的中斷點。 這是可選的，因為此設定也可以在個別範本上定義(請參閱范 [本類型和行動裝置群組](/help/sites-developing/page-templates-editable.md#template-type-and-mobile-device-groups))。
...
1. **啟用範本**
   ...
   * [從範本主控台](/help/sites-authoring/templates.md#enabling-and-allowing-a-template-template-author) 啟用 **範本** 。
...
* 設定 - `/conf/<my-folder>/settings/wcm/templates/<my-template>`
   ...
   範本 [和相關內容原則](#template-definitions) ，定義頁面設定。

* Model - OSGi bundles

   OSGI搭 [售組合](/help/sites-deploying/osgi-configuration-settings.md) ，實作功能。

* 檢視 - `/apps/<my-site>/components`

   在作者和發佈環境中，內容都由元件呈 [現](/help/sites-developing/components.md)。
