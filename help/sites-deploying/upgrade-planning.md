...
您可在adobe.com](/help/release-notes/release-notes.md)的[AEM區段中找到AEM 6.4的新功能。 請務必留意貴組織中常用的UI或產品功能變更。 當您查看新功能時，也請留意任何對您的組織有價值的功能。 查看AEM 6.4中的變更後，為作者制定訓練計畫。 這可能包括利用免費可用的資源，例如通過[Adobe數字學習服務](https://www.adobe.com/training.html)提供的幫助功能視頻或正式培訓。
...
AEM 6.4已對存放庫結構進行變更，有助於更順暢地升級。 這些更改涉及根據Adobe或客戶是否擁有該內容，將內容從/etc資料夾移出到包括/libs、/apps和/content的資料夾，從而限制在發行期間覆蓋內容的可能性。 在6.4升級時，存放庫重新調整的方式不應要求變更程式碼，不過建議在規劃升級時於AEM 6.4的[存放庫重新調整中查看詳細資訊。](/help/sites-deploying/repository-restructuring.md)
...
在初次評估後，更全面的下一步可能是對測試實例執行升級並執行一些基本的煙霧測試。 Adobe也提供一些。 此外，應不僅查看您要升級到的版本，而且查看源版本和目標版本之間的任何版本的[已棄用和已移除功能](/help/release-notes/deprecated-removed-features.md)清單。 例如，如果從AEM 6.2升級至6.4，除了AEM 6.4的功能外，請務必檢閱AEM 6.3已棄用和已移除的功能。

![screen_shot_2018-04-04at120912](assets/screen_shot_2018-04-04at120912.png)

6.4版中推出的模式偵測器，應能相當精確地預估大部分情況下在升級期間的預期結果。 不過，對於更複雜的自訂和部署，若您有不相容的變更，您可以根據[執行就地升級](/help/sites-deploying/in-place-upgrade.md)中的指示，將開發執行個體升級至AEM 6.4。 完成後，對此環境執行一些高級煙霧測試。 本練習的目的不是詳盡地完成測試用例清單並生成缺陷的正式清單，而是讓我們粗略地估計要升級代碼以獲得6.4相容性所需的工作量。 當與[模式檢測](/help/sites-deploying/pattern-detector.md)以及前一節中確定的體系結構更改相結合時，可以向項目管理團隊提供粗略估計，以規劃升級。
...
我們提供了與AEM 6.4相容的[升級代碼和自定義](/help/sites-deploying/upgrading-code-and-customizations.md)過程。當執行此迭代過程時，應根據需要對Runbook進行更改。 另請參閱AEM 6.4](/help/sites-deploying/backward-compatibility.md)中的[向後相容性，以了解在大多數情況下，如何讓自訂內容保持向後相容，而不需要在升級後立即開發。
