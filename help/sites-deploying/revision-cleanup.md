...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>升級至AEM 6.4時，應注意什麼？</td> 
   <td><p>AEM 6.4會變更TarMK的持續性格式。這些變更不需要主動移轉步驟。 現有存放庫會執行滾動式移轉，對使用者而言是透明的。 AEM 6.4（或相關工具）首次存取存放庫時，就會啟動移轉程式。</p> <p><strong>一旦開始移轉至AEM 6.4永續性格式，存放庫就無法還原回先前的AEM 6.3永續性格式。</strong></p> </td> 
  </tr> 
 </tbody> 
</table>
...
|  | TarMK GC #2:5次循環後32.902分鐘(1974140毫秒)壓實失敗 | 此訊息不表示有無法復原的錯誤，但只有壓縮在經過特定嘗試後才會終止。 此外，請閱讀 [後段](https://jackrabbit.apache.org/oak/docs/nodestore/segment/overview.html#how-does-compaction-works-with-concurrent-writes). | 請閱讀以下內容 [Oak檔案](https://jackrabbit.apache.org/oak/docs/nodestore/segment/overview.html#how-does-compaction-works-with-concurrent-writes)，以及 [執行線上修訂清除](/help/sites-deploying/revision-cleanup.md#running-online-revision-cleanup) 區段。 |
