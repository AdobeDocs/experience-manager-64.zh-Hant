...
**AEM6.4介紹** 「線上修訂清 **除」流**  **** 程的精簡階段的兩種新模式：
...
AEM 6.4還引入了在壓縮過程中更有效的內容重複資料消除機制，這進一步減少了儲存庫的磁碟佔用空間。

以下兩張圖表顯示了內部實驗室測試的結果，這些測試表明，與6.3相比，磁碟的平均執行時間和平均佔用空間AEM在6.4中AEM減少了：
...
### AEM 6.4升級注意事項{#aem-upgrade-considerations}
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>升級至6.4時，應該注意AEM什麼？</td> 
   <td><p>TarMK的永續性格式將隨AEM6.4變更。這些更改不需要主動遷移步驟。 現有儲存庫將進行滾動遷移，該遷移對用戶是透明的。 遷移過程是在第一次AEM訪問儲存庫時啟動的。</p> <p><strong>一旦開始向AEM6.4持久性格式的遷移，儲存庫就無法恢復回以前的AEM6.3持久性格式。</strong></p> </td> 
  </tr> 
 </tbody> 
</table>
