...
>[!NOTE]
>
>本頁概述AEM 6.4的升級程式。 如果您有已部署至應用程式伺服器的安裝，請參閱應用程 [式伺服器安裝的升級步驟](/help/sites-deploying/app-server-upgrade.md)。
...
若要執行升級，請務必使用jar檔案啟動AEM以啟動執行個體。 若要升級至6.4，另請參閱 [Lazy Content Migration](/help/sites-deploying/lazy-content-migration.md) （懶惰內容移轉）中的其他內容重組和移轉選項，您可使用upgrade命令進行選擇。
