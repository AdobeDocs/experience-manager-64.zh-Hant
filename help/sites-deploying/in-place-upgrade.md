...
>[!NOTE]
>
>本頁概述6.4的AEM升級程式。如果您有部署到應用程式伺服器的安裝，請參閱[應用程式伺服器安裝升級步驟](/help/sites-deploying/app-server-upgrade.md)。
...
要執行升級，請務必開始使AEM用jar檔案來開啟實例。 若要升級至6.4，請參閱[Lazy Content Migration](/help/sites-deploying/lazy-content-migration.md)中的其他內容重組和移轉選項，您可使用upgrade命令選擇這些選項。
