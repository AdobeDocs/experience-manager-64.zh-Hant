...
>
>您可以前往 *http://localhost:4502/system/console/status-slingsettings* 並檢查 **&quot;運行模式&quot;** 行。
...
1. 轉到JMX控制台並使用 **org.apache.jackrabbit.oak:狀態（「待機」）** 豆。 如需如何執行此動作的詳細資訊，請參閱 [監控](#monitoring).
1. 停止冷備用實例。
...
1. 維護過程完成並重新啟動實例後：
   ...
   * 待機時，資料儲存垃圾收集僅可透過 **BlobGarbageCollection** MBean - `startBlobGC()`. 此 **儲存庫管理** 備用上沒有MBean。
