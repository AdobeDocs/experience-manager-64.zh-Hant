...
例如，如果實例是以運行模式啟動的， `author,dev,emea`並且兩者 `/apps/*/config.author/` 都 `/apps/*/config.emea.author/` 為定義配置\
`com.day.cq.wcm.core.impl.VersionManagerImpl`，則將應用中 `/apps/*/config.emea.author/` 的配置。
