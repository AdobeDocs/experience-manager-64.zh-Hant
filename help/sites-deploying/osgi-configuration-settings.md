...
* **安裝資料夾名稱regexp** 和安裝文 **件夾的最大層次深度** —指定在何處和到哪個深度搜索儲存庫資料夾以查找要安裝的資源。 使用萬用字元時(如中。&amp;ast;/install)將搜索所有適當的匹配項，例如 `/libs/sling/install` 和 `/libs/cq/core/install`。
...
AEM中 [有兩個](/help/sites-authoring/select-ui.md) UI可用：
