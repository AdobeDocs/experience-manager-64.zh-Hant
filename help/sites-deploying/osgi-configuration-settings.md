...
* **隊列大小**，即池耗盡時線程隊列的最大大小。
   ...
   建議的值為`-1`，因為這會將佇列設為「無限制」；如果設定了限制，則超過限制時可能會發生損失。
...
* **自動** 索引，啟用/停用瀏覽的目錄呈現。
* **啟用** （或停用）預設轉譯，例如 **HMTL**、 **純文字**、 **** JSON **或XML**。
...
* **安裝資料夾** 名稱重 **新展開安裝資料夾的最大層次結構深度**  — 指定在何處和到哪個深度搜索儲存庫資料夾以查找要安裝的資源。使用萬用字元時(如中。&amp;ast;/install)會搜尋所有適當的相符項目，例如`/libs/sling/install`和`/libs/cq/core/install`。
...
* 使用本機安裝（例如，使用`http://localhost:4502/system/console/jcrresolver`）來判斷哪個資源解析器處於活動狀態。
...
建立新配置時，請勿更改工廠配置，而是使用此處提供的配置管理器為此元件建立新的工廠配置：**http://localhost:4502/system/console/configMgr/**。 代理配置在&#x200B;**org.apache.http.proxyconfigurator中可用。**
...
AEM中提供[兩個可用的UI](/help/sites-authoring/select-ui.md):
...
**Day CQ WCM除錯篩** 選器開發時，這很實用，因為它允許在存取頁面時使用尾碼，例如？debug=layout。例如，http://localhost:4502/cf#/content/geometrixx/en/support.html?debug=layout將提供開發人員可能感興趣的版面資訊。
...
* **傳送資料的** URL，會設定用來追蹤頁面統計資料的URL（如果追蹤器請求經過Dispatcher，則此為重要）;例如，預設值為 `http://localhost:4502/libs/wcm/stats/tracker`。
