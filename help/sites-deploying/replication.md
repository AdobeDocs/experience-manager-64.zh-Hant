...
請求將[排入](/help/sites-deploying/osgi-configuration-settings.md)的隊列中，以發送給適當的代理進行處理。
...
* 在「網站」標籤中，會針對個別頁面設定[彩色狀態指標](/help/sites-authoring/publishing-pages.md#determining-publication-status)。
...
* Dispatcher Flush —— 用於管理Dispatcher快取。 有關詳細資訊，請參見[ Authoring Environment](https://helpx.adobe.com/experience-manager/dispatcher/using/page-invalidate.html#invalidating-dispatcher-cache-from-the-authoring-environment)和[ Publishing Instance](https://helpx.adobe.com/experience-manager/dispatcher/using/page-invalidate.html#invalidating-dispatcher-cache-from-a-publishing-instance) Invalizing Dispatcher Cache。

* [反向複製](#replicating-from-publish-to-author) -用於從發佈複製到作者。反向複製不用於社群功能，如論壇、部落格和評論。 由於未啟用外框，因此會有效禁用它。 使用反向複製需要自定義配置。

* 靜態代理——這是一個「將節點的靜態表示儲存到檔案系統中的代理」。 例如，使用預設設定時，內容頁面和dam資產會儲存在`/tmp`下方，以HTML或適當的資產格式。 有關配置，請參見`Settings`和`Rules`頁籤。 這樣，當直接從應用程式伺服器要求頁面時，就可以看到內容。 這是專業代理，而且（可能）對於大多數實例都不是必需的。
...
>[!NOTE]
>
>這可結合使用[代理用戶Id](#settings)來選擇不同的內容以複製到各個發佈環境。 針對每個發佈環境：
>
>1. 配置複製代理以複製到該發佈環境。
...
**Dispatcher Flush**&#x200B;複製代理在作者上不活動。 您可以使用相同的URI，在發佈環境中存取相同的頁面；例如，`http://localhost:4503/etc/replication/agents.publish/flush.html`。
...
* `[!UICONTROL Trigger Size]` -當達到此大小限制時（以MB為單位）啟動批複製。預設值為5 MB。
