...
* `db`:Mongo資料庫的名稱。預設值為&#x200B;**Oak**。 不過，新AEM的6個安裝使用&#x200B;**aem-author**&#x200B;作為預設資料庫名稱。
...
1. 如果AEM已配置為使用Tar或MongoDB儲存，請在繼續操作之前，從`aem-install/crx-quickstart/install`資料夾中刪除任何現有配置檔案。 需要移除的檔案包括：
...
快取支援非同步上傳至DataStore。 檔案會在本機儲存（在檔案系統上），而非同步作業會開始上傳檔案。 非同步上傳的數量受測試快取大小的限制。 使用`stagingSplitPercentage`參數來設定預備快取的大小。 此參數定義用於測試快取的快取大小百分比。 此外，可下載的快取百分比計算為&#x200B;**(100 - `stagingSplitPercentage`)&amp;ast;`cacheSize`**。
...
1. 轉到複製代理設定，方法是開啟到&#x200B;*http://localhost:4502/etc/replication/agents.author/publish.html*&#x200B;的頁。
...
>[!NOTE]
>
>將6AEM.4與TarMK搭配使用時，依預設會將二進位檔儲存在FileDataStore中。 若要搭配Azure DataStore使用TarMK，您必須AEM開始使用`crx3tar-nofds`執行模式，例如：
