...
* `db`:Mongo資料庫的名稱。預設值為&#x200B;**Oak**。 不過，新的AEM 6安裝會使用&#x200B;**aem-author**&#x200B;作為預設資料庫名稱。
...
>[!NOTE]
>
>將AEM 6.4與TarMK搭配使用時，預設會將二進位檔儲存在`FileDataStore`中。 若要搭配S3資料儲存區使用TarMK，您必須使用`crx3tar-nofds`執行模式啟動AEM，例如：
...
1. 如果AEM已設定為可搭配Tar或MongoDB儲存，請先從`aem-install/crx-quickstart/install`資料夾移除任何現有的設定檔案，然後再繼續。 需要移除的檔案包括：
...
>[!NOTE]
>
>將AEM 6.4與TarMK搭配使用時，預設會將二進位檔儲存在FileDataStore中。 若要搭配Azure DataStore使用TarMK，您必須使用`crx3tar-nofds`執行模式啟動AEM，例如：
