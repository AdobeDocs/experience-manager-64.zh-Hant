...
本節介紹一系列基準，用於評估AEM備份的效能以及備份活動對應用程式效能的影響。 AEM備份在執行時會對系統造成巨大負載，我們會測量這一點，以及嘗試調制這些效果的備份延遲設定的效果。 其目標是提供一些關於實際配置和生產資料量中備份預期效能的參考資料，並就如何估計計畫系統的備份時間提供指導。
...
* **Idle** StateBackups在AEM上執行時，不會執行任何其他活動。
