...
刪除索引時，建議您先將屬性設為並執行測試，以確 `type` 保應用程 `disabled` 式在實際刪除前正常運作，以暫時停用索引。 請注意，索引在停用時不會更新，因此，如果重新啟用索引，並可能需要重新建立索引，則可能沒有正確的內容。
...
>[!NOTE]
>
>在AEM 6.4中， [oak-run.jar是MongoMK或RDBMK儲存庫上重新建立索引的唯一支援方法](/help/sites-deploying/indexing-via-the-oak-run-jar.md#reindexingapproachdecisiontree) 。
