...
您可以在公司環境的伺服器上安裝AEM。 典型安裝實例包括：開發、測試和發佈環境。 有關如何將AEM軟體安裝到本地的基本詳細資訊，請參閱[快速入門](/help/sites-deploying/deploy.md#getting-started)部分。
...
* **cq-quickstart-6.4.0.jar:** 獨立執行檔 ** jarfile，包含啟動和執行所需的一切。

* **cq-quickstart-6.4.0.war：用於** 在 ** 協力廠商應用程式伺服器中部署的warfile。
