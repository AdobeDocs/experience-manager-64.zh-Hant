...
* **Web應用程式伺服器**:AEM可以以獨立模式（包括整合的Jetty Web伺服器）部署，或作為第三方應用程式伺服器（WebLogic、WebSphere等）內的Web應用程式部署。
...
您可以在公司環境中的伺服器上安裝AEM。 典型安裝實例包括：開發、測試和發佈環境。 請參閱 [快速入門](/help/sites-deploying/deploy.md#getting-started) 一節，以取得如何將AEM軟體安裝在本機的基本詳細資訊。
...
若要進一步了解Cloud Manager及其資源，請參閱 [**Cloud Manager使用手冊**](https://helpx.adobe.com/experience-manager/cloud-manager/user-guide.html).
...
在用戶端上，AEM可搭配所有現代瀏覽器(**Microsoft Edge**, **Internet Explorer** 11, **鉻黃** 51+ 、 **Firefox** 47+, **Safari** 8+)。 請參閱 [支援的客戶端平台](/help/sites-deploying/technical-requirements.md#supported-client-platforms) 以取得詳細資訊。
...
* **cq-quickstart-6.4.0.jar:** 獨立執行檔 *jar* 包含啟動和運行所需的一切的檔案。

* **cq-quickstart-6.4.0.war:** A *戰爭* 檔案，用於在第三方應用程式伺服器中部署。
...
如果從GUI安裝了實例，則將自動開啟一個瀏覽器窗口，並且還將開啟一個案頭應用程式窗口，顯示實例的主機和埠以及開/關開關：
