...
若使用TarMK，您應在升級前執行離線修訂清除。 這將使儲存庫遷移步驟和後續升級任務的執行速度更快，並有助於確保升級完成後線上修訂清除能夠成功執行。 有關運行離線修訂清除的資訊，請參閱[執行離線修訂清除](https://helpx.adobe.com/experience-manager/6-2/sites-deploying/storage-elements-in-aem-6.html#performing-offline-revision-cleanup)。
