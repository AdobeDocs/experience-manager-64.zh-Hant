...
* `customBlobStore`
   ...
   如果部署將二進位檔案儲存在資料庫中，則它們將構成工作集的一部分。 因此，建議不要在MongoDB中儲存二進位檔案，在NAS上提供諸如`FileSystem`資料儲存的替代資料儲存。

* `cache`

   快取大小(MB)。 這分佈於`DocumentNodeStore`中使用的各種快取中。 預設為256MB。 不過，Oak讀取效能將受益於更大的快取。
...
* `minRecordLength`
   ...
   大小（以位元組為單位）。 小於或等於此大小的二進位檔案將儲存在Document Node Store中。 儲存二進位檔案的內容，而不是儲存二進位檔案的ID。 對於大於此大小的二進位檔案，二進位檔案的ID將作為Document屬性儲存在節點集合中，二進位檔案的主體儲存在磁碟的`FileDataStore`中。 4096位元組是典型的檔案系統塊大小。

* `path`

   資料儲存的根路徑。 若是MongoMK部署，這必須是可供所有AEM例項使用的共用檔案系統。 通常使用網路連接儲存(NAS)伺服器。 對於Amazon Web Services等雲部署，`S3DataFileStore`也可用。

* `cacheSizeInMB`

   二進位快取的總大小（兆位元組）。 它用於快取小於`maxCacheBinarySize`設定的二進位檔案。
...
為了正確提供MongoDB部署服務，將裝載調度程式的作業系統必須運行&#x200B;**Apache httpd 2.4版或更高版本。**
...
如果AEM是在MongoMK永續性管理器部署上執行，則[頁面名稱的限制為150個字元](/help/sites-authoring/managing-pages.md#page-name-restrictions-and-best-practices)。
