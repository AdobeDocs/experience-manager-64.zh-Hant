...
有關MongoDB Cloud Manager的詳細資訊，請參閱[MongoDB文檔](https://docs.mongodb.com/)。
...
* `customBlobStore`
   ...
   如果部署將二進位檔儲存在資料庫中，則它們將成為工作集的一部分。 因此，建議不要在MongoDB中儲存二進位檔，提供NAS上`FileSystem`資料存放區的替代資料存放區。

* `cache`

   快取大小(MB)。 這分佈於`DocumentNodeStore`中使用的各種快取中。 預設為256MB。 不過，更大的快取可讓Oak讀取效能受益。
...
* `minRecordLength`
   ...
   大小（位元組）。 小於或等於此大小的二進位檔會與Document Node Store一起儲存。 儲存二進位檔的內容，而非儲存blob的ID。 對於大於此大小的二進位檔案，該二進位檔案的ID將作為Document的屬性儲存在節點集合中，而該二進位檔案的正文將儲存在磁碟的`FileDataStore`中。 4096位元組是典型的檔案系統塊大小。

* `path`

   資料儲存根的路徑。 若為MongoMK部署，這必須是可供所有AEM執行個體使用的共用檔案系統。 通常使用網路連接儲存(NAS)伺服器。 若為雲端部署，例如Amazon Web Services，也可使用`S3DataFileStore`。

* `cacheSizeInMB`

   二進位快取的總大小(MB)。 它用於快取小於`maxCacheBinarySize`設定的二進位檔。
...
有關如何使用Amazon Web Services設定MongoDB的文檔，請查看以下指南[AWS上的MongoDB](https://docs.aws.amazon.com/quickstart/latest/mongodb/welcome.html)。
...
為了正確提供您的MongoDB部署，將托管Dispatcher的作業系統必須運行&#x200B;**Apache httpd version 2.4或更高版本。**
...
如果AEM在MongoMK永續性管理器部署上執行，[頁面名稱限制為150個字元](/help/sites-authoring/managing-pages.md#page-name-restrictions-and-best-practices)。
