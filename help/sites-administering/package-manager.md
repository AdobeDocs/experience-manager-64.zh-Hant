...
* [封裝共用](#package-share)，這是一個集中式伺服器，其中包含公開可用的封裝以及您公司的私有封裝。公用套件可包含Hotfix、新功能、檔案等。
...
>[!NOTE]
>
>如需從公共區域和您公司內部的套件共用區域下載Hotfix、功能套件和套件的相關資訊，請參閱[套件共用](#package-share)。
>
>從包共用，您可以：
>
>* 從[「封裝共用」直接下載封裝至您的本機AEM執行個體](#downloading-and-installing-packages-from-package-share)。\
   >  下載時，軟體包將導入您的儲存庫，然後，您可以使用&#x200B;**軟體包管理器**&#x200B;立即將其安裝到本地實例。 這些套件包括Hotfix和其他共用套件。
   >
   >
* 從[「包共用」下載包到檔案系統](#downloading-packages-to-your-file-system-from-package-share)。
...
>[!NOTE]
>
>請參閱[將套件上傳至公司內部套件共用](#uploading-a-package) ，以將套件上傳至公司的封裝共用私人區域。
...
* 載入到AEM（從檔案系統](#uploading-packages-from-your-file-system)或從包共用](#downloading-and-installing-packages-from-package-share)下載的[中）[
...
已由[軟體分發替換。](#software-distribution)
