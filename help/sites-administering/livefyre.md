...
1. 從套件管理器，安裝下載的套件。 如需在AEM中使用Software Distribution和套件的指示，請參閱 [如何使用套件](/help/sites-administering/package-manager.md).
...
1. 編輯和儲存 */apps/social/integrations/livefyre/components/authorizablecomponent/authclientlib/auth.js* 實作符合您需求的Livefyre驗證委派。
   ...
   如需自訂驗證委派的詳細資訊，請參閱 [身分整合](https://experienceleague.adobe.com/docs/livefyre/implementation/identity-integration/t-about-identity-integration.html).

   如需AEM Clientlibs的詳細資訊，請參閱 [使用用戶端程式庫](/help/sites-developing/clientlibs.md).
...
為頁面啟用Livefyre後，容器必須設定為允許Livefyre元件。 請參閱 [在設計模式中配置元件](/help/sites-authoring/default-components-designmode.md) 以取得如何啟用不同元件的說明。
...
1. 選取要建立新的Livefyre應用程式，還是內嵌現有應用程式。
   ...
   有關插入元件的詳細資訊，請參見 [編輯頁面內容](/help/sites-authoring/editing-content.md).
