...
AEM 6.4隨附Livefyre功能套件1.2.6預先安裝。 此套件僅包含與AEM Sites的有限Livefyre整合，且必須先解除安裝，才能安裝更新的套件。 透過最新套件，您可體驗Livefyre與AEM的完整整合，包括網站、資產和商務。
...
1. 從「套件管理器」，安裝下載的套件。 如需在AEM中使用「軟體散發」和「套件」的指示，請 [參閱「如何使用套件」](/help/sites-administering/package-manager.md)。
