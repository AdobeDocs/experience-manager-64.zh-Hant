...
>[!NOTE]
>
>客戶端上下文已被ContextHub取代。 如需詳細資訊，請參閱 [配置](/help/sites-administering/contexthub-config.md) 和 [開發人員](/help/sites-developing/contexthub.md) 檔案。
...
* **編輯**
將會開啟新頁面，允許您 [編輯、添加或刪除配置檔案屬性](#editing-property-details).
...
編輯用戶端內容可用來設定（或重設）特定屬性的值。 這可讓您測試特定藍本(對 [細分](/help/sites-administering/campaign-segmentation.md) 和 [行銷活動](/help/sites-authoring/personalization.md))。
...
1. 在網頁瀏覽器中開啟工具主控台([http://localhost:4502/miscadmin#/etc](http://localhost:4502/miscadmin#/etc))。
