...
>[!NOTE]
>
>升級至6.4後，建議將檔案從/etc移動。 如需詳 [細資訊，請參閱AEM 6.4中的「通用資料庫重組](/help/sites-deploying/all-repository-restructuring-in-aem-6-4.md#translation-rules) 」。
...
例如，您可以建立規則，將作者新增的內容轉譯至您頁面上的所有AEM基礎文字元件。 規則可以識別 `/content` 節點和組 `text` 件的屬 `foundation/components/text` 性。
