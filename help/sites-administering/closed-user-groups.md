...
`granite:loginPath`屬性也適用。 只有在驗證要求相關混音類型定義時，才會受到尊重。 在非結構化JCR節點上添加具有此名稱的剩餘屬性將不顯示所需的效果，而負責更新OSGi註冊的處理程式將忽略該屬性。
...
將`granite:AuthenticationRequired` mixin類型添加到配置的受支援路徑中，將導致更新負責的處理程式的OSGi註冊，該註冊包含具有`sling.auth.requirements`屬性的新的附加條目。 如果給定的驗證要求指定了可選的`granite:loginPath`屬性，則該值會附加地向驗證器註冊一個「-」前置詞，以便排除在驗證要求之外。
...
編輯現有CUG策略時需要執行以下步驟。 請注意，修改後的政策需要回寫，而且需要使用`javax.jcr.Session.save()`來持續保留變更。
...
>[!NOTE]
>
>請注意`getEffectivePolicies`和後續程式碼範例之間的差異，這些範例會逐步顯示階層，以找出特定路徑是否已屬於現有CUG的一部分。
...
`JackrabbitAccessControlManager`定義的允許按承擔者編輯訪問控制策略的擴展不會與CUG訪問控制管理一起實現，因為定義中，CUG策略始終影響所有承擔者：與`PrincipalSetPolicy`一起列出的用戶將被授予讀訪問權限，而所有其他承擔者將無法讀取目標節點定義的樹中的內容。
...
>[!NOTE]
>
>如果`RequirementHandler`已設定且目標包含在受支援路徑所定義的樹狀結構中，則上述對指定目標節點的修改只會反映在Apache Sling Authenticator上（請參閱Configuration Options一節）。
>
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> 
    ...
    </ul> <p>此外，請參閱下面的<a href="/help/sites-administering/closed-user-groups.md#configuration-options" target="_blank">配置選項</a>。</p> </td> 
  </tr> 
  <tr> 
   <td>配置策略</td> 
...
### CUG設定和配置{#diferences-in-cug-setup-and-configuration}中的差異
...
### 與授權{#diferences-with-regards-to-authorization}有關的差異
...
在新的實施中，預設授權模型的訪問控制設定不受建立、修改或移除的任何CUG的影響。 而是將名為`PrincipalSetPolicy`的新策略類型應用為目標節點的附加訪問控制內容。 此附加策略將作為目標節點的子節點進行定位，並作為預設策略節點的同級節點（如果存在）。
...
從舊版CUG支援移轉至其他授權模型，會變更有效讀取權限的評估方式。 如[Jackrabbit檔案](https://jackrabbit.apache.org/oak/docs/security/authorization/composite.html)所述，只有在Oak資料庫中所配置的所有模型的權限評估授予讀取存取權時，才會授予允許檢視`CUGcontent`的指定承擔者讀取存取權。
...
1. 輸入要檢查CUG的根路徑，然後按&#x200B;**執行dry run**&#x200B;按鈕。 這會掃描所選位置中可辨識的轉換CUG。
