...
同樣適用於 `granite:loginPath` 屬性。 只有由驗證要求相關的混合類型定義時，才會接受。 在非結構化JCR節點上添加具有此名稱的剩餘屬性將不會顯示所需的效果，而負責更新OSGi註冊的處理程式將忽略該屬性。
...
新增 `granite:AuthenticationRequired` 配置的支援路徑中的mixin類型將導致負責處理程式的OSGi註冊更新，該處理程式包含新的附加條目，該條目包含 `sling.auth.requirements` 屬性。 如果指定的身份驗證要求指定了可選 `granite:loginPath` 屬性，則值會以「 — 」首碼額外註冊給驗證器，以便從驗證需求中排除。
...
編輯現有CUG策略需要執行以下步驟。 請注意，修改後的政策需要回復，且變更需使用 `javax.jcr.Session.save()`.
...
>[!NOTE]
>
>只有在 `RequirementHandler` 已設定，且目標包含在受支援路徑定義的樹中（請參閱「設定選項」一節）。
>
...
建立新驗證需求的步驟，包括登入路徑。 請注意，登入路徑的需求和排除項目只會向Apache Sling Authenticator註冊(若 `RequirementHandler` 已針對包含目標節點的樹進行配置。
...
移除現有驗證需求的步驟。 只有在 `RequirementHandler` 已針對包含目標節點的樹進行配置。
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> 
    ...
    </ul> <p>另請參閱 <a href="/help/sites-administering/closed-user-groups.md#configuration-options" target="_blank">配置選項</a> 下方。</p> </td> 
  </tr> 
  <tr> 
   <td>配置策略</td> 
...
在新實施中，預設授權模型的訪問控制設定不會受到正在建立、修改或刪除的任何CUG的影響。 而是新類型的策略，稱為 `PrincipalSetPolicy` 會以其他存取控制內容的形式套用至目標節點。 此附加策略將作為目標節點的子節點定位，如果存在，則為預設策略節點的同級。
...
從舊的CUG支援移至其他授權模型，會變更評估有效讀取權限的方式。 如 [Jackrabbit檔案](https://jackrabbit.apache.org/oak/docs/security/authorization/composite.html)，指定的主體可檢視 `CUGcontent` 只有在Oak存放庫中所設定模型的權限評估已授予讀取存取權時，才會授予讀取存取權。
