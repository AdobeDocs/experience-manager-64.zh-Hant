...
這將觸發下載包含系統狀態和配置有用資訊的zip。 封存包含例項設定、組合清單、OSGI、Sling度量和統計資料，這可能會產生大型檔案。 您可以使用**Download Status ZIP **視窗，來降低大型狀態檔案的影響。 您可從以下網址訪問該窗口：**AEM >工具>作業>診斷>下載狀態ZIP。**
...
如需執行AEM 6.4修訂版清除的詳細資訊，請參閱此專屬文章](/help/sites-deploying/revision-cleanup.md)。[
