...
1. **Revision Clean Up **task ，位於「每日維護窗口」 **菜單下** 。

1. **Lucene Binaries Cleanup **task ，位於「每日維護窗口」 **菜單下** 。

1. **Workflow purge **task ，位於「Weekly Maintenance Window」(每週維 **護窗口)菜單下** 。
...
如需執行AEM 6.4修訂版清理的詳細資訊，請參 [閱此專屬文章](/help/sites-deploying/revision-cleanup.md)。
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td>布林屬性，定義用戶是否可以停止任務。 如果任務聲明其可停止，則必須在執行過程中檢查其是否已停止，然後採取相應行動。 預設值為false。</td> 
   <td>true</td> 
   <td>可選</td> 
  </tr> 
  <tr> 
