...
>[!NOTE]
>
>診斷頁面中的設定不會影響記錄到日誌檔案的內容，反之亦然。 因此，雖然錯誤日誌可能會捕獲INFO消息，但您可能無法在日誌消息UI中看到這些消息。 此外，透過UI，可從特定封裝擷取DEBUG訊息，而不會影響錯誤記錄。 有關如何配置日誌檔案的詳細資訊，請參見[Logging](/help/sites-deploying/configure-logging.md)。
...
這將觸發下載包含系統狀態和配置有用資訊的zip。 封存包含例項設定、組合清單、OSGI、Sling度量和統計資料，這可能會產生大型檔案。 您可以使用**Download Status ZIP **視窗，來降低大型狀態檔案的影響。 您可從以下網址訪問該窗口：**AEM >工具>操作>診斷>下載狀態ZIP。**
...
1. **版本清理**任務，位於&#x200B;**每日維護窗口**&#x200B;菜單下。

1. **Lucene Binaries Cleanup **task ，位於&#x200B;**Daily Maintenance Window**&#x200B;菜單下。

1. **Workflow purge **task ，位於&#x200B;**Weekly Maintenance Window**&#x200B;菜單下。
...
如需執行6.4版修訂清除的詳細資訊，請AEM參閱此專屬文章](/help/sites-deploying/revision-cleanup.md)。[
...
>[!NOTE]
>
>有關「工作流維護」的詳細資訊，請參閱[本頁](workflows-administering.md#regular-purging-of-workflow-instances)。
...
在部署服務後，它將會顯示在Operations Dashboard UI，並可新增至其中一個可用的維護排程：
