...
1. 指定下列設定：
   ...
   * **匯入工具**
      ...
      特定 [商務提供商](/help/sites-administering/concepts.md#commerce-providers)，預設為 `Geometrixx`.
...
1. 指定下列設定：
   ...
   * **商務提供程式**
      ...
      匯入工具 [商務提供商](/help/sites-administering/concepts.md#commerce-providers);預設Geometrixx。
...
1. 嚮導將開啟。 使用 **基本** 和 **產品標籤** ，輸入 [產品屬性](/help/sites-administering/concepts.md#product-attributes) 用於新產品或產品變體。
...
>
>這表示依預設，會被 [dispatcher](https://helpx.adobe.com/experience-manager/dispatcher/using/dispatcher-configuration.html)，請視需要進行設定。
...
1. 此 [產品屬性](/help/sites-administering/concepts.md#product-attributes) 即會顯示。 使用 **編輯** 和 **完成** 進行任何變更。
...
1. 根據需要修改和移動形狀。
1. 按一下形狀。
1. 按一下瀏覽圖示會開啟 [資產選擇器](/help/assets/asset-selector.md#using-the-asset-selector).
...
1. 切換至 [預覽](/help/sites-authoring/editing-content.md#previewing-pages) 模式。
...
>[!NOTE]
>
>下列範例處理直接在 [行銷活動](/help/sites-authoring/personalization.md)，這用於憑單。
>
>促銷活動也可以在 [體驗](/help/sites-authoring/personalization.md) 行銷活動內。
>
>如需詳細資訊，請參閱 [促銷活動和憑單](#promotions).
...
1. 嚮導將開啟。 使用 **基本**, **內容**, **付款** 和 **履行** 頁簽 [關於新訂單的資訊](/help/sites-administering/concepts.md#order-information).
...
1. 此 [訂購資訊](/help/sites-administering/concepts.md#order-information) 即會顯示。 使用 **編輯** 和 **完成** 進行任何變更。
