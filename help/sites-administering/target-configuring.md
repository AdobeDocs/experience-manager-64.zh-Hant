...
若要找到雲端設定和架構，請導覽至 **Cloud Services** via **工具** > **部署** > **雲**. ([http://localhost:4502/libs/cq/core/content/tools/cloudservices.html](http://localhost:4502/libs/cq/core/content/tools/cloudservices.html))\
在Adobe Target下方，按一下或點選 **顯示配置**.
...
1. 按一下 **連線至Target** 初始化與Target的連線。 如果連接成功，則顯示** Connection successful**消息。 按一下 **確定** 在消息上，然後 **確定** 對話框。
...
設定Target雲端設定後，新增Target架構。 此架構可識別從可用 [用戶端內容](/help/sites-administering/client-context.md) 或 [ContextHub](/help/sites-administering/contexthub-config.md) 元件。 Target使用參數來判斷要套用至目前內容的區段。
...
1. 在「建立框架」對話框中，指定 **標題**，請選取 **Adobe Target架構**，然後按一下 **建立**.
   ...
   框架頁面隨即開啟。 Sidekick提供代表 [用戶端內容](/help/sites-administering/client-context.md) 或 [ContextHub](/help/sites-administering/contexthub-config.md) 你可以映射的。
