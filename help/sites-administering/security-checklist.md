...
>[!NOTE]
>
>進一步資訊 [也提供由開放Web應用程式安全項目(OWASP)發佈的有關最危險安全威脅的資訊](https://www.owasp.org/index.php/OWASP_Top_Ten_Project).
...
>[!NOTE]
>
>如果此執行個體在 [生產就緒模式](/help/sites-administering/production-ready.md). 如果由於任何原因，情況並非如此，您可以前往「套件管理器」解除安裝範例內容，然後搜尋並解除安裝所有We.Retail套件。 如需詳細資訊，請參閱 [如何使用套件](package-manager.md).
...
* [AdobeGraniteHTML程式庫管理員](/help/sites-deploying/osgi-configuration-settings.md):
...
1. 檢查預設轉譯器的配置，這可能是問題區域。
   ...
   * 尤其是JSON轉譯器，可將樹狀結構橫移至多個層級。
      ...
      **JSON最大結果** ( `json.maximumresults`)
      ...
      在 [Apache SlingGETServlet](/help/sites-deploying/osgi-configuration-settings.md). 超過此限制時，呈現會收合。 AEM內Sling的預設值為 `1000`.

   * 作為預防措施，可禁用其他預設渲染器(HTML、純文字檔案、XML)。 再次，借由設定 [Apache SlingGETServlet](/help/sites-deploying/osgi-configuration-settings.md).
   >[!CAUTION]
   >
...
為降低此功能可能造成的DoS風險， `AssetDownloadServlet` 最新AEM版本的發佈例項預設會停用OSGi元件。

如果您的設定需要啟用資產下載伺服器，請參閱 [這篇文章](/help/assets/download-assets-from-aem.md#disable-asset-download-servlet) 以取得更多資訊。
...
1. 開啟CRXDE Lite，前往 *https://&lt;serrveraddress>:4502/crx/de/index.jsp*
1. 選取 `/etc/key` 節點。
1. 前往 **復寫** 標籤。
1. 按下 **復寫** 按鈕。
