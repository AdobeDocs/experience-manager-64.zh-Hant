...
2. 捲動至 **Adobe Analytics** 按一下 **[+]** 下一頁 **可用配置**.
3. 按一下 **[+]** 連結至您的Adobe Analytics設定。
...
5. 在 **報表套裝** 按一下側面盒（主面板右側）的「 」部分 **新增項目**. 然後使用下拉式清單來選取報表套裝ID(例如 `geometrixxauth`)，架構將與此互動。

   >[!NOTE]
   >
   >選取報表套裝ID時，左側的內容尋找器會填入Adobe Analytics變數(SiteCatalyst變數)。

6. 然後使用 **運行模式** 下拉式清單（位於報表套裝ID旁），以選取您要傳送資訊至報表套裝的伺服器例項。

   ![aa-framework-01](assets/aa-framework-01.png)

7. 若要讓架構可在網站的發佈例項上使用，請前往 **頁面** sidekick標籤，按一下 **啟動框架。**
...
請參閱 [查看頁面分析資料](/help/sites-authoring/pa-using.md) 以取得詳細資訊。
