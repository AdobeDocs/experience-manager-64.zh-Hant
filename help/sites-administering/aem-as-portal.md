...
您可以在AEM中執行JSR 286相容的portlet。 Portlet元件允許您在頁上嵌入Portlet。 請參閱[管理AEM內容Portlet](#administering-the-aem-content-portlet)。
...
若要設定AEM內容以顯示在入口網站中，請參閱[ Installing, Configuring and Using AEM in a portlet](#installing-configuring-and-using-aem-in-a-portlet)。
...
假設入口伺服器在主機localhost、連接埠8080上執行，而AEM portlet Web應用程式已載入Web應用程式內容&#x200B;*cqportlet*，則Web主控台的URL為`http://localhost:8080/cqportlet/cqbridge/system/console`。 預設用戶和密碼為&#x200B;**admin**。
...
1. 假設入口伺服器運行於主機&#x200B;**localhost**、**埠8080 **，並且AEM portlet Web應用程式掛載於上下文&#x200B;**cqportlet**&#x200B;中，用於刷新快取的URL為`http://localhost:8080/cqportlet/cqbridge/cqpcache?Path=$(path)`。 使用GET做為方法。
...
1. 按一下「編輯」。****
1. 在&#x200B;**Settings**&#x200B;標籤中，選擇&#x200B;**Enabled**&#x200B;核取方塊，選擇**Dispatcher Flush **作為序列化類型，並輸入重試超時（例如60000）。
...
發佈檢視只有一個按鈕，可將工具列切換至「管理」檢視。 發佈檢視由[上一個bundle](/help/sites-deploying/configuring-osgi.md)中的publish.html檔案表示。 在HTML中，您可以使用下列佔位符，在呈現時，這些佔位符將由portlet替換為各自的內容：
...
管理檢視有四個按鈕：「編輯」、「網站」標籤、「重新整理」和「上一步」。 管理視圖由[上一個包](/help/sites-deploying/configuring-osgi.md)中的manage.html檔案表示。 在HTML中，您可以使用下列佔位符，在呈現時，這些佔位符將由portlet替換為各自的內容：
...
當從AEM要求內容時，Portlet會使用目前的顯示模式（發佈或作者）和目前的路徑來組合完整的URL。 使用預設值時，第一個URL為`http://localhost:4503/content/geometrixx/en.portlet.html`。 `htmlSelector`的值會自動新增至副檔名前的URL。

如果Portlet切換到幫助模式並選擇了`appendHelpViewModeAsSelector` ，則`help`選擇器也會附加，例如`http://localhost:4503/content/geometrixx/en.portlet.html.help`。 如果將portlet窗口最大化並選擇了`appendMaxWindowStateAsSelector` ，則還會附加選擇器，例如`http://localhost:4503/content/geometrixx/en.portlet.max.help`。
