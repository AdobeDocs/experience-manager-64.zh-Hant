...
若要設定AEM內容以顯示在入口網站中，請參閱 [在portlet中安裝、配置和使用AEM](#installing-configuring-and-using-aem-in-a-portlet).
...
假設入口伺服器在主機localhost上運行，埠8080和AEM portlet web應用程式在Web應用程式上下文中裝載 *cqportlet*,web主控台的url為 `http://localhost:8080/cqportlet/cqbridge/system/console`. 預設的用戶和密碼為 **管理員**.
...
1. 假設入口伺服器在主機上運行 **localhost**、 **埠8080 **和AEM portlet web應用程式已裝載在上下文中 **cqportlet**，則用於排清快取的url為 `http://localhost:8080/cqportlet/cqbridge/cqpcache?Path=$(path)`. 使用GET作為方法。
...
1. 按一下 **編輯。**
1. 在 **設定** 頁簽，選擇 **已啟用** 核取方塊，選取**Dispatcher排清**作為序列化類型，然後輸入重試逾時(例如60000)。
...
1. 在OSGi設定主控台中，依預設位於http://localhost:4502/system/console/configMgr ，選取 **CQ PIN驗證處理常式** 從下拉式功能表。
...
發佈檢視只有一個按鈕可將工具列切換為「管理」檢視。 發佈檢視由 [上一捆綁](/help/sites-deploying/configuring-osgi.md). 在HTML中，可以使用以下佔位符，這些佔位符在呈現時將替換為相應的內容：
...
管理視圖有四個按鈕：編輯、網站索引標籤、重新整理及返回。 管理檢視由 [上一捆綁](/help/sites-deploying/configuring-osgi.md). 在HTML中，可以使用以下佔位符，這些佔位符在呈現時將替換為相應的內容：
...
要安裝自定義佈局，請訪問Portlet的OSGI Web控制台 **套件組合** 區段，並上傳套件組合。
...
1. 選取或清除 **隱藏標題欄** 複選框，具體取決於您是否要查看portlet的標題欄。
...
從AEM請求內容時，Portlet使用當前顯示模式（發佈或製作）和當前路徑來組合完整的URL。 若使用預設值，則第一個url為 `http://localhost:4503/content/geometrixx/en.portlet.html`. 的值 `htmlSelector` 會在擴充功能之前自動新增至URL。

如果Portlet切換到幫助模式，則 `appendHelpViewModeAsSelector` ，則 `help` 選取器也會附加，例如 `http://localhost:4503/content/geometrixx/en.portlet.html.help`. 如果Portlet窗口已最大化，並且 `appendMaxWindowStateAsSelector` ，則也會附加選取器，例如 `http://localhost:4503/content/geometrixx/en.portlet.max.help`.
