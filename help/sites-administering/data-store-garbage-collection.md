...
>[!NOTE]
>
>只有在您已設定外部檔案資料儲存時，資料儲存廢棄項目收集工作才會啟動。 如果尚未配置外部檔案資料儲存，則任務將在調用後返回 `Cannot perform operation: no service of type BlobGCMBean found` 消息。 如需 [如何設定檔案資料儲存區的詳細資訊](/help/sites-deploying/data-store-config.md#file-data-store) ，請參閱「在AEM 6中設定節點儲存區和資料儲存區」。
