...
>[!NOTE]
>
>預配置的報告不需要佔用大量的效能，但仍建議在生產環境中使用每日快照。 如果可以在網站上活動不多時，在一天中的某個時間運行這些每日快照；這可以用 `Daily snapshots (repconf.hourofday)` 參數 **Day CQ報表設定**;請參閱 [OSGI設定](/help/sites-deploying/configuring-osgi.md) 以取得如何設定此項目的詳細資訊。
...
>title="磁碟使用情況報表"
>abstract="作為 CTT 計算機的輸入，使用者可以透過執行「http://HOST:PORT/etc/reports/diskusage.html」下的磁碟使用情況報表來預估 AEM 存放庫的大小。使用者還能使用「路徑」參數來預估特定存放庫路徑的大小，例如：「http://HOST:PORT/etc/reports/diskusage.html?path=/content/dam」。"
