...
description: 了解如何整合AEM與Dynamic Media Classic。
...
>* 如果您使用Dynamic Media Classic搭配自訂的代理設定，請設定這兩個HTTP用戶端代理設定，因為Experience Manager的某些功能是使用3.x API，而其他功能則是使用4.x API。 3.x的設定 [http://localhost:4502/system/console/configMgr/com.day.commons.httpclient](http://localhost:4502/system/console/configMgr/com.day.commons.httpclient) 和4.x [http://localhost:4502/system/console/configMgr/org.apache.http.proxyconfigurator](http://localhost:4502/system/console/configMgr/org.apache.http.proxyconfigurator).
>


## Experience Manager/Dynamic Media Classic整合與Dynamic Media {#aem-scene-integration-versus-dynamic-media}

Experience Manager使用者可以選擇使用Dynamic Media的兩種解決方案：

* 搭配使用您的整合式Experience Manager例項 *Dynamic Media Classic*.
* 使用 *Dynamic Media* 整合至Experience Manager。

如果最適合您，請使用下列條件來判斷哪個解決方案：

* 如果您是 **現有** Dynamic Media Classic客戶的豐富型媒體資產位於Dynamic Media Classic以供發佈和傳送，但您想要將這些資產與Sites(WCM)製作和/或Experience Manager Assets整合以進行管理，請使用 [Experience Manager/Dynamic Media Classic點對點整合](#aem-scene-point-to-point-integration) 本文檔中描述。
...
* **[!UICONTROL 互動式媒體]** - **[!UICONTROL 互動式媒體]** 元件用於那些轉盤橫幅、互動式影像和互動式視訊，這些內容在熱點或影像地圖中具有互動性。 此元件是智慧的。 視您新增影像或影片而定，您有各種選項。 此外，檢視器回應速度快。 也就是說，螢幕的大小會根據螢幕大小自動變更。 所有檢視器均為HTML5檢視器。
...
1. 請依照您使用案例的特定指示操作：
   ...
   <table> 
    ...
    <td><p>上傳資產至Experience Manager，並使用Dynamic Media產生轉譯以供下載/共用。 自動將Experience Manager Assets發佈至Dynamic Media Classic以進行傳送。</p> <p><strong>重要：</strong> Experience Manager中產生的重複處理和轉譯不會同步到Dynamic Media Classic</p> </td> 
    <td><p>開啟</p> <p>（請參閱步驟3）</p> </td> 
    <td>關閉</td> 
    <td>關閉</td> 
    <td><p><a href="#configuringautouploadingfromaemassets">開啟</a></p> <p>（請參閱步驟4）</p> </td> 
...
1. (可選；請參閱使用案例表格) — 如果您選擇啟用「從資產自動上傳至Dynamic Media Classic」，您必須新增下列項目：
...
* 如果您必須針對某個使用案例使用Dynamic Media，並針對另一個使用案例使用Dynamic Media Classic整合，讓Dynamic Media資產不會與Dynamic Media Classic系統互動，則請勿將Dynamic Media Classic設定套用至Dynamic Media資料夾。 此外，請勿將Dynamic Media設定（處理設定檔）套用至Dynamic Media Classic資料夾。

## 整合Dynamic Media Classic與Experience Manager的最佳作法 {#best-practices-for-integrating-scene-with-aem}

將Dynamic Media Classic與Experience Manager整合時，以下方面需遵循一些重要的最佳實務：
...
如果資產已在Dynamic Media Classic中，且在通用存放庫中管理這些資產並不重要，則將資產匯出至Experience Manager Assets只是為了將資產同步回Dynamic Media Classic以進行傳送，便不必往返。 否則，最好將資產保留在單一存放庫中，並僅同步至Dynamic Media Classic以進行傳送。
...
1. 如果您想要讓Experience Manager資產自動上傳至Dynamic Media Classic，請開啟選項，並將Dynamic Media Classic新增至DAM更新資產工作流程。 您也可以手動上傳資產。
1. 將Dynamic Media Classic元件新增至sidekick。 如此可讓使用者在其Experience Manager頁面上使用Dynamic Media Classic元件。
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>臨機資料夾</td> 
   <td>駐留在WCM中手動上傳至Dynamic Media Classic之目標資料夾外的資產，會自動放置在Dynamic Media Classic上的個別臨機資料夾中。 您可在雲端設定中以Experience Manager配置此功能。</td> 
  </tr> 
  <tr> 
   <td>混合媒體</td> 
  ...
  <tr> 
   <td>Target子資料夾</td> 
   <td><p>如果您在目標資料夾內使用子資料夾，請對每個資產使用唯一的名稱（不論位置），或設定Dynamic Media Classic（在「設定」區域），不會覆寫資產（不論位置）。</p> <p>否則，上傳至Dynamic Media Classic Target子資料夾的同名資產將會上傳，但目標資料夾中同名資產則會刪除。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
如果在代理後運行Experience Manager或具有特殊的防火牆設定，則必須顯式啟用不同區域的主機。 伺服器在 `/etc/cloudservices/scene7/endpoints` 和可視需要自訂。 點選URL，然後編輯以變更URL（如有必要）。 在舊版Experience Manager中，這些值是硬式編碼。
...
>
>如果您想要測試促進整合，請將根資料夾指向僅子資料夾，而非整個公司。

>[!NOTE]
>
>您可以有多個設定：一個雲端設定代表Dynamic Media Classic公司的一名使用者。 如果您想要存取其他Dynamic Media Classic公司或使用者，請建立多個設定。

若要設定Experience Manager以將資產發佈至Dynamic Media Classic:

1. 點選Experience Manager圖示並導覽至 **[!UICONTROL 部署>Cloud Services]** 存取Adobe Dynamic Media Classic。

1. 點選 **[!UICONTROL 立即配置]**.

   ![chlimage_1-297](assets/chlimage_1-297.png)

1. 在 **[!UICONTROL 標題]** 欄位，並選填 **[!UICONTROL 名稱]** 欄位，輸入相應的資訊。 點選 **[!UICONTROL 建立]**.
...
1. 輸入您Dynamic Media Classic帳戶的電子郵件地址、密碼和地區，然後點選 **[!UICONTROL 連線至Dynamic Media Classic]**. 您已連線至Dynamic Media Classic伺服器，對話方塊會展開並顯示更多選項。
...
1. 點選 [!UICONTROL 工具] 圖示，然後導覽至 **[!UICONTROL 操作> Web控制台]**. Web控制台開啟。
1. 導覽至 **[!UICONTROL Adobe CQ Dynamic Media Classic Dam監聽器]** ，然後選取 **[!UICONTROL 已啟用]** 框。
...
如果Experience Manager例項設定為透過Dynamic Media Classic處理視訊編碼，依預設，任何上傳工作都會有35分鐘的逾時。 為了適應運行時間可能更長的視頻編碼作業，可以配置此設定：
...
1. 視需要在 **[!UICONTROL 活動作業超時]** 欄位。 以秒為單位接受任何非負數。 此數字預設為2100。
   ...
   >[!NOTE]
   >
   >最佳實務：大部分資產最多會在數分鐘內擷取（例如影像）。 但在某些情況下（例如較大的視訊），會將逾時值增加至7200秒（2小時），以容納長的處理時間。 否則，此Dynamic Media Classic上傳工作會標示為 **[!UICONTROL 上傳失敗]** 在JCR中繼資料中。

1. 點選 **[!UICONTROL 儲存]**.

### 從Experience Manager Assets上線 {#autouploading-from-aem-assets}

從Experience Manager6.3.2開始，系統現在已為您設定Experience Manager Assets，如果您上傳至數位資產管理員的任何數位資產位於CQ目標資料夾中，系統便會將這些資產自動更新至Dynamic Media Classic。
...
若要從Experience Manager Assets設定自動上傳：

1. 點選Experience Manager圖示並導覽至 **[!UICONTROL 部署>Cloud Services]** 然後，在「 Dynamic Media 」標題下的「可用配置」下，點選 **[!UICONTROL dms7(Dynamic Media)]**)
1. 點選 **[!UICONTROL 進階]** 頁簽，選擇 **[!UICONTROL 啟用自動上傳]** 核取方塊，然後點選 **[!UICONTROL 確定]**. 您現在必須設定DAM Asset工作流程，以納入上傳至Dynamic Media Classic。
...
1. 導覽回Experience Manager歡迎頁面，然後點選 **[!UICONTROL 工作流程]**. 按兩下 **DAM更新資產** 工作流程來開啟它。
1. 在sidekick中，導覽至 **[!UICONTROL 工作流程]** 元件，然後選取 **[!UICONTROL Dynamic Media Classic]**. 拖曳 **[!UICONTROL Dynamic Media Classic]** 前往工作流程並點選 **[!UICONTROL 儲存]**. 新增至目標資料夾中Experience Manager Assets的資產會自動上傳至Dynamic Media Classic。
...
如果您想在將資產推送至Dynamic Media Classic時立即發佈資產，則不需要設定任何選項。 此行為為預設值。
...
1. [使用Admin Console建立支援案例。](https://helpx.adobe.com/tw/enterprise/admin-guide.html/enterprise/using/support-for-experience-cloud.ug.html) 在您的支援案例中，請為您的Dynamic Media Classic帳戶啟用安全預覽的要求。
1. 請依照 [設定Dynamic Media Classic帳戶的安全預覽。](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/upload-publish/testing-assets-making-them-public.html#upload-publish)
...
1. 點選Experience Manager圖示並導覽至 **[!UICONTROL 部署>Cloud Services]**，點選 **[!UICONTROL Dynamic Media Classic]**，然後在Dynamic Media Classic中選取您的設定。
1. 點選 **[!UICONTROL 進階]** 標籤。 在 **[!UICONTROL 啟用安全視圖]** 下拉式功能表，選取 **[!UICONTROL AEM Publish啟動後]** 將資產推送至Dynamic Media Classic而不進行發佈。 (依預設，此值會設為 **[!UICONTROL 立即]**，會立即發佈Dynamic Media Classic資產。)

   請參閱 [Dynamic Media Classic檔案](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/upload-publish/testing-assets-making-them-public.html#upload-publish) ，以取得在公開資產前測試資產的詳細資訊。

   ![chlimage_1-302](assets/chlimage_1-302.png)

1. 點選 **[!UICONTROL 確定]**.

啟用安全檢視表示資產已取消發佈並推送至安全預覽伺服器。

若要查看是否已啟用安全預覽，請導覽至Dynamic Media Classic中頁面上的Experience Manager元件。 點選 **[!UICONTROL 編輯]**. 資產的URL中列有安全預覽伺服器。 在Experience Manager中發佈後，檔案參考中的伺服器網域會從預覽URL更新至生產URL。
...
若要在觸控最佳化UI中啟用Dynamic Media Classic for WCM:

1. 點選Experience Manager圖示並導覽至 **[!UICONTROL 網站]** 然後是您網站的根頁面（非語言特定）。

1. 在工具列中選取 [!UICONTROL 設定] 圖示和點選 **[!UICONTROL 開啟屬性]**.

1. 點選 **[!UICONTROL Cloud Services]** 點選 **[!UICONTROL 新增設定]** 選取 **[!UICONTROL Dynamic Media Classic]**.
1. 在 **[!UICONTROL Adobe Dynamic Media Classic]** 下拉式清單，選取所需的設定，然後點選 **[!UICONTROL 確定]**.

   ![chlimage_1-303](assets/chlimage_1-303.png)

   來自Dynamic Media Classic設定的視訊預設集可用於Experience Manager該頁面和子頁面上的Dynamic Media Classic視訊元件。

#### 在傳統使用者介面中啟用Dynamic Media Classic for WCM {#enabling-scene-for-wcm-in-the-classic-user-interface}

若要在傳統UI中啟用Dynamic Media Classic for WCM:

1. 在Experience Manager中，點選 **[!UICONTROL 網站]** 並導覽至您網站的根頁面（非語言特定）。

1. 在側腳中，點選 **[!UICONTROL 頁面]** 圖示和點選 **[!UICONTROL 頁面屬性]**.
...
### 設定Ad-hoc資料夾 {#configuring-the-ad-hoc-folder}

當資產不在CQ Target資料夾時，您可以設定資產上傳至Dynamic Media Classic的資料夾。 請參閱從CQ目標資料夾外部發佈資產。
...
1. 點選 **[!UICONTROL 進階]** 標籤。 在 **[!UICONTROL 臨機資料夾]** 欄位，您可以修改 **臨機** 檔案夾。 依預設，它是 **name_of_the_company/CQ5_adhoc**.

   ![chlimage_1-305](assets/chlimage_1-305.png)

### 設定通用預設集 {#configuring-universal-presets}

若要設定視訊元件的通用預設集，請參閱 [影片](/help/assets/s7-video.md).
...
1. 點選Experience Manager圖示並導覽至 **[!UICONTROL 「工具」>「操作」>「Web控制台」]**.
1. 在「 Adobe Experience Manager Web Console設定」面板中， **[!UICONTROL OSGi]** 菜單，點選 **[!UICONTROL 設定]**.
1. 在「名稱」欄下，尋找並點選 **[!UICONTROL Adobe CQ Dynamic Media Classic資產MIME類型服務]** 來編輯配置。
...
1. 在 **[!UICONTROL jobParam]** 值欄位。
   ...
   >
   >請確定您的PSD檔案有「圖層」。 如果嚴格來說是一個影像或帶有遮罩的影像，則只會將其處理為影像，因為沒有要處理的圖層。

1. 在CRXDE Lite頁面的左上角，點選 **[!UICONTROL 全部儲存]**.
...
* 檢查您嘗試上傳的資產是否位於 **[!UICONTROL CQ目標]** 資料夾(可在Dynamic Media Classic雲端設定中指定此資料夾)。
* 若非如此，您必須在 **[!UICONTROL 頁面屬性]** 頁面上傳至 **[!UICONTROL CQ adhoc]** 檔案夾。
...
>[!CAUTION]
>
>從現有Dynamic Media Classic公司帳戶匯入資產可能需要很長時間才會顯示在Experience Manager中。 請確定您在Dynamic Media Classic中指定的資料夾中的資產不太多（例如，根資料夾中的資產通常不太多）。
>
