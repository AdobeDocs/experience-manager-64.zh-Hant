...
description: 了解如何將AEM與Dynamic Media Classic整合。
...
>* 如果您使用Dynamic Media Classic搭配自訂的代理設定，請設定這兩個HTTP用戶端代理設定，因為Experience Manager的某些功能是使用3.x API，而有些則使用4.x API。 3.x以[http://localhost:4502/system/console/configMgr/com.day.commons.httpclient](http://localhost:4502/system/console/configMgr/com.day.commons.httpclient)配置，4.x以[http://localhost:4502/system/console/configMgr/org.apache.http.proxyconfigurator](http://localhost:4502/system/console/configMgr/org.apache.http.proxyconfigurator)配置。
...
* 搭配&#x200B;*Dynamic Media Classic*&#x200B;使用Experience Manager的整合例項。
* 使用整合至Experience Manager的&#x200B;*Dynamic Media*。

如果最適合您，請使用下列條件來判斷哪個解決方案：

* 如果您是&#x200B;**現有** Dynamic Media Classic客戶，其豐富型媒體資產位於Dynamic Media Classic以進行發佈和傳送，但您想要將這些資產與Sites(WCM)製作和/或Experience Manager資產整合以進行管理，請使用本檔案中所述的[Experience Manager/Dynamic Media Classic點對點整合](#aem-scene-point-to-point-integration)。
...
* **[!UICONTROL 互動式媒體]**  — 互 **[!UICONTROL 動式]** 媒體元件適用於傳送橫幅、互動式影像和互動式視訊，這些視訊在這些熱點或影像地圖中具有互動性。此元件是智慧的。 視您新增影像或影片而定，您有各種選項。 此外，檢視器回應速度快。 也就是說，螢幕的大小會根據螢幕大小自動變更。 所有檢視器都是HTML5檢視器。
...
1. 請依照您使用案例的特定指示操作：
   ...
   <table> 
    ...
    <td><p>上傳資產至Experience Manager，並使用Dynamic Media產生轉譯以供下載/共用。 自動將Experience Manager資產發佈至Dynamic Media Classic以進行傳送。</p> <p><strong>重要：</strong> 產生的處理重複，且Experience Manager中產生的轉譯不會同步至Dynamic Media Classic</p> </td> 
    <td><p>開啟</p> <p>（請參閱步驟3）</p> </td> 
    <td>關閉</td> 
    <td>關閉</td> 
    <td><p><a href="#configuringautouploadingfromaemassets">開啟</a></p> <p>（請參閱步驟4）</p> </td> 
...
將Dynamic Media Classic與Experience Manager整合時，以下方面需留意一些重要的最佳實務：
...
1. 如果您希望Experience Manager資產自動上傳至Dynamic Media Classic，請開啟選項，並將Dynamic Media Classic新增至DAM更新資產工作流程。 您也可以手動上傳資產。
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>臨機資料夾</td> 
   <td>駐留在WCM中手動上傳至Dynamic Media Classic的Target資料夾外的資產，會自動放置在Dynamic Media Classic上的個別臨機資料夾中。 您可在雲端設定中以Experience Manager配置此功能。</td> 
  </tr> 
  <tr> 
   <td>混合媒體</td> 
  ...
  <tr> 
   <td>Target子資料夾</td> 
   <td><p>如果您在目標資料夾內使用子資料夾，請對每個資產使用唯一的名稱（不論位置），或設定Dynamic Media Classic（在「設定」區域），不會覆寫資產（無論位置）。</p> <p>否則，上傳至Dynamic Media Classic目標子資料夾的同名資產將會上傳，但目標資料夾中同名的資產則會刪除。 </p> </td> 
  </tr> 
 </tbody> 
</table>
...
如果導覽至`/etc/cloudservices/scene7/endpoints.html`，您會看到列出的伺服器（並可按一下URL加以編輯）:
...
從Experience Manager6.3.2開始，系統現在已為您設定Experience Manager資產，如果資產位於CQ目標資料夾中，您上傳至數位資產管理器的任何數位資產都會自動更新至Dynamic Media Classic。
...
1. [使用Admin Console建立支援案例。](https://helpx.adobe.com/enterprise/admin-guide.html/enterprise/using/support-for-experience-cloud.ug.html) 在您的支援案例中，請求為Dynamic Media Classic帳戶啟用安全預覽。
1. 請依照[為您的Dynamic Media Classic帳戶設定安全預覽的指示操作。](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/upload-publish/testing-assets-making-them-public.html#upload-publish)
...
1. 點選&#x200B;**[!UICONTROL 進階]**&#x200B;標籤。 在&#x200B;**[!UICONTROL 啟用安全檢視]**&#x200B;下拉式選單中，選取「AEM發佈啟動時&#x200B;]**」 ，即可將資產推送至Dynamic Media Classic，而不需發佈。**[!UICONTROL (預設情況下，此值會設為&#x200B;**[!UICONTROL Immediale]**，其中會立即發佈Dynamic Media Classic資產。)
   ...
   請參閱[Dynamic Media Classic檔案](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/upload-publish/testing-assets-making-them-public.html#upload-publish) ，以取得在公開之前測試資產的詳細資訊。
...
1. 在&#x200B;**[!UICONTROL jobParam]**&#x200B;值欄位中指定Dynamic Media Classic上傳工作參數。
   ...
   >
   >確保PSD檔案具有「圖層」。 如果嚴格來說是一個影像或帶有遮罩的影像，則只會將其處理為影像，因為沒有要處理的圖層。
...
* 檢查您嘗試上傳的資產是否位於&#x200B;**[!UICONTROL CQ目標]**&#x200B;資料夾中(您可在Dynamic Media Classic雲端設定中指定此資料夾)。
* 若未設定，您必須在&#x200B;**[!UICONTROL 頁面屬性]**&#x200B;中設定該頁面的雲端設定，以允許上傳至&#x200B;**[!UICONTROL CQ adhoc]**&#x200B;資料夾。
...
>[!CAUTION]
>
>從現有的Dynamic Media Classic公司帳戶匯入資產可能需花很長的時間才會顯示在Experience Manager中。 請確定您在Dynamic Media Classic中指定的資產不太多的資料夾（例如，根資料夾的資產通常太多）。
>
