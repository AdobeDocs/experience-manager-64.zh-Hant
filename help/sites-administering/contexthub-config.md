...
要從jsontest.com站點的MD5服務中保存資料，請使用以下屬性中的建立ContextHub儲存區](/help/sites-administering/contexthub-config.md#creating-a-contexthub-store)過程：[
...
使用[新增UI模組](/help/sites-administering/contexthub-config.md#adding-a-ui-module)中的程式，將UI模組新增至現有UI模式，例如範例Perona UI模式。 對於UI模組，請使用下列屬性值：
