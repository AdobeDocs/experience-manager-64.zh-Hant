...
Analytics的設計考量到繼承機制。 通常，您會在頁面屬性[雲端服務](/help/sites-developing/extending-cloud-config.md)標籤中新增對Analytics設定的參考，以啟用網站的Analytics。 然後，除非頁面需要不同的設定，否則設定會自動繼承至所有子頁面，而不需要再次參照。 新增對網站的參考也會自動建立數個類型`cq;PollConfig`的節點（12代表AEM 6.3及舊版，6代表AEM 6.4），這些節點會執行個體化用來將Analytics資料匯入AEM的PollingImporters。 因此：
