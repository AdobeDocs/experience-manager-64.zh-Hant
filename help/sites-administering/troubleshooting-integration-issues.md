...
Analytics的設計考量到繼承機制。 通常，您會在頁面屬性[Cloud Services](/help/sites-developing/extending-cloud-config.md)標籤內新增對Analytics設定的參考，以啟用網站的Analytics。 然後，除非頁面需要不同的設定，否則設定會自動繼承至所有子頁面，而不需要再次參照。 將參考新增至網站也會自動建立數個類型`cq;PollConfig`的節點(AEM 6.3的12個及舊版，AEM 6.4的6個)，將用來將Analytics資料匯入AEM的PollingImporter實例化。 因此：
