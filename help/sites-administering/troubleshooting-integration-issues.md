...
Analytics的設計考量到繼承機制。 通常，您會在頁面屬性「雲端服務」索引標籤中新增對Analytics設定的參考，以啟 [用網站的Analytics](/help/sites-developing/extending-cloud-config.md) 。 然後，除非頁面需要不同的設定，否則設定會自動繼承至所有子頁面，而不需要再次參照。 新增對網站的參考也會自動建立數個節點（AEM 6.3和舊版的節點12或AEM 6.4的節點6），這些節點會執行個體化用來將Analytics資料匯入AEM的 `cq;PollConfig` PollingImporters。 因此：
