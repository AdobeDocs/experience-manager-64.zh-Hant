...
* **名稱***（可選）*命名空間的名稱。 如果未指定，則從「標題」建立有效的節點名稱。 請參閱 [TagID](/help/sites-developing/framework.md#tagid)。
...
選取命名空間或其他標籤時，選取**`References`**圖示會識別已套用標籤的內容。
...
>[!NOTE]
>
>建議僅允許「作者」編 [輯標籤](#editing-tags) , `title`而不要移動或重新命名標籤。
...
當選取名稱空間或其他標籤時，選取**`Merge`**圖示將會開啟一個面板，其中可選取要合併的路徑。
...
當選取命名空間或其他標籤時，選取**`Delete`**圖示會從作者環境中永久移除標籤。 如果標籤已發佈，也會從發佈環境中移除。 如果選取的標籤是容器標籤，其所有子標籤也會移除。
