...
選取命名空間或其他標籤時，請選取**`References`**圖示會識別已套用標籤的內容。
...
選取命名空間或其他標籤時，請選取**`Merge`**圖示會開啟一個面板，供您選取要合併的路徑。
...
選取命名空間或其他標籤時，請選取**`Publish`**圖示來啟動發佈環境中的標籤。 與頁面內容類似，無論是否為容器標籤，都只會發佈選取的標籤。
...
選取命名空間或其他標籤時，請選取**`Unpublish`**圖示會在製作環境中停用標籤，並將其從發佈環境中移除。 類似於 `Delete`操作，如果選取的標籤是容器標籤，則其所有子標籤將在製作環境中停用，並從發佈環境中移除。

### 刪除標籤 {#deleting-tags}

![chlimage_1-203](assets/chlimage_1-203.png)

選取命名空間或其他標籤時，請選取**`Delete`**圖示會從製作環境中永久移除標籤。 如果標籤已發佈，也會從發佈環境中移除。 如果選取的標籤是容器標籤，其所有子標籤也將一併移除。
