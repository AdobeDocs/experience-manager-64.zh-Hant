...
>[!NOTE]
>
>AEM 6.4.5.0或更新版本必須搭配使用可編輯的範本 [SPA Editor](/help/sites-developing/spa-overview.md).
...
* 可編輯的範本
   ...
   * 可以是 [已建立](#creating-a-new-template-template-author) 和 [編輯](#editing-templates-template-authors) 由範本作者使用 **範本** 主控台和編輯器。 此 **範本** 主控台可在 **一般** 區段 **工具** 控制台。
...
>[!CAUTION]
>
>切勿輸入任何需要 [國際化](/help/sites-developing/i18n.md) 填入範本。
...
1. 選擇 **建立**。將顯示確認，請選擇 **開啟** 開始 [編輯範本](#editing-templates-template-authors) 或 **完成** 返回模板控制台。
...
1. 您現在可以在 [建立新頁面](/help/sites-authoring/managing-pages.md#creating-a-new-page)，不過您可能想要 [編輯範本](#editing-templates-template-authors) 根據你的要求。
...
若 **頁面原則** 選項 **頁面資訊** 功能表 [選擇所需的頁面策略](#editing-a-template-structure-template-author):
...
* **新增元件**
   ...
   * 要在元件已解鎖時顯示的標籤
   ...
   >[!NOTE]
   >
   >將元件和資產新增至範本雖然不相同，但有許多類似動作，例如 [頁面編寫](/help/sites-authoring/editing-content.md).
...
* **策略和屬性（一般）**
   ...
   ***政策***
   ...
   >[!NOTE]
   >
   >如果添加了多個相同類型的元件作為初始內容，則相同的策略適用於所有元件。 這反映了 **[設計模式](/help/sites-authoring/default-components-designmode.md)** ，適用於靜態範本。
   ...
   *功能*
   ...
   >[!CAUTION]
   >
   >請注意，在AEM裁切比例中，定義為 **高度/寬度**. 這與傳統的寬度/高度定義不同，且是由於舊版相容性原因而完成。 若您定義 **名稱** UI中會顯示這個。

   >[!NOTE]
   >
   >[實作RTF編輯器的元件的內容原則](/help/sites-administering/rich-text-editor.md) 只能針對RTE透過其UI設定可用的選項來定義。
...
>[!NOTE]
>
>**初始內容** 可使用該範本建立之頁面的編輯模式時，可考慮使用該模式。 因此，未在 **初始內容** 模式，而 **[結構](/help/sites-authoring/templates.md#editing-a-template-structure-template-author)** 模式。
