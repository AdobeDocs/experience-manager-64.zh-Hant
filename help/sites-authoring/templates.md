...
* 可編輯的範本
   ...
   * 範本作 [者可使用](#creating-a-new-template-template-author) Template [主控台和編輯](#editing-templates-template-authors) 器來建立和編輯範本 **** 。 您可 **以在** 「工具」控制台的「一般 **」區段中存取「范** 本 **** 」控制台。
...
1. 選擇 **建立**。將顯示確認資訊，選擇「 **Open** 」(開啟 [)](#editing-templates-template-authors) 開始編輯模板 **，或選擇「** Done」（完成）返回模板控制台。
...
雖然「頁 **面資訊** 」功能表上的「頁面原則」選 **項可讓您選** 取所需的頁面原則 [](#editing-a-template-structure-template-author):
...
* **原則與屬性（一般）**
   ...
   *功能*
   ...
   >[!NOTE]
   >
   >[實施富格文本編輯器的元件的內容策略](/help/sites-administering/rich-text-editor.md) ，只能針對RTE通過其UI設定提供的選項定義。
