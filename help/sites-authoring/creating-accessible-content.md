...
seo-description: 協助殘疾人存取及使用網路內容
...
在&#x200B;**元資料**&#x200B;標籤的&#x200B;**影像**&#x200B;元件對話方塊中，可使用&#x200B;**替代文字**&#x200B;欄位：

![在觸控最佳化UI中編輯影像元件的對話方塊；顯示「替代文字」欄位。](assets/screen_shot_2018-03-21at165045.png)

AEM預設需要填入&#x200B;**替代文字**&#x200B;欄位。 如果影像是純裝飾性的，而替代文字是無意義的，則可以檢查&#x200B;**Image is decoration**&#x200B;選項。
...
* 說明性照片：這些是人、物體或地方的影像。 想想照片在頁面上的作用；適當的文本等效項可能是[object]*的照片，但可能取決於周圍的文本。*
...
* 驗證碼：驗證碼是一種完全自動化的公共圖靈測試，用於區分電腦和人&#x200B;*。*&#x200B;這是一種用於網頁上的安全檢查，用於區分人類和惡意軟體，但可能造成無障礙障礙。 這些影像需要使用者描述他們看到的內容，才能通過安全性測試。 顯然無法為影像提供替代文字，因此您需要考慮替代的非圖形解決方案。
...
* 可能無法存取作者在&#x200B;*standard*&#x200B;二維、多欄、彩色網頁版面中呈現的資訊
...
AEM可讓您使用適當的HTML元素輕鬆建構網頁。 在RTE中開啟您的頁面內容（文本元件），然後使用&#x200B;**Paraformat**&#x200B;菜單（段落符號）指定適當的結構元素（例如段落、標題等）。
...
* **使用標題：**
   ...
   只要您已啟用RTE的協助功能(請參閱[AEM和協助工具](#AdobeExperienceManagerandAccessibility)),AEM就會提供3層頁面標題。 您可以使用這些項目來識別內容的區段和子區段。標題1是標題的最高級別，標題3是最低級別。系統管理員可以配置系統以允許使用更多標題級別。
...
* 錯誤範例：
   ...
   * 文字：如需2010年秋季夜班的詳細資訊，請按一下<u>此處</u>。
   * 原因：它沒有明確、明確地指明其目的地。

* 範例：

   * 文字：<u>秋季2010</u>夜班 — 詳細資訊。
...
* 如果以英文寫入頁面，`<html>`元素應為：`<html lang = “en-gb”>`
...
![新增項目至選項群組。組標題是「聯繫我」 — 在「標題」欄位中定義。](assets/chlimage_1-209.png)
