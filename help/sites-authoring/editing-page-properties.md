...
* **允許的範本**
   ...
   [定義可用的範本清單](/help/sites-authoring/templates.md#enabling-and-allowing-a-template-template-author) 分支內。

* **驗證需求**

   啟用（或停用）驗證的使用以存取頁面。

   此處可以設定驗證要求以及指定的登錄頁。 頁面的封閉使用者群組定義於 **[權限](/help/sites-authoring/editing-page-properties.md#permissions)** 標籤。

   >[!CAUTION]
   >
   >此 **[權限](/help/sites-authoring/editing-page-properties.md#permissions)** 頁簽允許根據 `granite:AuthenticationRequired` 米辛。 如果使用已棄用的CUG設定來設定頁面權限，則會根據cq:cugEnabled屬性的存在，在下方顯示警告訊息 **驗證需求** 和選項將無法編輯， [權限](/help/sites-authoring/editing-page-properties.md#permissions) 可編輯。
   >
   >
...
* **個人化**
   ...
   選取 [要指定目標範圍的品牌](/help/sites-authoring/personalization.md).
...
>[!NOTE]
>
>資產也提供大量屬性編輯功能。 這非常相似，但有幾點不同。 請參閱 [編輯多個資產的屬性](/help/assets/managing-multiple-assets.md) 以取得詳細資訊。
>
