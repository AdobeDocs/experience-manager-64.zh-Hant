...
1. 新增內 **[!UICONTROL 容片段]** ; 從「元件」瀏 **[!UICONTROL 覽器]** ，或「插入 **[!UICONTROL 新元件」]**。

1. 您可以:

   * 開啟「 **[!UICONTROL Assets]** 」瀏覽器並篩選 **[!UICONTROL 「內容片段]** 」（預設為「影像」）。 然後將所需片段拖曳至元件例項。
   * 選擇內容片段元件，然後從工 **[!UICONTROL 具欄中]** 「配置」。 在對話方塊中，您可以開啟選取對話方塊，以瀏覽並選取所需的 **[!UICONTROL 內容片段]**。

   >[!NOTE]
   >
   >另一種方法是將特定內容片段直接拖曳至頁面上。 這會自動建立關聯的元件（內容片段）。

1. 一開始會顯示 **[!UICONTROL Main]** Element和 **[!UICONTROL Master]** (variation)的內容。 您可以 [視需要選取其他元素和／或變](#selecting-the-element-or-variation) 化。
...
開啟片段的「設 **[!UICONTROL 定]** 」對話方塊，以設定要在目前頁面上使用的片段。 對話方塊可視使用的元件而定。
...
您可以使用元件工具列上的「編輯」圖示，開啟片段來源以 **[!UICONTROL 進行編輯]** （資產）。 這可讓您編輯 [和管理內容片段](/help/assets/content-fragments.md)。
...
將特定內容片段新增至頁面時，片段的每個HTML段落（以及片段的上／下）之間都會有 **[!UICONTROL Drag components]** here預留位置。

這可讓您在中間( [亦即中間內容)](/help/assets/content-fragments.md#in-between-content-when-page-authoring-with-content-fragments) （在任何可用點）新增額外內容，而不需變更根片段。
...
>[!NOTE]
>
>如果您在單一頁面上有多個內容片段，「關聯的 **[!UICONTROL 內容]** 」索引標籤會顯示適合所有片段的資產。

在將含有關聯內容的片段新增至頁面後，會在側面板中開啟新的標&#x200B;**[!UICONTROL 簽(關聯內容]**)。
...
如 [果資產（例如影像）已插入片段本身](/help/assets/content-fragments-variations.md#inserting-assets-into-your-fragment)，則頁面編輯器中編輯這些資產的選項有限。
...
* 在「資產」主控台中建 [立片段後，可發佈片段](/help/assets/content-fragments-managing.md#publishing-and-referencing-a-fragment)。
