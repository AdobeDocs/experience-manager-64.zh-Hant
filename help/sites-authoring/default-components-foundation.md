...
>[!NOTE]
>
>本節說明基礎元件，這些元件已隨AEM提供許多版本，且可在標準AEM安裝中立即使用。 不過，AEM 6.4已棄用許多基礎元件。
>
>Adobe建議運用更現代化且可擴充的[核心元件](https://helpx.adobe.com/experience-manager/core-components/using/authoring.html)。 這些是[We.Retail範例內容](/help/sites-developing/we-retail.md)的一部分，也可以分別安裝[並由您的管理員用於開發](https://helpx.adobe.com/experience-manager/core-components/using/using.html)。
...
支援使用基礎元件，但已由核心元件取代，因為核心元件提供更高的擴充性和彈性。
...
>[!CAUTION]
>此基礎元件已淘汰。 Adobe建議改用[核心元件](https://docs.adobe.com/content/help/zh-Hant/experience-manager-core-components/using/introduction.html)。
...
>[!CAUTION]
>此基礎元件已淘汰。 Adobe建議改用[核心元件影像元件](https://docs.adobe.com/content/help/zh-Hant/experience-manager-core-components/using/components/image.translate.html)。
...
`+` 添加更多條目和上/下按鈕以調整順序。
   ...
   * **排序方式**
      ...
      從下拉式選單中選取`jcr:title`、`jcr:created`、`cq:lastModified`或`cq:template`。
...
>
>如需AEM所使用HTML5功能的詳細資訊，請參閱[支援的平台](/help/release-notes/release-notes.md#supported-platforms)區段。
...
>[!NOTE]
>
>此元件已使用[HTML範本語言(HTL)](https://helpx.adobe.com/experience-manager/htl/user-guide.html)實作。
...
>[!NOTE]
>
>**表**&#x200B;元件基於[RTF編輯器](/help/sites-authoring/rich-text-editor.md),**[Text](#text)**&#x200B;元件亦然。
>
...
>[!NOTE]
>
>**Text**&#x200B;元件以[RTF編輯器](/help/sites-authoring/rich-text-editor.md)為基礎，**[Table](#table)**&#x200B;元件亦然。
>
>建議將&#x200B;**表**&#x200B;元件用於表，但也可以使用&#x200B;**文本**&#x200B;元件來構建表。
...
* **類型/大**
小從下拉清單中選擇小或大。小即生成為影像。 大是作為文本生成的。
...
以下是範例表單。 它由&#x200B;**Form**&#x200B;元件（開頭和結尾）組成，其中兩個&#x200B;**Form Text**&#x200B;欄位用於輸入，一個&#x200B;**General Text**&#x200B;欄位用於導入文本，以及一個&#x200B;**Submit**&#x200B;按鈕。
...
* **進階**
   ...
   * **動**
作類型表單需要動作。動作會定義透過使用者提交的資料（類似於HTML中的action=）觸發以執行的操作。 有些需要對應 
**動作設定**.
