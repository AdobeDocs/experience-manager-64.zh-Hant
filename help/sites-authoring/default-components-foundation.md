...
>
>Adobe建議運用更現代且可擴充的 [核心元件](https://helpx.adobe.com/experience-manager/core-components/using/authoring.html). 這些是 [We.Retail範例內容](/help/sites-developing/we-retail.md) 也可以 [獨立安裝，用於開發](https://helpx.adobe.com/experience-manager/core-components/using/using.html) 由您的管理員進行。
...
>[!CAUTION]
>此基礎元件已淘汰。 Adobe建議使用 [核心元件影像元件](https://experienceleague.adobe.com/docs/experience-manager-core-components/using/components/image.html) 。
...
`+` 添加更多條目和上/下按鈕以調整順序。
   ...
   * **排序方式**
      ...
      選擇 `jcr:title`, `jcr:created`, `cq:lastModified`，或 `cq:template` 從下拉式功能表。
...
>
>請參閱 [支援平台](/help/release-notes/release-notes.md#supported-platforms) 區段，以取得AEM所使用HTML5功能的詳細資訊。
...
>[!NOTE]
>
>此元件已透過 [HTML範本語言(HTL)](https://helpx.adobe.com/experience-manager/htl/user-guide.html).
...
>[!NOTE]
>
>此 **表格** 元件以 [RTF編輯器](/help/sites-authoring/rich-text-editor.md)，亦即 **[文字](#text)** 元件。
>
...
>[!NOTE]
>
>此 **文字** 元件以 [RTF編輯器](/help/sites-authoring/rich-text-editor.md)，亦即 **[表格](#table)** 元件。
>
>建議使用 **表格** 表的元件，但也可使用 **文字** 元件。
...
另請參閱 [設定視訊元件](/help/sites-administering/config-video.md) 用於HTML5元素。
...
以下是範例表單。 由 **表單** 元件（開始和結束），有兩個 **表單文字** 輸入欄位， **一般文字** 用於導入文本的欄位和 **提交** 按鈕。
...
* **進階**
   ...
   * **動作類型**
表單需要動作。 動作會定義使用者提交的資料(類似於HTML中的action=)所觸發的執行操作。 有些需要對應 
**動作設定**.
