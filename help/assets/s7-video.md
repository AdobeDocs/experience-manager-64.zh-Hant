---
title: 影片
description: 了解集中式視訊資產管理，您可在其中上傳Experience Manager資產的視訊，以自動編碼至Dynamic Media Classic。 您也可以直接從Dynamic Media Classic影片Experience Manager資產。 Dynamic Media Classic視訊整合可運用自動裝置和自動頻寬偵測，將最佳化視訊的觸及範圍延伸至所有畫面。
...
Assets提供集中的視訊資產管理功能，您可以將視訊直接上傳至Assets，以自動編碼至Dynamic Media Classic。 您也可以直接從Assets存取Dynamic Media Classic影片，以進行頁面編寫。
...
如需詳細資訊，請參閱[Dynamic Media Classic檔案中的最適化視訊集](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/assets/dynamicmedia/video-profiles.html#dynamicmedia)。
...
## 從AdobeDAM {#uploading-your-master-video}上傳主視訊至Scene7

1. 導覽至CQ DAM Target資料夾，您已在該資料夾中使用Scene7編碼設定檔設定雲端設定。
1. 按一下&#x200B;**[!UICONTROL Upload]**&#x200B;上傳主視訊。 視訊上傳和編碼在DAM更新資產工作流程完成且&#x200B;**[!UICONTROL 發佈至Scene7]**&#x200B;有勾號後完成。

   >[!NOTE]
   >
   >產生視訊縮圖需要一些時間。

   將DAM主視訊拖曳至視訊元件上可存取&#x200B;*all* Scene7編碼的Proxy轉譯以供傳送。
...
>[!NOTE]
>
>S7視訊元件現成可用，使用通用視訊設定檔。 不過，您可以以Experience Manager取得HTML5型視訊播放器。 只要複製現成可用HTML5視訊播放器的內嵌程式碼，並將其放入您的Experience Manager頁面即可。
...
1. 從&#x200B;**[!UICONTROL Configuration Console]**&#x200B;導覽至導覽樹狀結構中的&#x200B;**[!UICONTROL Tools > DAM > Video Profiles]**。
1. 建立Scene7影片設定檔。 在&#x200B;**[!UICONTROL 新……]**&#x200B;下拉式清單，選取&#x200B;**[!UICONTROL 建立頁面]**，然後選取Scene7視訊設定檔範本。 為新視訊設定檔頁面命名，然後按一下「**[!UICONTROL 建立]**」。
...
**[!UICONTROL Foundation Video]**&#x200B;元件必須知道要使用哪些視訊設定檔來建立視訊來源清單。 開啟視訊元件設計對話方塊，並設定使用新視訊設定檔的元件設計。
