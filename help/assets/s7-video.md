---
title: 影片
description: 瞭解集中式視訊資產管理，您可在其中上傳Experience Manager Assets的視訊，以便自動編碼至Dynamic Media Classic。 您也可以直接從Experience Manager Assets存取Dynamic Media Classic影片。 Dynamic Media Classic視訊整合運用自動裝置和自動頻寬偵測，將最佳化視訊的觸及面延伸至所有螢幕。
...
資產提供集中式視訊資產管理，您可以直接將視訊上傳至資產，以自動編碼至Dynamic Media Classic。 您也可以直接從Assets存取Dynamic Media Classic影片，以進行頁面製作。
...
如需詳細資訊，請參閱[動態媒體經典檔案，瞭解最適化視訊集](https://experienceleague.adobe.com/docs/experience-manager-cloud-service/assets/dynamicmedia/video-profiles.html#dynamicmedia)。
...
啟用和設定Dynamic Media Classic整合併不會從現成可用的DAM擷取工作流程自動移除或停用這兩個工作流程步驟。 如果您已在Experience Manager中使用以FFMPEG為基礎的視訊編碼，則您的製作環境可能已安裝FFMPEG。 在此例中，使用DAM擷取的新視訊會編碼兩次：一次是從FFMPEG編碼器，一次是從Dynamic Media Classic整合。

如果您已在AEM和FFMPEG中設定FFMPEG視訊編碼，則可從DAM擷取工作流程中移除兩個FFMPEG工作流程。
...
如果對上述任一問題或兩個問題都回答「是」，則直接將視訊上傳至Adobe DAM。 如果這兩個問題的答案都是「否」，則直接將視訊上傳至Dynamic Media Classic。 每個藍本的工作流程將在下節中說明。

### 如果您要直接將視訊上傳至Adobe DAM {#if-you-are-uploading-your-video-directly-to-adobe-dam}

如果您需要資產的工作流程或版本修訂，請先上傳至Adobe DAM。 建議的工作流程如下：
...
1. 在Dynamic Media Classic中，[設定排程的FTP上傳和編碼至Scene7（系統自動化）](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/upload-publish/uploading-files.html#preparing-your-assets-and-folders-for-uploading)。
1. 在Experience Manager中，存取內容搜尋器的&#x200B;**[!UICONTROL Scene7]**&#x200B;標籤中WCM中的視訊資產。
...
1. 在&#x200B;**[!UICONTROL Cloud Services]**&#x200B;中，導覽至您的&#x200B;**[!UICONTROL Scene7]**&#x200B;組態，然後按一下「編輯&#x200B;**[!UICONTROL a5/>」。]**
...
如果Scene7中的預設集已更新，則必須在Experience Manager中更新視訊的檢視器和編碼預設集。 在這種情況下，請導覽至雲端設定中的Scene7設定，然後按一下「更新檢視器和編碼預設集&#x200B;]**」。**[!UICONTROL 

![chlimage_1-364](assets/chlimage_1-364.png)

## 從Adobe DAM {#uploading-your-master-video}上傳您的主視訊至Scene7
...
1. 按一下「上傳&#x200B;****」上傳主影片。 視訊上傳和編碼在DAM更新資產工作流程完成後完成，且&#x200B;**[!UICONTROL 發佈至Scene7]**&#x200B;有核取標籤。
   ...
   將DAM主視訊拖曳至視訊元件上，可存取&#x200B;*all* Scene7編碼的Proxy轉譯以進行傳送。

## Foundation Video Component與Scene7 Video元件{#foundation-video-component-versus-scene-video-component}

使用Experience Manager時，您可以同時存取Sites中的視訊元件和Scene7視訊元件。 這些元件不能互換。

Scene7視訊元件僅適用於Scene7視訊。 基礎元件可處理從Experience Manager（使用ffmpeg）和Scene7視訊儲存的視訊。
...
>[!NOTE]
>
>S7視訊元件現成可用，使用通用視訊設定檔。 不過，您可以在Experience Manager中取得以HTML5為基礎的視訊播放器。 輕鬆複製現成可用的HTML5視訊播放程式的內嵌代碼，並將它放入您的Experience Manager頁面。

## Experience Manager視訊元件{#aem-video-component}

即使建議使用Scene7視訊元件來檢視Scene7視訊，為了完整性，請搭配使用Scene7視訊和Foundation視訊元件。

### Experience Manager視訊與Scene7視訊比較{#aem-video-and-scene-video-comparison}

下表提供Experience Manager Foundation Video元件與Scene7 Video元件之間支援功能的高階比較：
...
根據Scene7雲端設定中選取的Scene7編碼預設集，建立各種視訊編碼。 Foundation Video元件若要使用它們，必須針對每個選取的Scene7編碼預設集建立視訊描述檔。 此方法可讓視訊元件依此選擇DAM轉譯。

>[!NOTE]
>
>必須啟動新視訊設定檔及對其所做的變更才能發佈。

1. 在Experience Manager中，點選「**[!UICONTROL 工具] > [!UICONTROL 組態控制台]**」。
1. 從&#x200B;**[!UICONTROL 配置控制台]**&#x200B;導航到導航樹中的&#x200B;**[!UICONTROL 工具> DAM >視頻配置檔案]**。
1. 建立Scene7視訊設定檔。 在&#x200B;**[!UICONTROL 新……]**&#x200B;下拉式清單，選取&#x200B;**[!UICONTROL 建立頁面]**，然後選取Scene7視訊設定檔範本。 為新視訊設定檔頁面指定名稱，然後按一下「建立&#x200B;**[!UICONTROL 」。]**
