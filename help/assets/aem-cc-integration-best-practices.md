...
Adobe Experience Manager資產是數位資產管理(DAM)解決方案，可與Adobe Creative Cloud整合，以協助DAM使用者與創意團隊合作，簡化內容建立程式中的協作。
...
>[!NOTE]
>
>本指AEM南不再提供Creative Cloud資料夾共用功能。 Adobe建議使用較新的功能，例如[Adobe資產連結](https://helpx.adobe.com/tw/enterprise/using/adobe-asset-link.html)或[案頭應用程式AEM](https://experienceleague.adobe.com/docs/experience-manager-desktop-app/using/introduction.html)，讓創意使用者存取中管理的資產AEM。
...
|---|---|---|
| 創意使用者可從AEM中發現資產、開啟和使用資產、編輯和上傳變AEM更至，以及將新檔案上傳至中，毋需離AEM開Creative Cloud應用程式。 | [Adobe資產連結](https://helpx.adobe.com/enterprise/using/adobe-asset-link.html) | Photoshop、Illustrator和InDesign |
| 商業使用者可簡化開啟和使用資產、編輯和上傳變AEM更至，以及從案頭環境上AEM傳新檔案的程式。 他們使用一般整合來開啟原生案頭應用程式中的任何資產類型，包括非Adobe類型。 | [AEM 桌面應用程式](https://experienceleague.adobe.com/docs/experience-manager-desktop-app/using/using.html) | Win和AEMMac案頭版案頭應用程式 |
...
本文主要針對協作需求的前兩個方面。資產規模分配和採購作為一個使用案例被簡要提及。針對這些需求解決方案，請考慮Adobe品牌入口網站或資產共用公域。應根據具體要求審查[Experience Manager資產](/help/assets/managing-assets-touch-ui.md)的替代解決方案，如[品牌入口](https://helpx.adobe.com/tw/experience-manager/brand-portal/user-guide.html)、可以基於[資產共用](https://adobe-marketing-cloud.github.io/asset-share-commons/)元件、[連結共用](/help/assets/link-sharing.md)構建的解決方案。
...
* (3)使用案頭應用AEM程式上傳資料夾功能

為支援資產散發使用案例，應考慮其他解決方案：

* [Brand ](https://helpx.adobe.com/experience-manager/brand-portal/user-guide.html) Portal，提供可設定、SaaS附加元件至AEM Assets以發佈資產。
* 自訂解決方案是根據[資產共用共用共用](https://adobe-marketing-cloud.github.io/asset-share-commons/)程式碼庫建立。
* AEM[連結共用](/help/assets/link-sharing.md)以使用連結臨機共用資產。
* [AEM Assets網](/help/assets/managing-assets-touch-ui.md) 路介面與存取控制設定保護的外部AEM使用者區域，以及必要的IT/網路組態調整，讓這些外部使用者存取AEM。
...
這是Experience Manager與Creative Cloud整合最佳實務的簡要摘要。 閱讀本檔案的其餘部分，以詳細瞭解這些內容。
...
通常，內部創意團隊或內部網路的廣告公司／創意專業人員都可存取DAM例項，包括登AEM入。

在這種情況下，案頭應AEM用程式可協助您輕鬆存取最終／核准的資產，並讓您將創意就緒的資產儲存至DAM。

#### 無法存取DAM的創意使用者{#creative-users-without-access-to-dam}

無法直接存取DAM例項的外部機構和自由工作者可能需要存取已核准的資產，或想要將其新設計新增至DAM。

在這種情況下，您可以利用AEM/Creative Cloud整合來改善工作流程。 先決條件是創意使用者必須擁有Adobe ID，並擁有儲存服務的Creative Cloud帳戶。

使用下列策略來存取最終／核准的資產：

* 若要提供大量資產的存取權：使用[AEM Assets品牌入口網站](https://experienceleague.adobe.com/docs/experience-manager-brand-portal/using/home.html?lang=en)或客戶在發佈基礎架構上實作[資產共用&lt;a3/AEM>](assets-finder-editor.md)

* 若要存取一些資產：除了「AEMAEM Assets品牌入口網站」或「資產共用」外，您還可使用與「Adobe Creative Cloud」共用資料夾。 請注意，本整合有某些限制，本文將詳細說明。

### 使用案例{#use-cases}

下列使用案例說明DAM與設計人員案頭之間的各種工作流程類型。

#### 使用DAM {#creating-new-designs-using-assets-from-dam}的資產建立新設計
...
* 對WIP檔案使用專用的儲存區／系統，如「Adobe Creative Cloud資產」同步資料夾：與DAM使用者無關的頻繁更新，最好由專用系統處理，而非來自AEM Assets。 WIP資產可以使用Adobe Creative Cloud案頭應用程式與本地磁碟同步，並保存在本地儲存中，依此類推。
* 針對上傳至DAM的最終資產和資產使用個別的資料夾／共用：為避免疑義，最終資產應有自己的對應／共用資料夾（上方的「最終」範例），而要上傳回DAM的資產應有自己的資產（「創意就緒」）

#### 變更DAM {#changing-existing-assets-managed-in-dam}中管理的現有資產

在某些情況下，DAM中的資產可能需要變更。 範例包括：

* 在AEM Assets進行的審查和批准要求更改資產
* 現有最終資產的主要更新
* 快速編輯現有檔案（尤其是在最終核准之前）

在這種情況下，桌AEM面應用程式提供最簡單的方式來執行這些作業。
...
* **1：將** 資產從DAM共用至案頭，或直接在所選應用程式(例如Adobe Photoshop等)的案頭上開啟。建議簽出以鎖定檔案。
* **2：小** 幅更新：編輯檔案並儲存變更。
* 步驟2的替代流程

   * **答：主** 要更新：如果檔案需要詳細的更改集，則應間歇性地保存並複製到WIP資料夾／區域。
   * **B：在** WIP資料夾中繼續對檔案進行工作。儲存的變更不會同步至DAM中的版本
   * **C：更** 新完成後，檔案會複製回或儲存至對應的資料夾

* **3：資** 產更新會反映在DAM中。簽入資產以解除鎖定。
* **4:** 資產投入生產。

以下是有關如何在整個流程中管理資產的一些一般建議：

* 請避免直接儲存您從案頭應用程式映射的網路共用中開啟的檔案，AEM除非您對檔案所做的變更很小。
...
* 若要上傳大型／階層式資料夾，請使AEM用案頭應用程式，此應用程式提供[資料夾上傳](https://helpx.adobe.com/experience-manager/desktop-app/aem-desktop-app.html#bulkupload)功能。 您也可以上傳階層式資料夾結構。 資產會在背景上傳，因此不會系結至網頁瀏覽器作業
* 如果您要從單一檔案夾上傳幾個檔案，請直接從案頭拖曳檔案至網頁UI，或使用AEM Assets網頁UI中的「建立」選項。
...
如果您使用「網路檔案分享」來管理數位資產，則只使用案頭應用程式所映射AEM的網路分享就可視為方便的替代方式。 從網路檔案分享轉換時，請記住AEM,Web UI提供豐富的數位資產管理功能集，遠超網路分享（搜尋、系列、中繼資料、協作、預覽等）的可能，而案頭應用程式則提供方便的連結，讓伺服器端的DAM存放庫與桌上型電腦的作品連結。
...
要規劃並執行資產從現有系統遷移到新系統或遷移儲存在伺服器上的大量資產，請參閱[遷移指南](/help/assets/assets-migration-guide.md)。 案頭應AEM用程AEM式與Creative Cloud整合不支援此類遷移。 由於需要吸收的資產數量龐大，以及對元資料映射、轉換和提取的額外要求，因此應使用不同的工具和方法來處理遷移。

>[!MORELIKETHIS]
>
>* [Adobe資產連結](https://helpx.adobe.com/in/enterprise/admin-guide.html/in/enterprise/using/adobe-asset-link.ug.html)
>* [案頭應用程式最佳範例](https://experienceleague.adobe.com/docs/experience-manager-desktop-app/using/archive/best-practices-for-v1.html)
...
>* [AEMAdobe Stock整合](aem-assets-adobe-stock.md)
