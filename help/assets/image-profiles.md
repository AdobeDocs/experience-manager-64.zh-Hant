...
You use **Unsharp Mask** to fine-tune a sharpening filter effect on the final downsampled image. 您可以控制效果的強度、效果半徑 (以像素計量)，以及將忽略的對比度臨界值。此效果使用的選項與Adobe Photoshop的「遮色片銳利化」濾鏡相同。
...
銳利化影像中會 [說明銳利化](/help/assets/assets/s7_sharpening_images.pdf)。
...
將智慧型裁切新增至現有的影像描述檔時，如果您想要為資產儲存庫中的現有資產產生裁切，則需要重新觸發 [](assets-workflow.md) DAM更新資產工作流程。
...
1. 執行下列任一項作業：
   ...
   * 導覽至 **https://&lt;AEM伺服器>/mnt/overlay/dam/gui/content/assets/foldersharewizard.html/content/dam** ，然後套用適當的描述檔並點選「 **儲存」**。
