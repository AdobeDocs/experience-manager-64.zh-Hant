...
動態媒體——混合媒體需要啟用並設定使用。 根據您的使用案例，動態媒體有數種支援 [的設定](#supported-dynamic-media-configurations)。
...
[動態媒體](https://www.adobe.com/solutions/web-experience-management/dynamic-media.html) ，預設為停用。 若要運用動態媒體功能，您必須使用動態媒體執行模式來啟用動態媒體 **[!UICONTROL ，例如，]** publish **** 執行模式。 在啟用之前，請務必檢閱技 [術需求](/help/sites-deploying/technical-requirements.md#requirements-for-aem-dynamic-media-add-on)。

>[!NOTE]
>
>透過執行模式啟用動態媒體會取代AEM 6.1和AEM 6.0中的功能，您可透過將 **[!UICONTROL dynamicMediaEnabled標幟設為]** true ****，來啟用動態媒體。 此標幟在AEM 6.2和更新版本中沒有功能。 此外，您不需要重新啟動快速入門，就能啟用動態媒體。

透過啟用動態媒體，動態媒體功能將可在UI中使用，而且每個上傳的影像資產都會收到一個轉譯，用於快速傳遞動態影像轉譯。 `cqdam.pyramid.tiff` 這些PTIFF具有顯著的優點，包括(1)僅能管理單一主影像並即時產生無限轉譯，毋需額外儲存空間，以及(2)能夠使用互動式視覺效果，例如縮放、平移、回轉等。
...
1. 在命令行上，啟動快速啟動時，請執行以下操作：
   ...
   * 在啟 **[!UICONTROL 動jar檔案時，將-r dynamicmedia]** 添加到命令行的末尾。
...
1. 請求 `http://localhost:4502/is/image` 並確定影像伺服器現在正在執行。

   >[!NOTE]
   >
   >若要疑難排解動態媒體的問題，請參閱 **[!UICONTROL crx-quickstart/logs/目錄中的下列記錄]** :
   >
...
要在啟用動態媒體後禁用它，請刪除 **[!UICONTROL -r dynamicmedia]** run mode（動態媒體運行模式）標誌。

**要在動態媒體啟用後禁用它**:

1. 在命令行中，啟動快速啟動時，可以執行下列任一操作：

   * 啟動JAR `-r dynamicmedia` 檔案時，不要添加到命令行。

   ```shell
   java -Xmx4096m -Doak.queryLimitInMemory=500000 -Doak.queryLimitReads=500000 -jar cq-quickstart-6.4.0.jar
   ```

1. Request `http://localhost:4502/is/image`. 您會收到動態媒體已停用的訊息。

   >[!NOTE]
   停用「動態媒體」執行模式後，會自動跳過產生轉譯 `qdam.pyramid.tiff` 的工作流程步驟。 這也會停用動態轉譯支援和其他動態媒體功能。
   另請注意，在設定AEM伺服器後停用「動態媒體」執行模式時，該執行模式下上傳的所有資產現在都無效。

## （可選）將動態媒體預設集和組態從6.3移轉至6.4，零停機時間 {#optional-migrating-dynamic-media-presets-and-configurations-from-to-zero-downtime}
...
超過最大記憶體限制的映像將被拒絕。
若要變更PTIFF建立的記憶體限制，請導覽至「工具>作業>網頁主控台> Adobe CQ Scene7 PTiffManager **[!UICONTROL 」並變]**`maxMemory` 更值。
...
1. 在「編輯動態媒體複製的使用者設定」頁面中，點選「 **[!UICONTROL Keystore]** 」標籤，然後點 **[!UICONTROL 選「建立KeyStore]**」。
...
1. 在「設定KeyStore訪問密碼」對話框中輸 **[!UICONTROL 入密碼並確認密碼]** 。
   ...
   >[!NOTE]
   記住您輸入的密碼。 以後配置複製代理時，您需要再次 **[!UICONTROL 輸入它]** 。
...
您也可以執行下列任一操作來檢查：
* 檢查複製日誌以確保資產已複製。
* 發佈影像。 點選影像，然後在下拉 **[!UICONTROL 式選單中]** ，選取「檢視器」。 選取檢視器預設集，然後點選 **[!UICONTROL URL]**，並複製URL並貼在瀏覽器中，以確認您可以看到影像。
...
1. 導覽至「使 **[!UICONTROL 用者管理]** 」頁：

   `localhost:4502/libs/granite/security/content/useradmin.html`
1. 在「使 **[!UICONTROL 用者管理]** 」頁面上，導覽至 **[!UICONTROL 動態媒體複製使用者]** ，然後點選以開啟。
1. 點選「 **[!UICONTROL KeyStore]** 」標籤。 如果出 **[!UICONTROL 現「Create KeyStore]** 」按鈕，則需要重做「Setting up Authentication arel [(以前設定驗證](#setting-up-authentication) )」下的步驟。
1. 如果必須重做 **[!UICONTROL KeyStore]** ，則可能還需要 [](config-dynamic.md#configuring-the-replication-agent) 再次配置複製代理。
...
1. 導航到 **[!UICONTROL s7delivery Replication Agent節點]** 。
...
* 已新增影像預設集。
* 在「 **雲端服務」下設定「動態媒體設定」(6.3****[!UICONTROL 之前版本)]**。 此測試需要影像服務URL
...
1. 在「動 **[!UICONTROL 態媒體設定瀏覽器]** 」頁面的左窗格中，選取全域 ****，然後點選「 **[!UICONTROL 建立]**」。
1. 在「建 **[!UICONTROL 立動態媒體設定]** 」對話方塊的「 **[!UICONTROL 標題]** 」欄位中，輸入標題。
1. 如果您要設定視訊的動態媒體，
...
**使用時機：** 在您設定動態媒 **[!UICONTROL 體設定（6.3之前版本）時]**，會啟動許多功能，包括視訊報告。 此設定會在地區性Analytics公司中建立報表套裝。 如果您設定多個「作者」節點，您會為每個節點建立個別的報表套裝。 因此，報告資料在安裝之間不一致。 此外，如果每個「作者」節點都參照相同的「混合發佈」伺服器，則上次「作者」安裝會變更所有視訊報表的目標報表套裝。 此問題會將Analytics系統的報表套裝過多過載。

**開始使用：** 完成下列三項工作，以設定視訊報告。

1. 在第一 [!DNL Video Analytics] 個「作者」節點上 **[!UICONTROL 設定「動態媒體設定（6.3之前版本）]** 」後，建立預設套件。 此初始任務很重要，因為它允許新配置繼續使用相同的報表套裝。
1. 在配置 [!DNL Video Analytics] 動態媒體配置(6.3 ***版)之前，將預設集套件安*** 裝到任何新的「作者 ****** 」節點。

1. 驗證並調試軟體包安裝。

### 在配置第 [!DNL Video Analytics] 一個「作者」節點後建立預設包 {#creating-a-video-analytics-preset-package-after-configuring-the-first-author-node}

完成此任務後，將會有一個包含預設集的包文 [!DNL Video Analytics] 件。 這些預設集包含報表套裝、追蹤伺服器、追蹤命名空間和Marketing Cloud組織ID（若有的話）。

1. 如果您尚未這麼做，請設定 **[!UICONTROL 動態媒體設定（6.3之前版本）]**。
1. （可選）檢視並復 **[!UICONTROL 制報表套裝ID]** （您必須擁有JCR的存取權）。 雖然不需要 **[!UICONTROL 報表套裝ID]** ，但它可讓驗證更輕鬆。
1. 使用「包管理器」( **[!UICONTROL Package Manager)建立包]**。
1. 編輯套件以包含篩選。

   在AEM中： `/conf/global/settings/dam/dm/presets/analytics/jcr:content/userdata`

1. 建立套件。
1. 下載或共用預 [!DNL Video Analytics] 設集套件，以便與後續的新「作者」節點共用。

### 在配置其 [!DNL Video Analytics] 他「作者」節點之前安裝預設包 {#installing-the-video-analytics-preset-package-before-you-configure-additional-author-nodes}

請務必先完成此任務 _，然後再_ 配 **[!UICONTROL 置動態媒體配置（6.3之前版本）]**。 若無法這麼做，則會建立另一個未使用的報表套裝。 此外，即使視訊報告仍能正常運作，資料收集仍無法最佳化。

請確定第一個「作 [!DNL Video Analytics] 者」節點的預設套件可在新的「作者」節點上存取。

1. 將您建立 [!DNL Video Analytics] 的預設包上載到「包管 **[!UICONTROL 理器」]**。
1. 安裝預 [!DNL Video Analytics] 設套件。
...
1. 執行下列任一操作以驗證軟體包安裝，並在必要時調試軟體包安裝：
   ...
   * **使用[!DNL Video Analytics]JCR檢查預設**&#x200B;要通過JCR [!DNL Video Analytics] 檢查預設，您必須具有 **[!UICONTROL CRXDE Lite的訪問權]**。

      AEM —— 在 **[!UICONTROL CRXDE Lite]**，導覽至 `/conf/global/settings/dam/dm/presets/analytics/jcr:content/userdata  `

      就是 `http://localhost:4502/crx/de/index.jsp#/conf/global/settings/dam/dm/presets/analytics/jcr%3Acontent/userdata`

      如果您無權在「作者」節點上存 **[!UICONTROL 取CRXDE Lite]** ，則可以透過「發佈」伺服器檢查預設集。

   * **透過影像[!DNL Video Analytics]伺服器檢查預設集**

      您可以直接透過 [!DNL Video Analytics] 影像伺服器要求來驗證預設 `req=userdata` 集。

      例如，若要查看「作 [!DNL Video Analytics] 者」節點上的預設集，您可提出下列要求：
   ...
   * **透過AEM[!DNL Video Analytics]中的「視訊報告」工具檢查預設集**
   ...
   如果在您設定動態媒體設定（6.3之前）服務之前執行 **[!UICONTROL 視訊報告，也會顯示此錯誤]** 。
...
* 不先安裝預 [!DNL Video Analytics] 設集套件會造成建立新報表套裝。
* 從AEM 6.3升級至AEM 6.4或AEM 6.4.1，然後設定 **[!UICONTROL Dynamic Media Configuration(Pre 6.3)]**，仍會建立報表套裝。 此問題已知且已修正，適用於AEM 6.4.2。

### 關於預設 [!DNL Video Analytics] 集 {#about-the-video-analytics-preset}

預設 [!DNL Video Analytics] 集（有時稱為分析預設集）會儲存在動態媒體的檢視器預設集旁邊。 它基本上與檢視器預設集相同，但包含用於設定AppMeasurement和視訊心率報告的資訊。
...
1. 在AEM中，導覽至 **[!UICONTROL CRXDE Lite中的下列位置]** （需要管理員權限）:
...
若要使用檢視器預設集來傳送資產，您必須複製／發佈檢視器預設集。 (All viewer presets must be activated _and_ replicated to obtain the URL or embed code for an asset.) 如需詳 [細資訊，請參閱Publishing Viewer](managing-viewer-presets.md#publishing-viewer-presets) Presets。
...
在非動態媒體部署中，您會將 _AEM作者環境_ （包括影像和視訊）的所有資產複製至AEM發佈節點。 此工作流程是必要的，因為AEM發佈伺服器也會傳送資產。
...
這些篩選條件可讓您排除 _資產_ ，使其不複製至AEM發佈節點。

### 使用預設資產篩選器進行複製 {#using-default-asset-filters-for-replication}

如果您使用Dynamic Media for 1)在製作中使用影像 __ ，或是2)在影像和視訊中使用，則可使用我們依現狀提供的預設濾鏡。 下列篩選器預設為作用中：
...
### 在非生產部署中為映像設定資產篩選器 {#setting-up-asset-filters-for-imaging-in-non-production-deployments}
...
1. 找 [!UICONTROL `damRenditionFilters`]到，按一下右鍵並選擇 **[!UICONTROL 貼上]**。
...
如果您在伺服器上使用多個篩選器（例如，一個要發佈的複製篩選器和s7delivery的第二個篩選器），則需要確保這兩個篩選器在 **jcr:content** 節點中為其指派不同的 **[!UICONTROL userId]** 。 請參閱下列影像：
...
1. 要定義篩選器的Mime類型，可以按如下方式查找Mime類型：
   ...
   在左側邊欄中，展 **[!UICONTROL 開內容> dam > &lt;`locate_your_asset`> > jcr:content >中繼資料]**，然後在表格中尋找 `dc:format`。
...
1. 在 **[!UICONTROL Adobe Experience Manager Web Console的「設定」頁面上]** ，點選 **[!UICONTROL OSGi > Configuration]** ，以列出目前在AEM中執行的所有搭售。
...
1. 在搭售清單中，點選 **[!UICONTROL Adobe CQ Scene7 ImageServer右側的「編]**&#x200B;輯」圖示 **** 。
...
1. 點選「 **[!UICONTROL 儲存]**」。
1. 在搭售清單中，點選 **[!UICONTROL Adobe CQ Scene7 PlatformServer右側的「編]**&#x200B;輯」圖示 **** 。
1. 在 **[!UICONTROL Adobe CQ Scene7 PlatformServer]** 對話方塊中，設定下列預設值選項：
   ...
   | **[!UICONTROL 快取根]** | 快取 | 指向響應快取資料夾的一個或多個路徑。 相對路徑會針對內部s7imaging bundle資料夾進行解析。 |
...
請確定您點選「 **[!UICONTROL Access Control]** 」（存取控制）標籤(位於「 **[!UICONTROL Properties]** 」（屬性）標籤右側)，然後將每個人和動態媒體複製使用者的存取控制權限 `jcr:read` 設定為。
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>bkgcolor</td> 
   <td>FFFFFF</td> 
   <td><p>預設背景顏色。 RGB值，用於填滿不含實際影像資料之回覆影像的任何區域。</p> <p>另請參 <a href="https://marketing.adobe.com/resources/help/en_US/s7/is_ir_api/is_api/image_catalog/r_bkgcolor.html">閱「影像服務</a> API」中的BkgColor。</p> </td> 
  </tr> 
  <tr> 
   <td>defaultpix</td> 
   <td>300,300</td> 
   <td><p>預設檢視大小。 如果請求未明確使用wid=、hei=或scl=指定檢視大小，伺服器會限制回覆影像不大於此寬度和高度。</p> <p>指定為兩個整數，0或更大，以逗號分隔。 寬度和高度（以像素為單位）。 其中一個或兩個值都可設為0，以保持不受約束。 不適用於巢狀／內嵌請求。</p> <p>另請參 <a href="https://marketing.adobe.com/resources/help/en_US/s7/is_ir_api/is_api/image_catalog/r_defaultpix.html">閱Image Serving API中的</a> DefaultPix。</p> <p>不過，通常您使用檢視器預設集或影像預設集來傳送資產。 Defaultpix僅適用於未使用檢視器預設集或影像預設集的資產。</p> </td> 
  </tr> 
  <tr> 
   <td>defaultthumbpix</td> 
   <td>100,100</td> 
   <td><p>預設縮圖大小。 用於縮圖請求(req=tmb)，而非屬性：:DefaultPix。</p> <p>如果縮圖要求(req=tmb)未明確指定大小，而未明確使用wid=、hei=或scl=指定檢視大小，則伺服器會限制回覆影像不大於此寬度和高度。</p> <p>指定為兩個整數，0或更大，以逗號分隔。 寬度和高度（以像素為單位）。 其中一個或兩個值都可設為0，以保持不受約束。 </p> <p>不適用於巢狀／內嵌請求。</p> <p>另請參 <a href="https://marketing.adobe.com/resources/help/en_US/s7/is_ir_api/is_api/image_catalog/r_defaultthumbpix.html">閱Image Serving API中的</a> DefaultThumbPix。 </p> </td> 
  </tr> 
  <tr> 
   <td>過期</td> 
   <td>36000000</td> 
  ...
  <tr> 
   <td>jpegquality</td> 
   <td>80</td> 
   <td><p>預設JPEG編碼屬性。 指定JPEG回覆影像的預設屬性。</p> <p>整數和標幟，以逗號分隔。 第一個值在1.100範圍內，並定義品質。 第二個值可以是0表示正常行為，或者1表示禁用JPEG編碼器通常採用的RGB色度下採樣。</p> <p>另請參 <a href="https://marketing.adobe.com/resources/help/en_US/s7/is_ir_api/is_api/image_catalog/r_jpegquality.html">閱「影像伺服</a> API」中的JpegQuality。</p> </td> 
  </tr> 
  <tr> 
   <td>maxpix</td> 
   <td>2000,2000</td> 
   <td><p>回覆影像大小限制。 傳回給用戶端的回覆影像寬度和高度上限。</p> <p>如果請求導致寬度或高度大於屬性：:MaxPix的回覆影像，則伺服器會傳回錯誤。</p> <p>另請參 <a href="https://marketing.adobe.com/resources/help/en_US/s7/is_ir_api/is_api/image_catalog/r_maxpix.html">閱Image Serving</a> API中的MaxPix。</p> </td> 
  </tr> 
  <tr> 
   <td>resmode</td> 
   <td>SHARP2</td> 
   <td><p>預設重新取樣模式。 指定用於縮放影像資料的預設重新採樣和插值屬性。</p> <p>在請求中未指定resMode=時使用。</p> <p>允許的值包括BILIN、BICUB或SHARP2。</p> <p>列舉。 若為bilin，則設為2；若為bicub，則設為3；若為sharp2內插模式，則設為4。 使用sharp2取得最佳效果。</p> <p>另請參 <a href="https://marketing.adobe.com/resources/help/en_US/s7/is_ir_api/is_api/image_catalog/r_is_cat_resmode.html">閱Image Serving API</a> 中的ResMode。</p> </td> 
  </tr> 
  <tr> 
   <td>解析度</td> 
   <td>72</td> 
   <td><p>預設物件解析度。 提供預設物件解析度，以防特定目錄記錄未包含有效的目錄：:Resolution值。</p> <p>實數，大於0。 通常以每英吋像素表示，但也可以以其他單位表示，例如每米像素。</p> <p>另請參 <a href="https://marketing.adobe.com/resources/help/en_US/s7/is_ir_api/is_api/image_catalog/r_resolution.html">閱</a> Image Serving API中的解析度。</p> </td> 
  </tr> 
  <tr> 
   <td>thumbnaitime</td> 
   <td>1%,11%,21%,31%,41%,51%,61%,71%,81%,91%</td> 
   <td>這些值代表視訊播放時間的快照，並會傳遞 <a href="https://encoding.com/">至encoding.com</a>。 如需詳 <a href="/help/assets/video.md#about-video-thumbnails">細資訊，請參閱</a> 「關於視訊縮圖」。</td> 
  </tr> 
 </tbody> 
</table>
...
進階使用案例可使用手動設定 **[!UICONTROL icc=modifier]** 來明確選取輸出色彩描述檔：
...
1. 在「工 **[!UICONTROL 具>一般> CRXDE Lite]**」中，導覽 `/conf/global/settings/dam/dm/imageserver/configuration/settings` 至包含預設Adobe色彩描述檔。
...
1. 通過滾動到「屬性」( **[!UICONTROL Properties]** )頁籤的底部並手動輸入屬性名稱、類型和值，添加顏色校正屬性，這些在下表中有說明。 輸入值後，點選「新 **[!UICONTROL 增]** 」，然 **[!UICONTROL 後「全部儲存]** 」以儲存值。
   ...
   色彩校正屬性在「色彩校正屬 **[!UICONTROL 性」(Color Corrections Properties]** )表格中說明。 可指派給色彩校正屬性的值位於「色彩描述檔」 **[!UICONTROL 表格中]** 。

   例如，Name **[!UICONTROL , add]**, select `iccprofilecmyk`Type **[!UICONTROL , add]** and `String``WebCoated`**** as Value Jorned. 點選 **[!UICONTROL 「新增]**」，然後 **[!UICONTROL 點選「全部儲存]** 」以儲存您的值。
...
例如，您可將 **[!UICONTROL iccprofilergb設為]** , `sRGB`將 **[!UICONTROL iccprofilecmyk]** 設為 `WebCoated`。 這麼做會執行下列動作：

* 啟用RGB和CMYK影像的色彩校正。
* 沒有色彩描述檔的RGB影像會假設在色域 `sRGB` 中。
* 沒有色彩描述檔的CMYK影像會假設在色域 `WebCoated` 中。
* 傳回RGB輸出的動態轉譯，在色域中傳 `sRGB` 回它。
* 傳回CMYK輸出的動態轉譯，在色域中傳 `WebCoated` 回它。
