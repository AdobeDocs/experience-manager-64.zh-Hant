...
description: 診斷Dynamic Media-Scene7運行模式。
...
* 已先AEM安裝任何6.4累積修復包(CFP) *先* 任何可用的Dynamic Media功能包。
* 已安裝可選功能包18912。

   此可選功能包用於FTP支援，或者您要將資產從Dynamic Media Classic遷移到Dynamic Media。

* 導航到Cloud Services用戶介面並確認已設定帳戶顯示在 **[!UICONTROL 可用配置]**。
* 確保 **[!UICONTROL Dynamic Media資產激活（場景7）]** 已啟用複製代理。

   此複製代理位於 **[!UICONTROL 代理]** 作者。
...
可以CRXDE Lite查看以下資產屬性，以確認資產從Dynamic Media成功AEM同步：

| **屬性** | **範例** | **說明** |
|---|---|---|
| `<object_node>/jcr:content/metadata/dam:scene7ID` | `a|364266` | 節點連結到Dynamic Media的一般指示。 |
| `<object_node>/jcr:content/metadata/dam:scene7FileStatus` | **[!UICONTROL 發佈完成]** 錯誤文本 | 資產上載到Dynamic Media的狀態。 |
| `<object_node>/jcr:content/metadata/dam:scene7File` | `myCompany/myAssetID` | 必須填充以生成Dynamic Media遠程資產的URL。 |
| `<object_node>/jcr:content/dam:lastSyncStatus` | `success` 或 `failed:<error text>` | 集（旋轉集、影像集等）、影像預設、查看器預設、資產的影像映射更新或已編輯的影像的同步狀態。 |

### 同步記錄 {#synchronization-logging}

同步錯誤和問題已登錄 `error.log` (伺服器AEM目錄) `/crx-quickstart/logs/`)。 可以使用足夠的日誌記錄來確定大多數問題的根本原因，但您可以將日誌記錄增加到 `com.adobe.cq.dam.ips` 通過Sling控制台進行打包([http://localhost:4502/system/console/slinglog](http://localhost:4502/system/console/slinglog))以收集更多資訊。
...
* 如果缺少上面的元資料值，則需要在移動、複製或刪除操作之前重新上載資產。

### 版本控制項 {#version-control}

替換現有Dynamic Media資產（同名和地點）時，您可以選擇保留兩個資產或替換或建立版本：

* 保留兩者都將為已發佈的資產URL建立一個具有唯一名稱的新資產。 比如說， **[!UICONTROL 影像.jpg]** 是原始資產 **[!UICONTROL 影像1.jpg]** 是新上載的資產。
...
<table> 
 <tbody> 
  ...
  <tr> 
   ...
   <td> 
    <ol> 
     <li><p>轉至CRX/DE:</p> 
      <ul> 
       <li>檢查JCR中是否有預設 <code>/etc/dam/presets/viewer/&lt;preset&gt; has lastReplicationAction</code> 定義。 請注意，如果從6.x升級到AEM6.4並選擇退出遷移，則此位置適用。 否則，地點是 <code>/conf/global/settings/dam/dm/presets/viewer</code>。</li> 
       <li>檢查以確保JCR中的資產 <code>dam:scene7FileStatus</code><strong> </strong>在元資料下顯示為 <code>PublishComplete</code>。</li> 
      </ul> </li> 
    </ol> </td> 
   <td><p>刷新頁面/導航到另一頁並返回（需要重新編譯側軌JSP）</p> <p>如果這行不通：</p> 
    <ul> 
     <li>發佈資產。</li> 
     <li>重新上載資產並發佈。</li> 
    </ul> </td> 
  ...
  <tr> 
   ...
   <td><p>首先按一下「資產選擇器」中的另一個資料夾，然後返回以選擇資產。</p> </td> 
  </tr> 
  <tr> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> 
    <ol> 
     ...
     <li>等待視頻完成處理。</li> 
     <li>如果重新載入視頻，請確保Dynamic Media編碼視頻工作流未運行。<br /> </li> 
     <li>重新上傳視頻。</li> 
    </ol> </td> 
  </tr> 
  <tr> 
   ...
   <td> 
    <ul> 
     <li>檢查運行模式是否 <span class="kbd">動態媒體_場景7</span>。</li> 
     <li>檢查是否配置了Dynamic Media雲服務。</li> 
     <li>檢查視頻配置檔案是否與上載資料夾關聯。</li> 
    </ul> </td> 
   <td> 
    <ol> 
     <li>檢查AEM實例 <span class="kbd">-r動態媒體場景7</span></li> 
     <li>檢查Cloud Services下的Dynamic Media配置是否正確設定。</li> 
  ...
  <tr> 
   ...
   <td><p>要確定視頻編碼是否仍在進行中或是否已進入失敗狀態：</p> 
    <ul> 
     <li>檢查視頻狀態 <code>http://localhost:4502/crx/de/index.jsp#/content/dam/folder/videomp4/jcr%3Acontent</code> &gt; <span class="kbd">dam:assetState</span></li> 
     <li>從工作流控制台監視視頻 <code>http://localhost:4502/libs/cq/workflow/content/console.html</code> &gt;實例、存檔、失敗頁籤。</li> 
    </ul> </td> 
   <td> </td> 
  </tr> 
  <tr> 
