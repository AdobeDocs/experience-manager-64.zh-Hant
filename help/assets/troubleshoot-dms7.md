...
description: 疑難排解Dynamic Media - Scene7執行模式。
...
* 已先安裝任何AEM 6.4 Cumulative Fix Pack(CFP) *befor* 任何可用的Dynamic Media Feature Pack。
...
* 確保 **[!UICONTROL Dynamic Media Asset Activation(scene7)]** 復寫代理已啟用。

   此複製代理位於 **[!UICONTROL 代理]** 在作者上。
...
| `<object_node>/jcr:content/metadata/dam:scene7FileStatus` | **[!UICONTROL PublishComplete]** 錯誤文字 | 資產上傳至Dynamic Media的狀態。 |
...
已登錄同步錯誤和問題 `error.log` (AEM伺服器目錄) `/crx-quickstart/logs/`)。 有足夠的記錄可供判斷導致大部分問題的根本原因，但您可以將 `com.adobe.cq.dam.ips` 封裝([http://localhost:4502/system/console/slinglog](http://localhost:4502/system/console/slinglog))，以收集詳細資訊。
...
* 同時保留兩者將為已發佈的資產URL建立一個具有唯一名稱的新資產。 例如， **[!UICONTROL image.jpg]** 是原始資產， **[!UICONTROL image1.jpg]** 是新上傳的資產。
  ...
  <tr> 
   ...
   <td> 
    <ol> 
     <li><p>前往CRX/DE:</p> 
      <ul> 
       <li>檢查JCR中的預設集 <code>/etc/dam/presets/viewer/&lt;preset&gt; has lastReplicationAction</code> 已定義。 請注意，如果您從AEM 6.x升級至6.4，並選擇退出移轉，此位置即適用。 否則，位置是 <code>/conf/global/settings/dam/dm/presets/viewer</code>.</li> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> 
    <ol> 
     ...
     <li>等待視頻完成處理。</li> 
     <li>若您重新載入視訊，請確定Dynamic Media「編碼視訊」工作流程未執行。<br /> </li> 
     <li>重新上傳視訊。</li> 
    </ol> </td> 
  </tr> 
  <tr> 
   ...
   <td> 
    <ul> 
     <li>檢查運行模式是 <span class="kbd">dynamicmedia_scene7</span>.</li> 
     <li>檢查是否已設定Dynamic Media雲端服務。</li> 
     <li>檢查視訊設定檔是否與上傳資料夾相關聯。</li> 
    </ul> </td> 
   <td> 
    <ol> 
     <li>使用檢查您的AEM例項 <span class="kbd">-r dynamicmedia_scene7</span></li> 
  ...
  <tr> 
   ...
   <td><p>要確定視頻編碼是否仍在進行中，或是否已進入故障狀態：</p> 
    <ul> 
     <li>檢查視訊狀態 <code>http://localhost:4502/crx/de/index.jsp#/content/dam/folder/videomp4/jcr%3Acontent</code> &gt; <span class="kbd">dam:assetState</span></li> 
     <li>從工作流程主控台監視視訊 <code>http://localhost:4502/libs/cq/workflow/content/console.html</code> &gt;實例、存檔、失敗頁簽。</li> 
    </ul> </td> 
   <td> </td> 
  </tr> 
  <tr> 
