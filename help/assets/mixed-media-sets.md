...
1. [建立混合媒體集。](#creating-mixed-media-sets)
   ...
   若要建立混合媒體集，請從「資 **[!UICONTROL 產」頁面]** ，點選「 ****&#x200B;建立>混合媒體集」，然後命名該集。 選擇資產，然後選擇影像的顯示順序。

   See [Working with Selectors.](working-with-selectors.md)

1. 視需要 [設定混合媒體檢視器預設](managing-viewer-presets.md)。
...
1. [發佈混合媒體集。](#publishing-mixed-media-sets)
   ...
   發佈混合媒體集會啟動URL和內嵌字串。 此外，您必須發 [布檢視器預設集](managing-viewer-presets.md#publishing-viewer-presets)。
...
如果需要，可以編輯混 [合媒體集](#editing-mixed-media-sets)。 此外，您還可以檢視和修改混 [合媒體集屬性](managing-assets-touch-ui.md#editing-properties)。

>[!NOTE]
>
>如果您在建立集時遇到問題，請參 [閱「疑難排解動態媒體- Scene7模式」](troubleshoot-dms7.md)。
...
1. 在「混 **[!UICONTROL 合媒體集編輯器]** 」頁的「 **[!UICONTROL 標題]**」中，輸入混合媒體集的名稱。 名稱會出現在混合媒體集的橫幅中。 （可選）輸入說明。
...
1. 點選「 **[!UICONTROL 資產選擇器]** 」，以選取您要納入混合媒體集的資產。 選取它們，然後點選「 **[!UICONTROL 選取]**」。

   With the **[!UICONTROL Asset Selector]**, you can search for assets by typing in a keyword and tapping **[!UICONTROL Return]**. 您也可以套用篩選條件來調整搜尋結果。您可以依路徑、系列、檔案類型和標籤來篩選。選取篩選，然後從工具列點選 **[!UICONTROL 「篩選]** 」圖示。Change the view by selecting the View icon and selecting **[!UICONTROL List]**, **[!UICONTROL Column]**, or **[!UICONTROL Card]** view.
...
1. 視需要將資產拖曳至清單上或下方，以重新排序資產（必須選取重新排序圖示）。
   ...
   If you want to add thumbnails, click the **[!UICONTROL +]** icon next to the image and navigate to the thumbnail you want. 選取完所有縮圖影像後，點選「儲存」 **[!UICONTROL (Save]**)。
...
您可以直接在使用者介面中，對混合媒體集中的資產執行各種編輯工作，就像對資產 [中的任何資產一樣](managing-assets-touch-ui.md)。 您也可以在混合媒體集中執行下列動作：
...
1. 完成 **編輯時點選[!UICONTROL** Save。
   ...
   >[!NOTE]
   >
   >* 若要編輯混合媒體集中的資產——導覽至混合媒體集。 Tap (do not select) the set to open it in the AEM **[!UICONTROL Set Preview]** page. In the left rail, tap the down caret to open the drop-down list, then tap **[!UICONTROL Set Members]**. In the **[!UICONTROL Set Members]** page, hover on an asset, then tap **[!UICONTROL Edit]** (pencil icon) to open the editing page.
...
如需如 [何預覽混合媒體集](previewing-assets.md) ，請參閱預覽資產。

## 發佈混合媒體集 {#publishing-mixed-media-sets}

如需如 [何發佈混合媒體集](publishing-dynamicmedia-assets.md) ，請參閱發佈資產。
