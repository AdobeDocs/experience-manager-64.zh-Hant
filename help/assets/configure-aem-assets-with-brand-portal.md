...
>[!NOTE]
>
>透過以Brand Portal設定AEM Assets [!DNL Adobe I/O] AEM 6.4.8.0及更新版本支援。
>
...
>***僅適用於現有客戶***
>
>建議您繼續使用現有的舊版OAuth閘道設定。 如果您遇到舊版OAuth閘道設定的問題，請刪除現有設定，然後透過 [!DNL Adobe I/O].

本說明說明下列兩個使用案例：

* [新配置](#configure-new-integration-64):如果您是新的Brand Portal使用者，且想使用Brand Portal設定您的AEM Assets製作例項，可在 [!DNL Adobe I/O].
* [升級配置](#upgrade-integration-64):如果您是現有的Brand Portal使用者，且舊版OAuth閘道上的AEM Assets製作執行個體已使用Brand Portal進行設定，建議您刪除現有設定，並在上建立新設定 [!DNL Adobe I/O].
...
* 如果您是現有AEM客戶，請從下載AEM 6.4 [Adobe授權網站](http://licensing.adobe.com).

* 如果您是Adobe合作夥伴，請使用 [Adobe合作夥伴培訓計畫](https://adobe.allegiancetech.com/cgi-bin/qwebcorporate.dll?idx=82357Q) 請求AEM 6.4。
...
**聯絡客戶支援** 如果您找不到最新的AEM套件或Service Pack。
...
1. [建立 [!DNL Adobe I/O] 整合](#createnewintegration)
...
公開憑證可讓您在 [!DNL Adobe I/O].

1. 登入您的AEM Assets製作執行個體預設URL:http:// localhost:4502/aem/start.html
...
1. 按一下&#x200B;**[!UICONTROL 下載公開金鑰]**，並將 *AEM-Adobe-IMS.crt* 憑證檔案儲存在電腦上。憑證檔案用於 [建立 [!DNL Adobe I/O] 整合](#createnewintegration).
...
1. 按一下&#x200B;**[!UICONTROL 下一步]**。
   ...
   開啟新標籤，然後 [建立 [!DNL Adobe I/O] 整合](#createnewintegration) 取得IMS帳戶設定的整合詳細資訊。

### 建立 [!DNL Adobe I/O] 整合 {#createnewintegration}

[!DNL Adobe I/O] 整合項目會產生 API 金鑰、用戶端密碼，以及設定 IMS 帳戶設定所需的裝載 (JWT)。

1. 登入 [!DNL Adobe I/O] 在Brand Portal租用戶的IMS組織上具有系統管理員權限的主控台。
...
1. 指定 IMS 帳戶的&#x200B;**[!UICONTROL 標題]**。
   ...
   貼上您在 [建立 [!DNL Adobe I/O] 整合](#createnewintegration).
...
1. 從 **工具** ![工具](assets/tools.png) 面板，導覽至 **[!UICONTROL Cloud Services>>AEM Brand Portal]**.
...
1. 從 **工具** ![工具](assets/tools.png) 面板，導覽至 **[!UICONTROL 部署>>複製]**.
...
1. 導覽至 `/etc/cloudservices/mediaportal` 和刪除 **Cloud Service配置**.
...
您現在可以 [建立配置](#configure-new-integration-64) 在AEM 6.4 Author例項上 [!DNL Adobe I/O].
