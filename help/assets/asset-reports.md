...
1. 從&#x200B;**[!UICONTROL 建立報表頁面]**&#x200B;頁面，選擇您要建立的報表，然後按一下&#x200B;**[!UICONTROL Next]**。

   ![選擇報表類型](assets/choose_report.png)

   >[!NOTE]
   >
   >依預設，「內容片段」和連結分享會包含在「已下載」報表[!UICONTROL 中。 ]選取適當的選項，以建立連結共用的報表，或從下載報表中排除內容片段。

   >[!NOTE]
   >
   >[!UICONTROL Download]報表只會顯示個別選取並使用「快速動作」下載或下載的資產的詳細資訊。 但是，它不包含已下載資料夾中資產的詳細資料。
