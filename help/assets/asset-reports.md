...
本文說明AEM Assets中有關資產的各種報表，以及如何產生報表。
...
1. 從「建 **[!UICONTROL 立報表」頁面]**，選擇您要建立的報表，並點選／按「下一 **[!UICONTROL 步」]**。
   ...
   >[!NOTE]
   >
   >在您產生「已下載 **[!UICONTROL 資產」報表]** ，請確定已啟用「資產下載」服務。從Web主控台(`https://[server name]:[port name]/system/console/configMgr`)開啟「 **[!UICONTROL Day CQ DAM Event Recorder」 (日CQ DAM事件記錄器)]**&#x200B;設定，並在「事件類型」中選取「**[!UICONTROL Asset Downloaded(DOWNLOADED)]**」選項 (如果尚未選取)。
...
1. 在儲存報表的CRX儲存庫中設定報表詳細資訊，例如標題、說明、縮圖和資料夾路徑。 依預設，資料夾路徑 *為/content/dam*。 您可以指定不同的路徑。
   ...
   「連 **結共用** 」報表會顯示資產的URL，這些資產是從AEM Assets內與外部使用者共用的。它包含共用資產之使用者的電子郵件ID、共用資產之使用者的電子郵件ID、連結的共用日期和到期日。欄無法自訂。

   「連 **結共用** 」報表不包含子檔案夾和轉譯的選項，因為它只會發佈顯示在 */var/dam/share下的共用URL*。
