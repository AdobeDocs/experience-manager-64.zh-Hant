...
1. [建立回轉集。](#creating-spin-sets)
   ...
   若要建立回轉集，請選取「建立>回轉集」，然後為該集命名，選擇資產，然後依影像的顯示順序排序影像。****
...
1. 視需要設定[回轉集檢視器預設集](managing-viewer-presets.md)。
   ...
   管理員可以建立或修改回轉集檢視器預設集。 若要查看含有檢視器預設集的回轉集，請選取回轉集，然後在左側導軌下拉式功能表中選取&#x200B;**[!UICONTROL 檢視器]**。
...
1. [發佈回轉集。](publishing-dynamicmedia-assets.md)
   ...
   「發佈回轉集」會啟動影像在回轉集物件中的顯示順序。 請務必訂購，如此旋轉360度檢視就會順暢無礙。**[!UICONTROL URL]** 和 **** 內嵌字串。此外，您必須[發佈檢視器預設集](managing-viewer-presets.md)。
...
如果需要，可以[編輯回轉集](#editing-spin-sets)。 此外，您還可以查看和編輯[回轉集屬性](managing-assets-touch-ui.md#editing-properties)。
...
1. 執行下列任一項作業：
   ...
   * 在&#x200B;**[!UICONTROL 回轉集編輯器]**&#x200B;頁面中間附近，點選&#x200B;**[!UICONTROL 以開啟資產選擇器]**。

   點選以選取您要納入回轉集中的資產。 選取的資產上面有核取標籤圖示。完成後，在頁面右上角附近點選&#x200B;**[!UICONTROL 選擇]**。

   使用「資產選擇器」，您可以輸入關鍵字並點選「回報」來搜尋 **[!UICONTROL 資產]**。您也可以套用篩選條件來調整搜尋結果。您可以依路徑、系列、檔案類型和標籤來篩選。選取篩選，然後點選工具 **[!UICONTROL 列上的]** 「篩選」圖示。若要變更檢視，請在頁面右上角附近點選&#x200B;**[!UICONTROL 檢視]**&#x200B;圖示，然後點選&#x200B;**[!UICONTROL 欄檢視]**、**[!UICONTROL 卡片檢視]**&#x200B;或&#x200B;**[!UICONTROL 清單檢視]**。

   請參閱[使用選擇器](working-with-selectors.md)。

   ![chlimage_1-383](assets/chlimage_1-383.png)

1. 當您將資產新增至集合時，資產會自動以字母數字順序新增。 您可以在新增資產後手動重新排序或排序資產。 如有必要，將資產的&#x200B;**[!UICONTROL Reorder]**&#x200B;圖示拖曳至資產檔案名稱的右側，以在設定清單上或下重新排序影像。
...
1. 點選&#x200B;**[!UICONTROL Save]**。 您新建立的「回轉集」會顯示在您所建立的檔案夾中。

## 查看回轉集{#viewing-spin-sets}

您可以在使用者介面中建立回轉集，或使用[批次集預設集](/help/assets/config-dms7.md#creating-batch-set-presets-to-auto-generate-image-sets-and-spin-sets)自動建立回轉集。 不過，使用批集預設集建立的集合，請&#x200B;*not*&#x200B;出現在使用者介面中。 您可以三種不同的方式存取使用批次集預設集建立的集。 （即使您在使用者介面中建立回轉集，這些方法也可用）。

您也可以透過使用者介面檢視集，如[編輯回轉集](#editing-spin-sets)所述。

**要查看回轉集，請執行以下操作：**

1. 開啟個別資產的屬性時。 屬性指示所選資產是成員（在&#x200B;**[!UICONTROL 整合員]**&#x200B;下）的成員。 點選集的名稱可查看整個集。
...
如果您將AEM用縮圖取代為其他影像，以刪除該縮圖所使用的影像，原始資產仍會顯示。
   ...
   * 要刪除整個回轉集，請導航至回轉集，選擇它，然後選擇&#x200B;**[!UICONTROL 刪除]**
      ...
      >[!NOTE]
      >* 您可以導覽至回轉集編輯影像，點選左側導軌中的&#x200B;**[!UICONTROL Set Members]**，然後點選個別資產上的&#x200B;**[!UICONTROL Edit]**（鉛筆圖示）以開啟編輯視窗。
