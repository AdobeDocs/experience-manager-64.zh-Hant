...
title: 互動影片
seo-title: 互動影片
description: 瞭解如何在動態媒體中處理互動式視訊
seo-description: 瞭解如何在動態媒體中處理互動式視訊
...
請參閱[建立新檢視器預設集](managing-viewer-presets.md#creating-a-new-viewer-preset)（這是選用）和[建立互動檢視器預設集的特殊考量事項](managing-viewer-presets.md#special-considerations-for-creating-an-interactive-viewer-preset)。
...
1. **將互動式視訊新增至您的網站或網站，位於**

   如果您使用AEM Sites或AEM電子商務或兩者，您可將互動式媒體元件拖曳至頁面，將互動式視訊直接新AEM增至網頁。 請參閱[將Dynamic Media資產新增至頁面。](adding-dynamic-media-assets-to-pages.md)
...
* 若要在Google Chrome中查看所有傳出的HTTP要求，請使用鍵盤快速鍵`F12`(Windows)或`Command + Options + I`(Mac)開啟&#x200B;**[!UICONTROL 開發人員工具]**&#x200B;面板，然後按一下&#x200B;**[!UICONTROL 網路]**&#x200B;標籤。

* 在Mozilla Firefox中，您可以使用鍵盤快速鍵`F12`(Windows)或`Command + Options + I`(Mac)來啟動Firebug外掛程式，並使用其&#x200B;**[!UICONTROL Net]**&#x200B;標籤，或使用內建的「偵測器」工具及其「網路」標籤。
...
將上述方法套用至我們的範例網站時，我們有一個含有許多產品縮圖的網頁，每個產品縮圖都有一個&#x200B;**[!UICONTROL SEE MORE]**&#x200B;按鈕：
...
在製作環境中按一下縮略圖時，將顯示&#x200B;**[!UICONTROL Quickview]**&#x200B;對話框的預覽。

![chlimage_1-127](assets/chlimage_1-127.png)

您可以選擇建立自訂的互動式視訊檢視器預設集。 您可決定視訊播放器的樣式、互動式縮圖和視訊結尾的縮圖格線檢視。

互動式視訊檢視器預設集會正確呈現您新增的視訊和所有時間軸區段。 當您在&#x200B;**[!UICONTROL 預覽]**&#x200B;模式中按一下產品縮圖時，也會使用預設的Quickview範例，讓您在發佈前先測試其互動性。

儲存檢視器預設集後，其狀態會自動設為&#x200B;**[!UICONTROL 檢視器預設集]**&#x200B;頁面中的&#x200B;**[!UICONTROL On]**。 此狀態表示在動態媒體元件中及您使用它預覽視訊時，都可看到它。請確定您也手動發佈新的檢視器預設集。

請參閱[建立新檢視器預設集](managing-viewer-presets.md#creating-a-new-viewer-preset)以建立您自己的互動式視訊檢視器預設集。
...
如果您上傳的視訊或影像不正確，或您想要刪除已上傳的視訊或影像，請參閱[刪除資產](managing-assets-touch-ui.md#deleting-assets)。
...
1. 將視訊和相關的縮圖資產上傳至您想要的檔案夾或檔案夾。
   ...
   請參閱[使用FTP工作排程來上傳資產](managing-assets-touch-ui.md)。
...
1. 在&#x200B;**[!UICONTROL 建立互動式視訊]**&#x200B;頁面上，執行下列任一項作業：
...
1. (可選) 向左拖曳時間軸縮放滑桿上的橫條，以放大或向右縮小，借此控制您所新增區段的詳細程度。****
   ...
   根據視訊的長度，**[!UICONTROL 區段持續時間]**&#x200B;預設為下列值：
   ...
   為了說明，以下三個螢幕擷取畫面使用相同的影片。 請注意，每個區段的寬度會依&#x200B;**[!UICONTROL 時間軸縮放]**&#x200B;設定而改變。

   ![chlimage_1-129](assets/chlimage_1-129.png)

   螢幕擷取A

   以上螢幕擷取A顯示29秒產品視訊的預設檢視。 **[!UICONTROL 時間軸縮放]**&#x200B;預設值為5秒。

   ![chlimage_1-130](assets/chlimage_1-130.png)

   螢幕擷取B

   在上述的螢幕擷取B中，將&#x200B;**[!UICONTROL 時間軸縮放]**&#x200B;滑桿從預設的5秒拖曳至3秒。 請注意，個別&#x200B;**[!UICONTROL 時間軸刻度]**&#x200B;時間戳記現在都以3秒間隔設定。
...
1. （可選）執行下列任一項作業：
   ...
   * 若要刪除區段
      ...
      選取時間軸上的最後一個區段，然後在工具列上，點選「刪除區段」**[!UICONTROL 。]**&#x200B;如果選取了兩個或兩個以上的區段，則會停用「刪除區段」功能。****
...
1. 在「**[!UICONTROL Content]**」標籤下，點選「**[!UICONTROL 選擇資產]**」，然後瀏覽並選取您要用於視訊的所有影像資產。 選取的資產會新增至&#x200B;**[!UICONTROL 內容]**&#x200B;標籤中的&#x200B;**[!UICONTROL 資產選擇器]**&#x200B;面板。
...
如果您是AEM Sites的客戶，可將互動式媒體元件拖曳至您的頁面，以新增互動式視訊。 請參閱[將Dynamic Media資產新增至頁面。](adding-dynamic-media-assets-to-pages.md)
...
最後，請確定您的&#x200B;**[!UICONTROL Quickview]**&#x200B;對話方塊已附加至檢視器的容器元素。 預設內嵌程式碼提供實現此功能的範例步驟。 若要取得檢視器容器元素的參考，您可以使用下列程式碼行：
