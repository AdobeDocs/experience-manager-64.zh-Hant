...
可以定義當特定類型或格式的資產上傳至伺服器時自動執行的工作流程。 處理步驟定義為一系列Experience ManagerAssets媒體處理常式。 Adobe Experience Manager提供一些[內建處理常式、](#default-media-handlers)等，可以是[自訂開發的](#creating-a-new-media-handler)，或透過將程式委派至[命令列工具](#command-line-based-media-handler)來定義。

媒體處理常式是Experience Manager資產內對資產執行特定動作的服務。 例如，當MP3音訊檔案上傳至Experience Manager時，工作流程會觸發MP3處理常式，擷取中繼資料並產生縮圖。 媒體處理常式與工作流程搭配使用。 Experience Manager支援最常見的MIME類型。 您可以執行下列任一操作，對資產執行特定工作
...
Experience Manager有一些處理資產的預設工作流程。 要查看它們，請開啟工作流控制台，然後按一下&#x200B;**[!UICONTROL Models]**&#x200B;頁簽：以「Experience Manager資產」開頭的工作流程標題是資產專屬標題。
...
請參閱[開發工具](../sites-developing/dev-tools.md)以使用Maven外掛程式安裝和設定Eclipse，以及設定Maven專案所需的相依性。
...
1. 在Eclipse中，建立`myBundle` Maven專案：
   ...
   1. 在對話方塊中，展開Maven資料夾，選取Maven專案，然後按一下&#x200B;**[!UICONTROL Next]**。
   1. 勾選「**[!UICONTROL 建立簡單專案]**」方塊和「**[!UICONTROL 使用預設工作區位置]**」方塊，然後按一下「**[!UICONTROL 下一步]**」。
   1. 使用下列值定義Maven專案：
...
1. 建立`com.day.cq5.myhandler`包含`myBundle/src/main/java`下Java™類的包：
...
1. 建立Java™類`MyHandler`:
   ...
   1. 在Eclipse的`myBundle/src/main/java`下，按一下右鍵`com.day.cq5.myhandler`包，依次選擇新建和類。
...
1. 編譯Java™類並建立包：
   ...
   1. 按一下右鍵myBundle項目，選擇&#x200B;**[!UICONTROL 運行方式]**，然後選擇&#x200B;**[!UICONTROL Maven安裝]**。
...
1. 要查看工具是否正常運行，請將JPG映像添加到工作目錄，然後在命令行上運行命令`convert <image-name>.jpg -flip <image-name>-flipped.jpg`。
...
1. 更改&#x200B;**[!UICONTROL 啟用Web的格式副本]**&#x200B;步驟的設定，如下所示：
...
| cmd:&lt;command> | 定義運行的命令。 語法取決於命令列工具。 只能定義一個命令。 <br>以下變數可用來建立命令： <br>`${filename}`:輸入檔案的名稱，例如original.jpg  <br> `${file}`:輸入檔案的完整路徑名，例如/tmp/cqdam0816.tmp/original.jpg  <br> `${directory}`:輸入檔案的目錄，例如/tmp/cqdam0816.tmp  <br>`${basename}`:不帶副檔名的輸入檔案名稱，例如原始檔 <br>`${extension}`案：輸入檔案的副檔名，例如jpg |

例如，如果在Experience Manager伺服器所在的磁碟上安裝了ImageMagick，並且您使用&#x200B;**CommandLineProcess**&#x200B;作為實施，並使用以下值作為&#x200B;**Process Arguments**&#x200B;建立處理步驟：

`mime:image/gif,mime:image/tiff,tn:140:100,tn:48:48,tn:10:250,cmd:convert ${directory}/${filename} -flip ${directory}/${basename}.flipped.jpg`

接著，當工作流程執行時，步驟僅會套用至以mime類型形式具有`image/gif`或`mime:image/tiff`的資產。 它會建立原始影像的翻轉影像，將其轉換為.jpg，並建立三個具有尺寸的縮圖：140x100、48x48和10x250。
...
使用以下[!UICONTROL Process Arguments]使用ImageMagick建立啟用Web的格式副本：

`mime:image/tiff,mime:image/png,mime:image/bmp,mime:image/gif,mime:image/jpeg,cmd:convert ${filename} -define jpeg:size=1280x1280 -thumbnail "1280x1280>" cq5dam.web.1280.1280.jpeg`

>[!NOTE]
>
>`CommandLineProcess`步驟僅適用於資產（`dam:Asset`類型的節點）或資產的子系。
