...
可以定義工作流，當特定類型或格式的資產上傳至伺服器時自動執行。 處理步驟已定義為一系列AEM Assets媒體處理常式。 AEM提供一些內 [建的處理常式](#default-media-handlers) ，其他的處理常式可 [以自訂開發](#creating-a-new-media-handler) ，或借由將程式委派至命令列工 [具來定義](#command-line-based-media-handler)。
...
1. 安裝ImageMagick。 如需詳 [細資訊，請參閱ImageMagick](https://www.imagemagick.org/script/download.php) 檔案。
1. 設定工具，以便在命令行上運行轉換。
...
1. 若要查看工具是否正常運行，請將JPG影像添加到工作目錄並在命令行 `convert <image-name>.jpg -flip <image-name>-flipped.jpg` 上運行命令。
...
本節介紹如何設定 **[!UICONTROL 的Process]**`CommandLineProcess`參數。使用逗號分 [!UICONTROL 隔「處理參數] 」的值，不要以空格開頭。
...
>[!NOTE]
>
>此步 `CommandLineProcess` 驟僅適用於資產(類型的節 `dam:Asset`點)或資產的子項。
