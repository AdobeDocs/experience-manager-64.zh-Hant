...
* [內容片段模型](content-fragments-models.md)  — 啟用、建立和定義您的模型

* [變異 — 編寫片段內容](content-fragments-variations.md)  — 編寫片段內容並建立主版的變異

* [Markdown](content-fragments-markdown.md)  — 對片段使用Markdown語法

* [使用關聯內容](content-fragments-assoc-content.md)  — 新增關聯內容

* [中繼資料](content-fragments-metadata.md)  — 片段屬性 — 檢視和編輯片段屬性
...
>[!NOTE]
>
>AEM也支援片段內容的翻譯。 如需詳細資訊，請參閱[建立內容片段的翻譯專案](creating-translation-projects-for-content-fragments.md)。
...
* 包含結構化內容的片段
   ...
   * 這些是以[內容片段模型](content-fragments-models.md)為基礎，該模型預先定義所產生片段的結構。
...
* **插入** 資產片段（混合媒體片段）
   ...
   * 定義資產的位置。
   * 如需詳細資訊，請參閱片段編輯器中的[將資產插入您的片段](content-fragments-variations.md#inserting-assets-into-your-fragment) 。
...
* **插入片段中的資產（混合媒體片段）**
   ...
   * 您只能在片段編輯器](content-fragments-variations.md#inserting-assets-into-your-fragment)中使用[RTF格式，將片段新增至、從中刪除或移動。
   * 只能新增至多行文字元素（任何片段類型）。
   * 附於前文（段）。
...
* **相關聯的內容**
   ...
   * 資產是透過集合](content-fragments-assoc-content.md)與片段關聯的[;相關聯的集合可讓作者在編寫頁面時決定要使用的資產。

      * 集合可以透過範本、預設內容或由作者在片段製作期間，與片段建立關聯。
      * [資產(DAM)](managing-collections-touch-ui.md) 集合是片段相關聯內容的基礎。
...
* **主版**
   ...
   * 片段的一個完整部分
      ...
      * 每個內容片段都有一個主版例項。
      * 無法刪除主版。
   * 可在&#x200B;**[Valiations](content-fragments-variations.md)**&#x200B;下的片段編輯器中存取主版。
   * 主版並非如此，而是所有變異的基礎。
...
* **變數**
   ...
   * 儲存在片段中，以協助避免內容復本的分散。
   * 如果更新了主內容，則變數可以是與主內容同步的[](content-fragments-variations.md#synchronizing-with-master)。
   * 可以是[摘要](content-fragments-variations.md#summarizing-text)，以快速截斷文字至預先定義的長度。
   * 在片段編輯器的[Valuations](content-fragments-variations.md)標籤下可用。
...
* **內容模型**
   ...
   * 啟用，然後使用工具](content-fragments-models.md)建立。[
