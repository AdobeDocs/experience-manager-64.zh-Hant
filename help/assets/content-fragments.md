...
>[!NOTE]
>
>AEM也支援片段內容的轉譯。 如需詳 [細資訊，請參閱建立內容片段的轉譯專案](creating-translation-projects-for-content-fragments.md) 。
...
* 包含結構化內容的片段
   ...
   * 這些是以內容片段模 [型為基礎](content-fragments-models.md)，內容片段模型會預先定義產生的片段結構。
...
* **將資產插入** （混合媒體片段）
   ...
   * 定義資產的位置。
   * 如需詳 [細資訊，請參閱片段編輯器中](content-fragments-variations.md#inserting-assets-into-your-fragment) 「將資產插入片段」。
...
* **相關聯的內容**
   ...
   * 如需詳 [細資訊](content-fragments-assoc-content.md) ，請參閱關聯內容。
...
* **插入片段的資產（混合媒體片段）**
   ...
   * 只能在片段編輯器中使用 [Rich Text格式將片段添加到片段、從中刪除或移動](content-fragments-variations.md#inserting-assets-into-your-fragment)。
   * 只能新增至多行文字元素（任何片段類型）。
   * 附於前文（段）。
...
* **相關聯的內容**
   ...
   * 這些資產會 [透過系列關聯至片段](content-fragments-assoc-content.md); 關聯的系列可讓作者決定在編寫頁面時要使用哪些資產。

      * 系列可以透過範本、預設內容或作者在片段製作期間，與片段建立關聯。
      * [資產(DAM)集合](managing-collections-touch-ui.md) ，是片段相關內容的基礎。
   * （可選）您也可以將片段本身新增至系列，以協助追蹤。


* **片段中繼資料**

   * 使用「資 [產」中繼資料結構](metadata.md)。
   * 標籤可在下列情況下建立：
...
* **主版**
   ...
   * 碎片的一個整體
      ...
      * 每個內容片段都有一個主體實例。
      * 無法刪除主版。
   * 主版可在片段編輯器的「變數」下 **[存取](content-fragments-variations.md)**。
   * 主變數不是變數，而是所有變數的基礎。
...
* **變數**
   ...
   * 儲存在片段中，以協助避免內容復本散布。
   * 如果主版內 [容已更新](content-fragments-variations.md#synchronizing-with-master) ，則可與主版同步變數。
   * 可以摘 [要](content-fragments-variations.md#summarizing-text) ，快速將文字截斷為預先定義的長度。
   * 可在片段編輯 [器的](content-fragments-variations.md) 「變化」標籤下使用。
...
* **內容模型**
   ...
   * 啟 [用，然後使用工具建立](content-fragments-models.md)。
   * 建立結 [構化片段所需](content-fragments-managing.md#creating-content-fragments)。
   * 定義片段的結構（標題、內容元素、標籤定義）。
   * 內容模型定義需要一個標題和一個資料元素； 其他一切都是可選的。 模型會定義片段的最小範圍，並定義預設內容（如果適用）。 製作片段內容時，作者無法變更已定義的結構。

* **片段範本**

   * 建立簡 [單片段所需](content-fragments-managing.md#creating-content-fragments)。
