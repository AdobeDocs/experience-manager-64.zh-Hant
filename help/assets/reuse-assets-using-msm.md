...
要使用[!DNL Assets]的MSM，請至少安裝[!DNL Experience Manager] 6.4 Service Pack 5。 如需詳細資訊，請參閱[最新Service Pack的發行說明](/help/release-notes/sp-release-notes.md)。
...
* **方法1**:選取來源資產，然後按一 **[!UICONTROL 下頂端的]**  **[!UICONTROL 「建]** 立>即時組排」工具列。
* **方法2**:在使AEM用者介面中，按一 **[!UICONTROL 下介面右上]** 角的「建立>即時組排」。
...
1. 選擇源資產或資料夾。 在工具列中，按一下「建立>即時副本&#x200B;]**」。**[!UICONTROL 
   ![從介面建立即時AEM副本](assets/lc_create1.png)
...
1. 在介AEM面中，從右上角按一下「建立>即時副本」。****
   ![從介面建立即時AEM副本](assets/lc_create2.png)
1. 選擇源資產或資料夾。 按一下&#x200B;**[!UICONTROL 下一步]**。
...
您可以從使用者介面的不同區域檢視即時副本的資訊和MSM相關狀態，例如關係、同步、推AEM出等。 以下兩種方法適用於資產和檔案夾：
...
要查看與同步操作相關的狀態和資訊，請參閱[資料夾的所有即時副本的資訊和狀態](#information-status-of-all-lcs-of-folder)。

![同步操作將提取對源進行的更改](assets/lc_sync.png)

>[!NOTE]
>
>如果關係被暫停，則工具欄中不提供同步操作。 雖然同步動作可在[!UICONTROL References]導軌中使用，但即使在成功轉出時，修改也不會傳播。
...
您可以使用「分離」操作完全移除源和即時副本之間的關係。 即時副本分離後，即時副本會變成獨立的資產或資料夾。 在分離後，它會立即在介AEM面中顯示為新資產。 要將即時副本從其源中分離，請執行以下步驟。
...
AEM可讓您使用MSM Java API擴充功能。 對於Assets，擴充功能的運作方式與MSM for Site的運作方式相同。 如需詳細資訊，請參閱[擴充MSM](../sites-developing/extending-msm.md)和下列章節，以取得特定工作的詳細資訊：
...
即時副本和來源是可在一定程度上作為數字資產進行管理的資產或資料夾。 中的某些資產管理任AEM務對即時拷貝有特定影響。
