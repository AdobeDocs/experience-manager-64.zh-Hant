...
1. 若要上傳資產，請執行下列其中一項作業：
   ...
   * 在支援HTML5的瀏覽器中，直接將資產拖曳至介面。 不會顯示要更名檔案的對話框。
   ...
   如 **[!UICONTROL 果上傳的檔案大於1000 MB，且檔案小於1000 MB，則「暫停]**]」按鈕不會顯示。 不過，如果您取消少於1000 MB的檔案上傳，則會顯示「暫**[!UICONTROL 停&#x200B;]**」按鈕。
...
|  | 填充方法 | 必要.<br> 從您設定的「轉角」位置控制像素透明度。<br> 您可從下列填色方法中選擇： <ul><li>**泛洪填滿** -將所有像素變成透明，這些像素與您指定並連接到的「角」相符。</li><li>**比對像素** -將所有匹配的像素都透明，而不論其在影像上的位置。</li></ul> |
...
如果要從上載的映像自動建立映像集或回轉集，請按一下要使用的預設集的 **[!UICONTROL Active** （活動）列。 您可以選取多個預設集。
...
1. 點選「全 **[!UICONTROL 域導覽」圖示]** ，並從清單中選 **擇「時間軸」** 。
