...
您可以建立兩個影像預設集：桌上型電腦版本為500 x 500像素，行動版本為150 x 150像素。 您可以建立兩個影像預設集，一個稱為「放大 ** 」，以500x500像素顯示影像，另一個稱為「縮圖 ** 」，以150x150像素顯示影像。 若要以放大和縮圖大小傳送影像，AEM會尋找放大影像預設集和縮圖影像預設集的定義。 然後，AEM會以每個影像預設集的大小和格式規格動態產生影像。
...
您可以點選AEM標誌以存取全域導覽主控台，然後點選「工具」圖示並導覽至「資產>影像預設集」，以管理AEM中的 **[!UICONTROL 影像預設集]**。
...
>[!NOTE]
>
>當您在資產的「詳細資料檢視」中選取「轉譯」時， **[!UICONTROL 系統會顯示多種]** 轉譯 **** 。 您可以增加或減少顯示的影像預設集數目。 See [Increasing the number of image presets that display](#increasing-or-decreasing-the-number-of-image-presets-that-display).
...
當您開啟資產時，可以檢視子資產或頁面、點選「內容」功能表，然後選取「子資 **[!UICONTROL 產]** 」或 **[!UICONTROL 「頁面」]**。 子資產是真實資產。 也就是說，PDF頁面是由工作流程元件 `Create Sub Asset` 所擷取。 然後，這些資產會 `page1.pdf`儲存為 `page2.pdf`主要資產下方的資產，依此類推。 儲存後， **[!UICONTROL DAM更新資產工作流程會處理這些項目]** 。

若要使用動態媒體來預覽並產生AI、EPS或PDF檔案的動態轉譯，需要下列處理步驟：

1. 在 **[!UICONTROL DAM更新資產工作流程中，]** 點陣化PDF/AI影像預覽轉譯處理元件會使用設定的解析度，將原始資產的第一頁點陣化為轉 ****`cqdam.preview.png` 譯。

1. 然後 `cqdam.preview.png` ，在工作流程中， **** Dynamic Media Process Image Assets處理元件會將轉譯最佳化為PTIFF。
...
您可以 **[!UICONTROL 透過「]** DAM更新資產」工作流程存取「點陣化PDF/AI影像預覽轉譯 **[!UICONTROL 」流程元件選]** 項。

點選左上角的Adobe Experience Manager，導覽至「工具>工 **[!UICONTROL 作流程>模型」]**。 在「工 **[!UICONTROL 作流程模型]** 」頁面上，選 **[!UICONTROL 取「DAM更新資產」]**，然後在工具列上點選「 **[!UICONTROL 編輯」]**。 在「 **[!UICONTROL DAM更新資產」工作流程頁面]** ，點選兩下「點陣化PDF/AI影像預覽轉譯」 **[!UICONTROL (Rasterize PDF/AI Image Preview Rendition]** )處理元件以開啟其「 **[!UICONTROL 步驟屬性]** 」對話方塊。
...
**[!UICONTROL 最大寬度]** 和最 **[!UICONTROL 大高度]** ，限制點陣化的解析度。 例如，如果最大值未變更，而「解析度」設為300 ppi，則美國字母檔案點陣化為186 ppi。 也就是說，檔案是1581 x 2046像素。

點陣 **[!UICONTROL 化PDF/AI影像預覽轉譯]** (Rasterize PDF/AI Image Preview Rendition)程式元件已定義上限，以確保不會在記憶體中建立過大的影像。 此類大型映像可能會使提供給JVM（Java虛擬機）的記憶體溢出。 請務必為JVM提供足夠的記憶體來管理已配置的並行工作流數，每個工作流都有可能以最大配置的大小建立映像。
...
請參 [閱「整合AEM資產與InDesign Server」](indesign.md)。

**[!UICONTROL DAM Update Asset工作流程中的「媒體擷取」處理程式元件]** ，會執行數個預先設定的 **[!UICONTROL Extend Scripts]** ，以處理InDesign檔案。

![媒體提取過程參數中的擴展指令碼路徑](assets/media_extraction_arguments.png)

在 **[!UICONTROL DAM]** 更新資產工作流程中，「媒體擷取 **[!UICONTROL 」程式元件引數中的「延伸指令碼路徑]****** 」。
...
1. 點選「處 **[!UICONTROL 理縮圖]** 」步驟，然後點選「縮 **[!UICONTROL 圖]** 」標籤。Change the thumbnail size, as needed, and tap **[!UICONTROL OK]**.
...
>[!NOTE]
>
>如果您使用Internet Explorer 9，則建立預設集後不會立即出現在預設集清單中。 若要解決此問題，請停用IE9的快取。
...
如果您想要支援擷取INDD檔案，以便產生此檔案格式的動態轉譯，您可能需要先檢閱下列資訊，再建立影像預設集。  請參 [閱InDesign(INDD)檔案格式](#indesign-indd-file-format)。
...
1. 在「編 **[!UICONTROL 輯影像預設集]********** 」頁面上，視需要在「基本」和「進階」標籤中輸入值，包括名稱。 這些選項在「影像預設 [集選項」中概述](#image-preset-options)。預設集會出現在左窗格中，並可與其他資產一起即時使用。
...
當您建立或編輯影像預設集時，您可以選擇本節所述的選項。 此外，Adobe建議您從下列三個 *最佳實務* 選項選項開始：
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td><p>選擇「 <strong>無</strong>」、「銳 <strong>化」</strong>或「銳 <strong>化」或「銳化」</strong>。 </p> 
    ...
    <div>
      銳利化影像中會 <a href="https://marketing.adobe.com/resources/help/en_US/s7/sharpening/s7_sharpening_images.pdf">說明銳利化</a>。 
    </div> </td> 
  </tr> 
  <tr> 
...
除了「基本」和「進階」 **[!UICONTROL 標籤中的選項]****** ，您還可以定義影像修飾元，以在定義影像預設集時提供更多選項。 影像演算需仰賴動態媒體影像演算API。 API在 [HTTP通訊協定參考中有詳細定義](https://marketing.adobe.com/resources/help/en_US/s7/is_ir_api/is_api/http_ref/c_http_protocol_reference.html)。
...
* [op_invert](https://marketing.adobe.com/resources/help/en_US/s7/is_ir_api/is_api/http_ref/r_op_invert.html) —— 反轉每個顏色元件以產生負面影像效果。
...
* [op_blur](https://marketing.adobe.com/resources/help/en_US/s7/is_ir_api/is_api/http_ref/r_op_blur.html) —— 對影像套用模糊濾鏡。
...
* [opac](https://marketing.adobe.com/resources/help/en_US/s7/is_ir_api/is_api/http_ref/r_opac.html) —— 調整影像不透明度。 可讓您降低前景不透明度。
...
1. 選取預設集，然後點選「編 **[!UICONTROL 輯」]**。
1. 在「編 **[!UICONTROL 輯影像預設集]** 」頁面上，進行您想要的變更，然後點選「 **[!UICONTROL 儲存」]**。
...
1. 在AEM中，點選AEM標誌以存取全域導覽主控台。
1. 點選「工 **[!UICONTROL 具」圖示]** ，然後導覽至「資 **[!UICONTROL 產>影像預設集」]**。
1. 從影像預設集清單中選取影像預設集或多個影像預設集，然後點選「發 **[!UICONTROL 布」]**。
...
1. 在AEM中，點選AEM標誌以存取全域導覽主控台。
1. 點選「工 **[!UICONTROL 具」圖示]** ，然後導覽至「資 **[!UICONTROL 產>影像預設集」]**。
1. 選取預設集，然後點選「 **[!UICONTROL 刪除」]**。 動態媒體會確認您要刪除它。 點選 **[!UICONTROL 刪除]**。
