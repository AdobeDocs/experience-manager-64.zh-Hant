...
# 資產卸載最佳做法 {#assets-offloading-best-practices}
...
在員工上寫入作業後，作業管理員會呼叫負責 *com/adobe/granite/workflow/offloading主題的作業使用者* 。 然後，作業使用者會在資產上執行DAM更新資產工作流程。

## Sling Topology {#sling-topology}

Sling拓撲會將AEM例項分組，讓它們能夠彼此感知，不受基礎永續性的影響。 Sling拓撲的這項特性可讓您建立非叢集、叢集和混合藍本的拓撲。 實例可以向整個拓撲顯示屬性。 該框架提供回呼，用於監聽拓撲中的更改（實例和屬性）。 Sling拓撲為Sling分佈式作業提供基礎。

### Sling分散作業 {#sling-distributed-jobs}
...
如需有關設定資料儲存區的詳細資訊，請 [參閱「在AEM中設定節點儲存區和資料儲存區](../sites-deploying/data-store-config.md)」。
...
1. 開啟( `OffloadingAgentManager` )的`http://localhost:4502/system/console/configMgr/com.adobe.granite.offloading.impl.transporter.OffloadingAgentManager`設定。
1. 禁用自動代理管理。
...
### 關閉運輸包裝 {#turning-off-transport-packages}
...
1. 在http://localhost:4502/system/console/configMgr/com.adobe.granite.offloading.impl.transporter.OffloadingDefaultTransporter開啟 *OffloadingDefaultTransporter元件的元件組*[態](http://localhost:4502/system/console/configMgr/com.adobe.granite.offloading.impl.transporter.OffloadingDefaultTransporter)
1. 禁用屬 *性Replication Package(default.transport.contentpackage)*。

### 禁用工作流模型的傳輸 {#disabling-the-transport-of-workflow-model}

預設情況下， ** DAM更新資產卸載工作流將工作流模型添加到作業裝載中，以調用該工作器。 由於此工作流程預設會遵循現成可用的 *DAM更新資產模型* ，因此可移除此額外的裝載。
