...
如果您使用針對不同環境（如開發、接移和即時生產）設定的Adobe Experience Manager，則必須為每個環境配置Dynamic MediaCloud Services。

## Dynamic Media-Scene7模式的體系結構圖{#architecture-diagram-of-dynamic-media-scene-mode}

以下架構圖說明Dynamic Media-Scene7模式的運作方式。

有了新的架構，Experience Manager負責主資產並與Dynamic Media同步處理及發佈資產：

1. 當主資產上傳至Experience Manager時，會複製至Dynamic Media。 此時，Dynamic Media會處理所有資產處理和轉譯產生，例如視訊編碼和影像的動態變體。
1. 產生轉譯後，Experience Manager可以安全地存取和預覽遠端Dynamic Media轉譯(不會將二進位檔傳回Experience Manager例項)。
1. 在內容準備好發佈及核准後，它會觸發Dynamic Media服務將內容推出至傳送伺服器，並在CDN快取內容。
...
>[!NOTE]
>
>Dynamic Media-Scene7模式僅用於Experience Manager作者實例。 因此，您必須在「Experience Manager作者」實例上配置`runmode=dynamicmedia_scene7`，而不是「Experience Manager發佈」實例。

要啟用Dynamic Media，必須在終端窗口中輸入以下命令，使用命令行中的`dynamicmedia_scene7`運行模式啟動Experience Manager（使用的埠為4502）:

```shell
java -Xms4096m -Xmx4096m -Doak.queryLimitInMemory=500000 -Doak.queryLimitReads=500000 -jar cq-quickstart-6.4.0.jar -gui -r author,dynamicmedia_scene7 -p 4502
```

## （可選）將Dynamic Media預設集和配置從6.3遷移到6.4零停機時間{#optional-migrating-dynamic-media-presets-and-configurations-from-to-zero-downtime}

如果您將Dynamic MediaExperience Manager從6.3升級至6.4（包括零停機部署功能），請執行下列curl命令，將CRXDE Lite中的所有預設集和組態從`/etc`移轉至`/conf`。

>[!NOTE]
>
>如果您在相容模式下運行Experience Manager實例——即，已安裝相容包——則無需運行這些命令。

若要將自訂預設集和組態從`/etc`移轉至`/conf`，請執行下列Linux® curl命令：
...
功能套件18912可讓您透過FTP大量內嵌資產，或從Dynamic Media-混合模式或Dynamic Media經典移轉資產至Dynamic Media-Scene7-Experience Manager模式。 客人可從Adobe Professional Services購買。
...
在配置Dynamic MediaCloud Services之前，請更改密碼。 在您收到具有Dynamic Media認證的布建電子郵件後，必須[登入](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/intro/dynamic-media-classic-desktop-app.html?lang=en#system-requirements-dmc-app)至Dynamic MediaClassic案頭應用程式，才能變更密碼。 提供電子郵件中提供的密碼是系統生成的，並僅用於臨時密碼。 請務必更新密碼，以便使用正確的認證來設定Dynamic MediaCloud Service。

>[!NOTE]
>
>預設情況下，Cloud Services的配置路徑為`/content/dam`。 任何其它配置路徑都不受Dynamic Media-Scene7模式支援。
...
1. 在「Dynamic Media配置瀏覽器」頁面的左窗格中，點選&#x200B;**[!UICONTROL global]**&#x200B;並點選&#x200B;**[!UICONTROL Create]**。 請勿點選或選取[!UICONTROL global]左側的資料夾圖示。
1. 在[!UICONTROL 建立Dynamic Media配置]頁上，輸入標題、Dynamic Media帳戶電子郵件地址和密碼。 選擇您的地區。 此資訊是透過Adobe您的布建電子郵件提供給您的。 如果您未收到電子郵件，請聯絡Adobe客戶服務。

   點選&#x200B;**[!UICONTROL 連線至Dynamic Media]**。

   >[!NOTE]
   >
   >在您收到具有Dynamic Media認證的布建電子郵件後，請開啟[Dynamic Media Classic案頭應用程式](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/getting-started/signing-out.html#getting-started)，然後登入您的公司帳戶以變更密碼。 提供電子郵件中提供的密碼是系統生成的，並僅用於臨時密碼。 請務必更新密碼，以便使用正確的憑據設定Dynamic MediaCloud Service。

1. 如果連接成功，您還可以設定以下內容：

   * **[!UICONTROL 公司]** -Dynamic Media帳戶的名稱。有可能為不同子品牌、部門或不同的階移／生產環境提供多個Dynamic Media帳戶。
   * **[!UICONTROL 公司根資料夾路徑]**
...
1. 點選&#x200B;**[!UICONTROL Save]**。
1. 若要在發佈Dynamic Media內容之前安全地預覽，您必須「允許列出」Experience Manager作者例項，才能連線至Dynamic Media:

   * 開啟[Dynamic Media經典案頭應用程式](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/getting-started/signing-out.html#getting-started)，然後登入您的帳戶。 您的認證和登入詳細資訊是由布建時的Adobe提供。 如果您沒有此資訊，請聯絡技術支援。
...
如果要進一步自定義Dynamic Media-Scene7模式的配置和設定，或優化其效能，可以完成以下一個或多個可選任務：

* [（可選）Dynamic Media-Scene7模式設定和配置](#optional-setup-and-configuration-of-dynamic-media-scene-mode-settings-p)
...
### （可選）Dynamic Media-Scene7模式設定和配置</p> {#optional-setup-and-configuration-of-dynamic-media-scene-mode-settings-p}

當您處於&#x200B;**dynamicmedia_scene7**&#x200B;執行模式時，您會使用Dynamic Media經典使用者介面來變更您的Dynamic Media設定。
...
要開啟「[!UICONTROL 應用程式常規設定]」頁，請在「Dynamic Media經典全局導航欄」中按一下「**[!UICONTROL 設定>應用程式設定>常規設定]**」。

**[!UICONTROL Servers]**  —— 在帳戶布建時，Dynamic Media會自動為您的公司提供指派的伺服器。這些伺服器可用來建構網站和應用程式的URL字串。 這些URL呼叫是您帳戶專屬的。 除非Experience Manager支援明確指示，否則不要更改任何伺服器名稱。
...
>[!NOTE]
>
>要與Experience Manager保持一致，請選擇&#x200B;**[!UICONTROL 「覆寫當前資料夾中的基本映像名稱／副檔名]**」。
...
1. 開啟[Dynamic Media經典案頭應用程式](https://experienceleague.adobe.com/docs/dynamic-media-classic/using/getting-started/signing-out.html#getting-started)，然後使用布建期間提供的認證登入您的帳戶。 導覽至「**[!UICONTROL 設定>應用程式設定]**」。
...
1. 捲動至您必須變更的屬性。 例如，**[!UICONTROL 色彩管理屬性]**&#x200B;區域中的屬性。
   ...
   * [!UICONTROL 顏色轉換渲染方式] -指定渲染方式。可接受的值為`perceptual`、`relative`、`colometric`、`saturation`和`absolute colometric`。 Adobe建議使用`relative`作為預設值。
...
1. 在Experience Manager中，點選Experience Manager標誌以存取全域導覽主控台，然後點選「工具」（槌子）圖示並導覽至「一般」>「CRXDE Lite」(a3/)。********
1. 在左側導軌中，導覽至下列項目：
...
1. 在CRXDE Lite頁面的右側，在下方：
   ...
   * 按兩下&#x200B;**[!UICONTROL enabled]**&#x200B;欄位。 依預設會啟用所有資產MIME類型（設為&#x200B;**[!UICONTROL true]**），這表示資產會同步至Dynamic Media以進行處理。 如果您想要排除此資產MIME類型，請將此設定變更為&#x200B;**[!UICONTROL false]**。
...
您可以在「Experience Manager資產」中為不支援的格式新增自訂MIME類型。 為確保CRXDE Lite中添加的任何新節點不會被Experience Manager刪除，請在&#x200B;**[!UICONTROL image_]**&#x200B;之前移動MIME類型，其啟用值設定為&#x200B;**[!UICONTROL false]**。

**要為不支援的格式添加自定義MIME類型**

1. 在Experience Manager中，按一下「工具」>「操作」>「Web控制台」]**。**[!UICONTROL 
...
1. 在該頁上，向下滾動到名稱&#x200B;**[!UICONTROL Adobe CQScene7資產MIME類型服務]**。 在名稱的右側，點選「 **[!UICONTROL Edit configuration values]** (pencil icon)(編輯配置值 (鉛筆圖示) 」。

   ![編輯配置值](assets/2019-08-02_16-44-56.png)

1. 在&#x200B;**[!UICONTROL Adobe CQScene7資產MIME類型服務]**&#x200B;頁上，按一下任何加號表徵圖`+`。 在表格中，您按一下加號以新增MIME類型的位置微不足道。

   ![加號圖示](assets/2019-08-02_16-27-27.png)
...
1. 在Experience Manager中，按一下「工具」>「常規」>「CRXDE Lite」]**。**[!UICONTROL 
   ...
   ![CRXDE Lite頁](assets/2019-08-02_16-55-41.png)
...
1. 拖曳MIME類型`image_vnd.dwg`，並將它直接放在樹狀結構中的`image_`上方。
   ...
   ![拖曳MIME類型](assets/CRXDELite_CQDOC-14627.png)
...
首先，定義資產在集合中分組的命名慣例。 然後，您可以建立批集預設集，該預設集是一組唯一命名的自含指令，定義如何使用符合預設配方中定義之命名慣例的影像來建構該預設集。

上傳檔案時，Dynamic Media會自動建立一組檔案，其中包含所有符合作用中預設集中所定義之命名慣例的檔案。

**配置預設命名**

建立用於任何批集預設方式的預設命名慣例。 批集預設定義中選取的預設命名慣例可能是您的公司需要批次產生集的全部。 系統會建立批集預設，以使用您定義的預設命名慣例。 在公司定義的預設命名有例外的情況下，您可以針對特定內容集建立任意數量的批集預設集，其中包含所需的替代自訂命名慣例。

雖然使用批集預設功能不需要設定預設命名慣例，但您可以使用它來定義您想要在某個集合中分組的命名慣例元素數目。 如此有助於簡化批次集的建立。

或者，您可以使用沒有表單欄位的&#x200B;**[!UICONTROL 檢視程式碼]**。 在此視圖中，您完全使用規則運算式建立命名約定定義。

定義時可使用兩個元素：**[!UICONTROL Match]**&#x200B;和&#x200B;**[!UICONTROL Base Name]**。 這些欄位可讓您定義命名約定的所有元素，並識別用於命名包含這些元素的集合的約定部分。 公司的個別命名慣例可針對每個元素使用一或多行定義。 使用任意多行來定義您的獨特元素，並將它們分組為不同的元素。 例如，「主影像」、「色彩」元素、「替代檢視」元素和「色票」元素。
...
您可以建立、編輯和管理批次集預設集。 批集預設定義有兩種形式：一個是您設定的預設命名慣例，另一個是您即時建立的自訂命名慣例。
...
1. 在頁面頂部附近的導覽列上，點選「**[!UICONTROL 設定>應用程式設定>批次集預設集>批次集預設集」。**]
   ...
   [!UICONTROL View Form], as set in the upper-right conner of the   Details spage, is the default view.
...
1. 對於[!UICONTROL 序列順序]，定義在將影像集分組到Dynamic Media後顯示影像的順序。

   依預設，資產會以英數字元排序。 不過，您可以使用逗號分隔的規則運算式清單來定義順序。

1. 對於&#x200B;**[!UICONTROL 設定命名]**&#x200B;和&#x200B;**[!UICONTROL 建立慣例]**，請在&#x200B;**[!UICONTROL 資產命名慣例]**&#x200B;中定義的基本名稱中指定尾碼或前置詞。 此外，定義集在Dynamic Media資料夾結構中的建立位置。

   如果您定義大量集合，請將它們與包含資產本身的檔案夾分開。 例如，建立「影像集」檔案夾，並將產生的集放在此處。
...
回轉集的共用資產名稱部分的群組會新增至&#x200B;**[!UICONTROL Match]**&#x200B;欄位（如反白顯示）。 包含行和列的資產名稱的變數部分將分別添加到 **[!UICONTROL 行]** 和 **[!UICONTROL 列欄位中]** 。
...
1. 展開&#x200B;**[!UICONTROL 列位置]**，然後定義2D回轉集陣列中影像資產位置的名稱格式。
   ...
   請記住，這些運算式僅供圖例之用。 您可以建立您的規則運算式，但是您想要符合自己的需求。
...
1. 對於&#x200B;**[!UICONTROL 設定命名]**&#x200B;和&#x200B;**[!UICONTROL 建立慣例]**，請在&#x200B;**[!UICONTROL 資產命名慣例]**&#x200B;中定義的基本名稱中指定尾碼或前置詞。
   ...
   如果您定義大量集合，請將它們與包含資產本身的檔案夾分開。 例如，建立「回轉集」資料夾，將生成的集放在此處。
...
「Scene7上傳連線」設定會將Experience Manager資產同步至Dynamic Media經典伺服器。
...
在非Dynamic Media部署中，您會將&#x200B;*all*&#x200B;資產（影像和視訊）從您的Experience Manager作者環境複製到Experience Manager發佈節點。 此工作流程是必要的，因為Experience Manager發佈伺服器也會傳送資產。

不過，在Dynamic Media部署中，由於資產是透過Cloud Service傳送，因此不需要將這些相同的資產複製到Experience Manager發佈節點。 這種「混合式發佈」工作流程可避免額外的儲存成本以及複製資產的較長處理時間。 其他內容（例如網站頁面）仍會從「Experience Manager發佈」節點提供。

這些篩選條件可讓您透過&#x200B;*exclude*&#x200B;資產，防止複製至「Experience Manager發佈」節點。

#### 使用複製{#using-default-asset-filters-for-replication}的預設資產篩選器

如果您使用Dynamic Media進行影像處理或視訊處理，或兩者皆使用，則可使用Adobe提供的預設濾鏡。 下列篩選器預設為作用中：
...
1. 在Experience Manager中，點選Experience Manager標誌以存取全域導覽主控台，點選&#x200B;**[!UICONTROL 工具]**&#x200B;圖示並導覽至&#x200B;**[!UICONTROL 一般>CRXDE Lite]**。
...
1. 要定義篩選器的Mime類型，可以按如下方式查找Mime類型：
   ...
   在左側導軌中，展開&#x200B;**[!UICONTROL 內容> dam > &lt;`locate_your_asset`> > jcr:content > metadata]**，然後在表格中找到&#x200B;**[!UICONTROL dc:format]**。
