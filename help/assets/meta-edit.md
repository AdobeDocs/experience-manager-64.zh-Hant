...
1. 執行下列任一項作業：
   ...
   * 在資產頁面中，從工具列按一下／點選「檢視屬性」圖示&#x200B;**[!UICONTROL 資訊圖示]**。![](assets/do-not-localize/info_icon.png)
...
對資產中繼資料所做的任何變更都會寫入原始二進位檔，作為其XMP資料的一部分。 這是透過AEM的中繼資料回寫工作流程來完成。 對現有屬性（如`dc:title`）所做的變更會被覆寫，新建立的屬性（包括如`cq:tags`的自訂屬性）會與架構一起新增。
