...
+ 動態媒體{#dynamic}
   ...
   + [影片](s7-video.md)
   + [將Dynamic Media經典功能添加到頁面](scene7.md)
   + [HTTP2 傳送內容](http2.md)
...
+ 延伸資產{#extending}
   ...
   + [擴充資產搜尋](searchx.md)
   + [安裝並配置ImageMagick以與AEM Assets合作](best-practices-for-imagemagick.md)
...
+ 管理資產{#administer}
   ...
   + [AEM和Creative Cloud整合最佳實務](aem-cc-integration-best-practices.md)
   + [整合AEM Assets與InDesign伺服器](indesign.md)
   + [中繼資料設定檔](metadata-profiles.md)
   + [資產Digital Rights Management](drm.md)
   ...
   + [系列、程式碼片段和程式碼片段範本的多租賃](multi-tenancy.md)
   + [浮水印](watermarking.md)
...
+ 管理資產{#managing}
   ...
   + [登入和登出資產中的檔案](check-out-and-submit-assets.md)
   + [使用智慧型內容服務設定資產標籤](config-smart-tagging.md)
   + [啟用重複檢測](duplicate-detection.md)
   + [增強資產排序功AEM能](enhance-sort.md)
