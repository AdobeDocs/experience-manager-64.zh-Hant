...
seo-description: 智慧型影像處理運用每位使用者獨特的檢視特性，自動提供最適合其體驗的影像，進而提升效能和參與度。
...
智慧型影像技術使用Adobe SenseiAI功能，並與現有的「影像預設集」搭配使用，根據用戶端瀏覽器功能自動最佳化影像格式、大小和品質，以增強影像傳送效能。
...
|---|---|---|---|---|
| [影像1](https://techsupport.scene7.com/is/image/TechSupport/SmartImaging_6?hei=500&amp;fmt=jpg&amp;qlt=85&amp;resmode=bisharp&amp;op_usm=5,0.125,5,0) | ![pictore1](assets-dm/picture1.png) | 73.75 KB | 45.92 KB | 38% |
| [影像2](https://techsupport.scene7.com/is/image/TechSupport/SmartImaging_3?hei=500&amp;fmt=jpg&amp;qlt=85&amp;resmode=bisharp&amp;op_usm=5,0.125,5,0) | ![pictore2](assets-dm/picture2.png) | 191 KB | 70.66 KB | 63% |
...
否. Smart Imaging隨附於您現有的Dynamic Media經典或Adobe Experience Manager(Dynamic Media)授權(On-prem、AMS和Experience Manager，作為Cloud Service)。
...
智慧型影像功能可與您現有的「影像預設集」搭配使用，如果要求的檔案格式為JPEG或PNG，則會觀察除品質(`qlt`)和格式(`fmt`)以外的所有影像設定。 對於格式轉換，Adobe會依照影像預設集設定所定義，維持完整的視覺完整性，但檔案大小較小。 如果原始影像大小小於智慧型影像產生的大小，則會提供原始影像。
...
1. 在您的支援案例中提供下列資訊：
   ...
   1. 主要聯絡人姓名、電子郵件、電話。
   1. 要啟用智慧映像的所有域（即i`mages.company.com`或`mycompany.scene7.com`）。
...
在初始轉換期間，非快取的影像會直接點擊Adobe的原始伺服器，直到重新建立快取為止。 因此，Adobe計劃一次處理幾次客戶轉場，以便在從Adobe的來源提取請求時仍能維持可接受的效能。 對於大部分客戶，快取會在約1 - 2天內在CDN中重新完整建立。
...
>[!NOTE]
>
>並非所有影像都會轉換。 Smart Imaging決定轉換是否改善了效能。 有時，若沒有預期的效能提升，或格式不是JPEG或PNG，則不會轉換影像。
