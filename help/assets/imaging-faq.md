...
否. 您現有的Dynamic Media Classic或Adobe Experience Manager - Dynamic Media授權(內部部署、AMS和Experience Manager作為Cloud Service)包含智慧型影像。
...
智慧影像處理可與您現有的「影像預設集」搭配使用，如果請求的檔案格式為JPEG或PNG，則會觀察除質量(`qlt`)和格式(`fmt`)之外的所有影像設定。 對於格式轉換，Adobe會維持由影像預設集設定所定義的完整視覺保真度，但檔案大小較小。 如果原始影像大小小於智慧型影像產生的大小，則會提供原始影像。
