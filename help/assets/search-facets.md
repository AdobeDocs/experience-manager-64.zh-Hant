...
1. 從「搜尋Forms」頁面，選取「資產管理搜尋邊欄」**[!UICONTROL ，然後點選「**&#x200B;編輯&#x200B;**![aemassets_edit](assets/aemassets_edit.png)」。]**
   ...
   >[!NOTE]
   >
   >若要使用舊版中預先設定的&#x200B;**資產管理搜尋邊欄**&#x200B;的資料夾搜尋功能，請執AEM行下列步驟：
   > 
...
1. 點選／按一下&#x200B;**[!UICONTROL Preview]** ![preview](assets/preview.png)，產生在新增謂語後所顯示的「篩選」面板預覽。
1. 在「預覽」模式中查看謂語的佈局。
...
1. 若要關閉預覽，請點選／按一下預覽右上角的&#x200B;**[!UICONTROL Close]** ![close](assets/close.png)。
...
1. 執行下列任一項作業：
   ...
   * 點選「選項」欄位旁的「新增」圖示![，以指定您要在「篩選」面板中提供的選項的顯示文字和值。 ](assets/do-not-localize/aem_assets_add_icon.png)若要新增其他選項，請點選／按一下「新增圖示![」，然後重複此步驟。](assets/do-not-localize/aem_assets_add_icon.png)
...
「多值屬性」述詞可讓您搜尋資產以尋找多個值。 假設您在AEM Assets有多個產品的影像，而每個影像的中繼資料包含與產品相關的SKU編號。 您可以使用此謂語，根據多個SKU編號搜尋產品影像。
...
「標籤謂語」可讓您執行資產的標籤搜尋。 依預設，AEM Assets會根據您指定的標籤，搜尋一或多個標籤符合的資產。 換句話說，搜索查詢使用指定的標籤執行OR操作。 不過，您可以使用「符合所有標籤」選項來搜尋包含您所指定之所有標籤的資產。
...
1. 指定您要依據其搜尋資產或從建議清單中選取的標籤。
   ...
   ![AEM輸入標籤名稱時提供的建議](assets/chlimage_1-419.png)
...
| [!UICONTROL 路徑瀏覽器] | 搜尋謂詞，以搜尋預先設定根路徑的檔案夾和子檔案夾中的資產 | <ul><li>預留位置</li><li>根路徑</li><li>說明</li></ul> |
...
| 動作 | 權限 |
|---|---|
| [!UICONTROL 編輯] | CRX中/apps節點的讀取和寫入權限 |
| [!UICONTROL 刪除] | CRX中/apps節點的讀取、寫入和刪除權限 |
| [!UICONTROL 預覽] | 在CRX中讀取、寫入和刪除/var/dam/content節點的權限。 此外，「/apps」節點上的「讀取」和「寫入」權限。 |
