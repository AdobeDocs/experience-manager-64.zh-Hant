...
若要套 [用預先定義的標籤](/help/sites-administering/tags.md)，請在「頁面屬性 **」視窗中使用欄**`Tags/Keywords` 位下拉式清單，從頁面允許的標籤清單中選取。 Tthe **Standard Tags** tab is the default namespace, which means there is no `namespace-string:` prefixed to the taxonomy.
