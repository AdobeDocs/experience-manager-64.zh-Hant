...
* 網站編輯
* 網站管理
* [建立和編輯巢狀群組](groups.md) （子社區）

請參閱 [開始使用AEM Communities](getting-started.md) 若要體驗在製作環境中建立社群網站的速度，以及如何從製作和發佈環境建立社群群組。

>[!NOTE]
>
>用於建立的 [社群網站](sites-console.md), [社群網站範本](sites.md), [社群群組範本](tools-groups.md) 和 [社群功能](functions.md) 僅供製作環境使用。
...
* 確認一或多個發佈執行個體執行中
* 啟用 [隧道服務](deploy-communities.md#tunnel-service-on-author) 管理成員和成員組
* 識別 [主要發佈者](deploy-communities.md#primary-publisher)
* [配置複製](deploy-communities.md#replication-agents-on-author) 當主發佈伺服器埠不是預設埠時(4503)

最佳實務是，為確保網站已準備好支援許多功能，請採取下列步驟：

* 安裝 [最新功能套件](deploy-communities.md#latestfeaturepack)
* 啟用 [Adobe Analytics](analytics.md) 針對AEM Communities
* 設定 [電子郵件](email.md)
* 識別 [社群管理員](users.md#creating-community-members)
* [啟用OAuth處理常式](social-login.md#adobe-granite-oauth-authentication-handler) 用於社交登入
...
例如，下圖顯示主要的Communities Sites控制台，顯示兩個社群網站的資料夾： [啟用](getting-started-enablement.md) 和 [參與](getting-started.md):

![chlimage_1-448](assets/chlimage_1-448.png)

## 建立網站 {#site-creation}

網站建立主控台提供逐步方法，以根據選取的 [社群網站範本](sites.md) 和設定。
...
* **[!UICONTROL 社群網站範本]** 功能表：使用下拉式功能表來選擇可用 [社群網站範本](tools.md).
...
>
>此公約適用於發佈端 [使用者和使用者群組](users.md) （成員和成員群組），不會在製作環境中重複。
>
>因此，在製作環境中建立社群網站並指派受信任成員至各種角色時，必須從發佈環境中擷取成員資料。
>
>這是透過啟用 ` [AEM Communities Publish Tunnel Service](deploy-communities.md#tunnel-service-on-author)`供作者使用。
...
>[!NOTE]
>
>建議 [啟用社群網站](overview.md#enablement-community) 設為私人（如需詳細資訊，請連絡您的帳戶代表）。
>
...
* **[!UICONTROL 允許匿名存取]**
   ...
   如果選中，則社區站點為 *open* 任何網站訪客都可存取該網站。
...
* **[!UICONTROL 允許社交登入: Facebook]**
   ...
   若勾選此選項，允許網站訪客以其Facebook帳戶憑證登入。 選取的 [Facebook雲端組態](social-login.md#create-a-facebook-connect-cloud-service) 應配置為在建立社區站點後將用戶添加到社區站點的成員組。
...
* **[!UICONTROL 允許社交登入: Twitter]**
   ...
   若勾選此選項，允許網站訪客以其Twitter帳戶憑證登入。 選取的 [Twitter雲端組態](social-login.md#create-a-twitter-connect-cloud-service) 應配置為在建立社區站點後將用戶添加到社區站點的成員組。
...
>**[!UICONTROL 允許社交登入]**
>
>雖然範例Facebook和Twitter設定可能存在，且可供選取， [生產環境](../../help/sites-administering/production-ready.md)，則必須建立自訂Facebook和Twitter應用程式。 請參閱 [使用Facebook和Twitter進行社交登入](social-login.md).
...
此外，為社群網站選取標籤命名空間會限制定義目錄和資源時顯示的選取範圍。 請參閱 [標籤啟用資源](tag-resources.md) 以了解重要資訊。
...
* **[!UICONTROL 社群有特殊權限的成員]**
   ...
   開始鍵入以選擇一個或多個社區成員或成員組，以便在 `Allow Privileged Member` 已為 [社群功能](functions.md).
...
* **[!UICONTROL 啟動 Analytics]**
   ...
   僅限Adobe Analytics [已配置](analytics.md) （適用於社群功能）。
...
* **[!UICONTROL 雲端設定框架引用]**
   ...
   `Communities`是來自的架構範例 [Communities功能的Analytics設定](analytics.md#aem-analytics-framework-configuration) 檔案。
...
* **[!UICONTROL 允許機器翻譯]**
若勾選（預設為未勾選），網站內的UGC就會啟用機器翻譯。 這不會影響任何其他內容，例如頁面內容，即使網站設定為多語言網站亦然。 請參閱 [轉譯使用者產生的內容](translate-ugc.md) 以取得有關為AEM Communities設定授權翻譯服務的資訊。 請參閱 [轉譯多語言網站的內容](../../help/sites-administering/translation.md) 以取得完整概述。

![chlimage_1-456](assets/chlimage_1-456.png)

* **[!UICONTROL 為選取的語言啟用機器翻譯]**

   為機器翻譯啟用的語言預設為 [翻譯整合設定](translate-ugc.md#translation-integration-configuration). 通過刪除預設值和/或從下拉菜單中選擇其他語言，可以覆蓋此站點的這些預設設定。
...
此 `ENABLEMENT`選擇的社區站點模板包括 [指派函式](functions.md#assignments-function)，啟用功能授權後即可使用， [已配置](enablement.md). 包括分配功能的參考站點模板為 `Reference Structured Learning Site Template.`

* **[!UICONTROL 啟用管理員]**

   （必要）僅限 `Community Enablementmanagers` 可選擇組以管理此啟用社區。 啟用管理員負責將成員指派給資源。 另請參閱 [管理使用者和使用者群組](users.md).

* **[!UICONTROL Marketing Cloud 組織 ID]**

   （選用） [視訊心率分析](analytics.md#video-heartbeat-analytics) 授權。
...
* 您可以編輯 [結構](#modify-structure) 通過修改其屬性
...
網站的內容可使用與任何其他AEM網站相同的工具製作。 若要開啟網站進行製作，請選取 `Open Site` 顯示在滑鼠暫留網站上的iconm。 該站點將在新頁簽中開啟，以便Communities Sites控制台仍可訪問。
...
* 在網站結構中的社群函式例項上：
   ...
   * **`gear icon`**
      ...
      和 [特權成員組](users.md#privilegedmembersgroups)
...
發佈社群網站後，必須個別發佈使用 [群組主控台](groups.md).
...
>* 社群 — *&lt;site-name>*-*&lt;uid>* — 成員
>
建立，則會套用 [Facebook雲端服務](social-login.md#createafacebookcloudservice) 應已設定為將使用者新增至此群組。
