...
* 具有[特權](#privileged-members-group)以建立部落格、日曆、QnA和論壇條目

在發佈環境中註冊的使用者通常稱為&#x200B;*社群成員（成員）*，以區分他們與作者環境中的*users *。
...
在發佈環境中，根據社群網站的[設定](sites-console.md#user-management)，網站訪客可能成為社群成員
...
群群組社群群組是可在社群網站的發佈環境中建立的子社群，支援社群群組的建立。建立社群群組會產生更多頁面新增至網站，並以類似其父社群網站的方式進行管理。 如需詳細資訊，請造訪開發人員的[社群群組基本資訊](essentials-groups.md)，以及作者的[社群群組](creating-groups.md)。

* **[成員](../../help/sites-administering/security.md)**
組成員組是成員可能屬於的組，並通過組控制台進行管理。本頁討論的大部分內容都是關於成員組的。 為社區站點自動建立的成員組（前置詞*`Community`*）可以稱為社區組，因此必須考慮討論的上下文。
