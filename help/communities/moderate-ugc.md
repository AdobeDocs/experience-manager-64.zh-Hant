...
* [大量協調主控台](moderation.md)
   ...
   管理員和社群協調者可在公共環 [境中存取](users.md) 「協調」主控台，也可在作者環境中存取管理員。 當社群內容儲存在共用商店時，就 [可能如此](working-with-srp.md)。
...
* `Moderator`
社群協調者 [群組的成員](users.md#publishenvironmentusersandgroups) (具有協 [調者權限](in-context.md#moderatorpermissions))
* `Creator`\
   張貼內容的使用者
* `Member`\
   不具特殊權限的登入使用者
...
* [論壇](forum.md)[理念](ideation-feature.md)Qn A [，日曆與](working-with-qna.md)[Tab的設定](calendar.md) , ******[!UICONTROL 檢查協調的]**
...
1. 開 [啟Web Console](http://localhost:4502/system/console/configMgr)，請前往 `/system/console/configMgr`。

1. 找到 **[!UICONTROL AEM Communities Auto Moderation]** （AEM社群自動協調）設定並加以編輯。
1. 添加條 `SpamProcess` 目。
...
在大量 [協調控制台](moderation.md)，可以根據情緒是否為負面、中性或正面來篩選和檢視UGC。
...
也 **可以修改sentimentprocess.name** ，以參考自訂情緒規則集的位置。
