...
當包含[所需的客戶端庫](reviews-basics.md#essentials-for-client-side)時，`Reviews`元件將如此顯示。
...
* **[!UICONTROL 允許]**
回覆如果勾選，則允許回覆至審核。預設為未勾選。

* **[!UICONTROL 已]**
關閉如果選中，則新的審閱和回覆將關閉審閱。預設為未勾選。

* **[!UICONTROL 允許上]**
傳檔案如果勾選，則允許上傳檔案附件以供審核。預設為未勾選。

* **[!UICONTROL 最大檔案大]**
小僅在勾選「允 **[!UICONTROL 許檔案上]** 載」時相關。此欄位會限制已上傳檔案的大小（以位元組為單位）。 預設值為10 MB。

* **[!UICONTROL 最大消]**
息長度可輸入到文本框中的字元數上限。預設為4096個字元。

* **[!UICONTROL 僅在勾選「允]**
許檔案上傳」時，才 **[!UICONTROL 允許檔案]** 類型相關。以逗號分隔的副檔名清單，並以&quot;dot&quot;分隔。 例如：.jpg、.jpeg、.png、.doc、.docx、.pdf。 如果指定了任何檔案類型，則不允許指定那些未指定的檔案類型。 未指定預設值，因此允許所有檔案類型。

* **[!UICONTROL Rich Text]**
Editor如果勾選，則可輸入含有標籤的貼文。預設為未勾選。

* **[!UICONTROL 允許]**
投票如果選中，則包含主題的投票功能。預設為未勾選。
...
* **[!UICONTROL 預先協]**
調如果勾選，審核必須先經過核准，才會顯示在發佈網站上。預設為未勾選。

* **[!UICONTROL 刪除]**
審核如果選中，則發佈審核的成員將能夠刪除它。預設為未勾選。

* **[!UICONTROL 拒絕]**
審核如果勾選，則允許協調者拒絕審核。預設為未勾選。

* **[!UICONTROL 關閉／重新開啟]**
審核如果勾選，則允許協調者關閉並重新開啟審核。預設為未勾選。

* **[!UICONTROL 標籤]**
審核如果選中，允許成員將審核標籤為不適當。預設為未勾選。

* **[!UICONTROL 標幟原]**
因清單如果勾選，允許成員從下拉式清單中選擇將審核標籤為不適當的原因。預設為未勾選。

* **[!UICONTROL 自訂標幟原]**
因如果勾選，允許成員輸入自己將審核標籤為不適當的原因。預設為未勾選。

* **[!UICONTROL 協調]**
臨界值輸入成員在通知協調者之前必須標籤審核的次數。預設值為一次(1)。

* **[!UICONTROL 標幟]**
限制輸入審核在隱藏於公開檢視之前必須標幟的次數。此數字必須大於或等於**[!UICONTROL 協調臨界值]**。 預設值為5。
...
* **[!UICONTROL 注釋系統資源類型]**
   ...
   導覽至/apps中延伸`comments`元件（注釋系統）的resourceType。 保留空白，除非頁面範本[動態地在基礎指令碼中包含](scf.md#add-or-include-a-communities-component)注釋系統，而不是以資源（注釋節點）的形式新增至頁面。 閱讀[{{include}} helper](handlebars-helpers.md#include)的相關資訊
