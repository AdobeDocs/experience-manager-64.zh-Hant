...
* **[!UICONTROL 資源名稱(&amp;A);]**
   ...
   (*required*)資源的顯示名稱。 從顯示名稱中建立有效的節點名稱。
...
* **[!UICONTROL 說明]**
   ...
   (*optional*)要針對啟用資源顯示的說明。

* **[!UICONTROL 小型資產]**

   (*optional*)從AEM Assets中選取。 代表發佈環境中資源（例如目錄）的縮圖影像。

* **[!UICONTROL 大型資產]**

   (*optional*)從AEM Assets中選取。 在發佈環境中代表資源的大型影像，例如資源的主要頁面上。

* **[!UICONTROL 內容片段資產]**

   (*optional*)從AEM Assets中選取。 可在發佈環境中參考，但預設未使用的內容片段。
...
* **[!UICONTROL 從我的本地檔]**
案上載從本地檔案系統上載將使用本地檔案瀏覽器選擇並上載檔案。支援的檔案類型為SCORM.zip（HTML5或SWF）、MP4視訊、SWF、PDF和影像類型(JPG、PNG、GIF、BMP)。 檔案名稱會變成資產的名稱，並新增至資產資料庫。

* **[!UICONTROL 瀏覽資產庫]**
從資產庫中選擇。選取範圍限於社群網站內可見的項目。
...
* **[!UICONTROL 添]**
加受分配者使用下拉菜單從成 [員中選擇](#members-group)  — 要註冊為學習者的用戶和用戶組（以粗體面列出）。成員登入社群網站時，其登入的啟用資源（和學習路徑）會顯示在其[Assignments](functions.md#assignments-function)頁面上。
...
* **[!UICONTROL 學習路徑名稱]**
   ...
   (*required*)學習路徑的顯示名稱。 從顯示名稱中建立有效的節點名稱。

* **[!UICONTROL 標記]**

   （*可選*）可以選擇一個或多個標籤，該標籤將學習路徑與一個或多個目錄相關聯。 請參閱[標籤啟用資源](tag-resources.md)。
...
* **[!UICONTROL 強制實施學習路徑中的排序]**
   ...
   （*選用*）若設為「開啟」，則新增啟用資源的順序為要求學習者完成學習路徑的順序。 預設值為Off。

* **[!UICONTROL 資源]**

   從為目前社群網站建立的*已發佈*啟用資源中選擇的一或多個資源。
...
若要暫時讓發佈環境中的成員無法存取啟用資源，而不要將其刪除，請使用`Unpublish`資源的世界圖示。
