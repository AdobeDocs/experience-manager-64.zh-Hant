...
>[!NOTE]
>
>預設為名稱`communities`的mongoDB資料庫不應設定為用於[節點儲存或資料（二進位）儲存](../../help/sites-deploying/data-store-config.md)的資料庫的名稱。 另請參閱AEM 6](../../help/sites-deploying/storage-elements-in-aem-6.md)中的[儲存元素。
...
若要解決錯誤，請依照[安裝標準MLS](solr.md#installing-standard-mls)的指示，確定
