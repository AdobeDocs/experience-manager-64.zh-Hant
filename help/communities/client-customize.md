...
>[!CAUTION]
>
>任何以&#x200B;**scf-js-&amp;ast;**&#x200B;為首碼的CSS類別名稱，在javascript程式碼中都有特定用途。 這些類會影響元件的狀態（例如，從隱藏切換為可見），不應覆蓋或移除。
>
...
覆蓋或覆蓋元件時，不應移除SCF指令碼中的指令碼標籤。 為在HTML中插入JSON而自動建立的SCF指令碼標籤以屬性`data-scf-json=`true來識別。
...
* /etc/clientlibs/social/hbs/&lt;元件名稱>
