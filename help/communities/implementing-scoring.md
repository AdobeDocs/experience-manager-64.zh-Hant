...
* [指派標章](#assign-and-revoke-badges) ，以識別成員在社群中的角色

* [授予會員基本標誌](#enable-scoring) ，以鼓勵其參與（建立的內容數量）
* [進階授與徽章](advanced.md) ，將會員識別為專家（建立的內容品質）

**請注意** ，預設不會 [啟用標章授予](implementing-scoring.md#main-pars-text-237875536)。
...
Communities [Badges控制台](badges.md) （Communities Badges控制台）提供新增自訂徽章的功能，這些徽章可在獲得（授予）會員或在其擔任社群中特定角色（指派）時顯示。

### 指派的標章 {#assigned-badges}

基於角色的徽章由管理員根據社區成員在社區中的角色指派給社區成員。

指派（和已喚醒）的標章會儲存在選取的 [SRP](srp.md) ，無法直接存取。 在GUI可用之前，指派角色標章的唯一方法就是使用程式碼或cURL。 如需cURL指示，請參閱標題為「指派 [和撤銷標章」的章節](#assign-and-revoke-badges)。
...
* 必須為特 [徵元件](#enable-badges-for-component) 啟用標籤
* 計分和標籤規則必 [須套用](#apply-rules-to-content) 至放置元件的頁面（或上階）
...
自訂徽章可以使用「徽章」控 [制台安裝](badges.md) ，並可在徽章規則中指派或指定。
...
計分和徽章規則僅適用於在編寫模式中編輯元件組態以啟用徽章的元件 [例項](author-communities.md)。
   ...
   <td> 
    <ul> 
     ...
     <li>如果為「進階」，則根據品質和數量指定計分引擎 
      <ul> 
       <li>需要其 <a href="advanced.md">他套件</a></li> 
      </ul> </li> 
     <li>預設值為"basic"</li> 
    </ul> </td> 
  </tr> 
...
此發行包含兩個論壇功能的評 [分規則](functions.md#forum-function) （一個分別用於論壇和論壇功能的「注釋」元件）:
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>badgingType</td> 
   <td>字串</td> 
   <td><em>（可選）</em> ，將計分引擎識別為「基本」或「進階」。 如果需要進階計分引擎，請參閱進階 <a href="advanced.md">計分和標章</a>。 預設值為「basic」。</td> 
  </tr> 
  <tr> 
   <td> 
...
可以使用成員控制台或使用cURL命令 [以寫程式方式](members.md#badges-tab) ，將標章分配給成員。
...
>[!NOTE]
>
>新的布林屬性 `allowBadges`，可啟用／停用元件例項的標章顯示。 它可透過標示為「顯示標章」的核 [取方塊，在更新的元件編輯](author-communities.md) 對話 **方塊中設定**。

**[行事歷元](calendar.md)**件SocialEvent`topic`= com/adobe/cq/social/calendar
...
**[檔案庫元件](file-library.md)**SocialEvent`topic`= com/adobe/cq/social/fileLibrary
...
**[論壇元件](forum.md)**SocialEvent`topic`= com/adobe/cq/social/forum
...
**[QnA元件](working-with-qna.md)**SocialEvent`topic`= com/adobe/cq/social/qna
...
**[評分元件](rating.md)**SocialEvent`topic`= com/adobe/cq/social/tally/rating
...
**[投票元件](voting.md)**SocialEvent`topic`= com/adobe/cq/social/tally/poting
...
使用快速入門教學課程（參與）網站， [快速嘗試計分](getting-started.md) 和標籤：
...
接著， [重新發佈](sites-console.md#publishing-the-site) 社群網站。
...
如需詳細資訊，請參閱開發人員 [的「計分與標章](configure-scoring.md) 」頁面。

如需進階計分引擎的詳細資訊，請參 [閱進階計分和徽章](advanced.md)。

可配置的Leerboard組 [件](enabling-leaderboard.md)[和函式](functions.md#leaderboard-function) ，可簡化成員在社區站點上的顯示及其分數。
