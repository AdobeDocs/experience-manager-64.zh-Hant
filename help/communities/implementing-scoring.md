...
* [指](#assign-and-revoke-badges) 定標誌，以標識成員在社區中的角色

* [基本授予會](#enable-scoring) 員以鼓勵其參與（建立的內容數量）
* [進階授與徽](advanced.md) 章，將會員識別為專家（建立的內容品質）

**請注** 意，預設不會 [啟用標章授予](implementing-scoring.md#main-pars-text-237875536)。
...
社區[標章控制台](badges.md)提供添加自定義標章的功能，這些標章可在獲得（授予）成員或成員在社區中承擔特定角色（指派）時顯示。

### 指派的標章{#assigned-badges}

基於角色的徽章由管理員根據社區成員在社區中的角色指派給社區成員。

已分配（和已喚醒）的標誌儲存在選定的[SRP](srp.md)中，不能直接訪問。 在GUI可用之前，指派角色標章的唯一方法就是使用程式碼或cURL。 如需cURL指示，請參閱[指派和廢止標章](#assign-and-revoke-badges)一節。
...
* 標籤必須為[enabled](#enable-badges-for-component)，才能使用功能元件
* 計分和標籤規則必須[applied](#apply-rules-to-content)至元件所在的頁面（或上階）
...
可使用[標章控制台](badges.md)安裝自訂標章，並可在標章規則中指定或指定。
...
* 對於每個計分規則累積的點數，請指派[標誌](#badges)（[標誌規則](#badging-rules)）
...
若要啟用計分和標章，請將屬性`scoringRules`和`badgingRules`新增至網站內容樹中的任何節點。
...
| **屬性** | **類型** | **說明** |
|---|---|---|
| badgingRules | String[] | [標籤規則](#badging-rules)的陣列清單 |
...
計分和標籤規則僅對通過在[編寫模式](author-communities.md)中編輯元件配置而啟用標籤的元件的實例有效。

布林屬性`allowBadges`會啟用／停用元件例項的標章顯示。 它可在論壇的[元件編輯對話框](author-communities.md)中配置，並通過標有&#x200B;**顯示標章**&#x200B;的複選框對元件進行注釋。
   ...
   <td> 
    <ul> 
     ...
     <li>如果為「進階」，則根據品質和數量指定計分引擎 
      <ul> 
       <li>需要<a href="advanced.md">其他軟體包</a></li> 
      </ul> </li> 
     <li>預設值為"basic"</li> 
    </ul> </td> 
  </tr> 
...
此發行包含[論壇功能](functions.md#forum-function)的兩個計分規則（論壇功能的「論壇」和「評論」元件各一個）:
...
* `rules`和`sub-rules`節點都屬於cq:Page

* `subRules`是類型為[] Stringon規則節點的屬 `jcr:content` 性
...
<table> 
 <tbody> 
  ...
  <tr> 
   <td>badgingType</td> 
   <td>字串</td> 
   <td><em>（選用）</em> 將計分引擎識別為「基本」或「進階」。如果需要進階計分引擎，請參閱<a href="advanced.md">進階計分和標章</a>。 預設值為「basic」。</td> 
  </tr> 
  <tr> 
   <td> 
...
可以使用[members console](members.md#badges-tab)或使用cURL命令以程式設計方式將標章指派給成員。
...
>*member-profile-url*
>
>* 如果[Tunnel Service](users.md#tunnel-service)已啟用，則可參考作者實例
...
>[!NOTE]
>
>新的布林屬性`allowBadges`可啟用／停用元件例項的標章顯示。 它將可通過標有&#x200B;**顯示標章**&#x200B;的複選框在更新的[元件編輯對話框](author-communities.md)中進行配置。
...
接著，[republish](sites-console.md#publishing-the-site)社群網站。
...
可配置的Lederboard [元件](enabling-leaderboard.md)和[函式](functions.md#leaderboard-function)簡化了成員在社區站點上的顯示及其分數。
