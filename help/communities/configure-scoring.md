...
例如，在中 `this.isAssigned` 搜 `/libs/social/forum/components/hbs/topic/list-item.hbs`索：
...
對此行為所做的任何變更都應在自訂指令碼中進行（覆寫或覆蓋）。 請參 [閱用戶端自訂](client-customize.md)。
...
要快速設定slinglog檔案，請執行以下操作：

1. 存取 **[!UICONTROL Adobe Experience Manager Web Console記錄檔支援]**，例如

   * http://localhost:4502/system/console/slinglog

1. 選擇 **[!UICONTROL 添加新記錄程式]**

   1. 選擇 `DEBUG` 日誌 **[!UICONTROL 級別]**
   1. 輸入日誌檔案 **[!UICONTROL 的名稱]**，例如

      * logs/scoring-debug.log
   1. 輸入兩個 **[!UICONTROL Logger]** (class)條目(使用 `+` 表徵圖)
...
當選擇的SRP是JSRP或MSRP，但不是ASRP時，可以查看與計分和標籤相關的UGC。 (如果不熟悉這些術語，請參 [閱社群內容儲存](working-with-srp.md)[和儲存資源提供者概述](srp.md)。)
...
Adobe儲存庫的開發人員可 [以使用](deploy-communities.md#LatestReleases) ，安裝版本的最新Javadoc。 請參 [閱使用Maven for Communities:Javadocs](maven.md#javadocs)。
...
1. 具有唯一ID的AEM網站（使用精靈建立的社群網站）:
   ...
   * 使用在快速入門教學課程中建立的快速入門教學課程( [參與)網站](getting-started.md)
   * 找到論壇頁面節點
...
* [榜單功能](functions.md#leaderboard-function) ，可加入社群網站或群組範本。
* [Leerboard component](enabling-leaderboard.md), the featured component of the Leerboard function, for page authoring.
