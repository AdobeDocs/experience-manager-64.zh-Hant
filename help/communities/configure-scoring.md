...
當選擇的SRP是JSRP或MSRP，但不是ASRP時，可以查看與計分和標籤相關的UGC。 （如果不熟悉這些術語，請參閱[社區內容儲存](working-with-srp.md)和[儲存資源提供商概述](srp.md)。）
...
已安裝[版本](deploy-communities.md#LatestReleases)的最新Javadoc可供Adobe資料庫的開發人員使用。 請參閱[使用Maven for Communities:Javadocs](maven.md#javadocs)。
...
對於Geometrixx Engage網站，使用者及其分數位於含有計分規則名稱、社群網站的網站ID(`engage-ba81p`)、唯一ID和使用者ID的路徑中：
...
* [Leaderboard功](functions.md#leaderboard-function) 能，可加入社群網站或群組範本。
