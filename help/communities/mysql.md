...
* 預設埠為`3306`
* 在[JDBC OSGi配置](#configure-jdbc-connections)中輸入`Connection Name`作為`datasource`名稱
...
執行指令碼後，需要刷新`Navigator`的`SCHEMAS`部分才能查看新資料庫。 使用「方案」右側的刷新表徵圖：
...
* 輸入下列值：
   ...
   * **DBC連接URIJ**: `jdbc:mysql://localhost:3306/aem63reporting` 如果MySQL伺服器與&#39;this&#39;伺服器不相同，則指定伺服器代替localhost AEM
